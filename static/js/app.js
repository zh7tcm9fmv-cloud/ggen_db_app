...
function syncBrowseCombineToggleUi(wrap,combKey){if(!wrap||!combKey)return;const isOr=S[combKey]==='or';const btn=wrap.querySelector('.browse-filter-combine-toggle');if(!btn)return;btn.classList.toggle('browse-filter-combine-toggle--or',isOr);btn.setAttribute('role','switch');btn.setAttribute('aria-checked',isOr?'true':'false');btn.title=isOr?t('browse_filter_combine_tt_or'):t('browse_filter_combine_tt_and');btn.setAttribute('aria-label',isOr?t('browse_filter_combine_aria_or'):t('browse_filter_combine_aria_and'));const sAnd=btn.querySelector('.browse-combine-seg--and');const sOr=btn.querySelector('.browse-combine-seg--or');if(sAnd)sAnd.textContent=t('browse_filter_combine_and');if(sOr)sOr.textContent=t('browse_filter_combine_or')}
