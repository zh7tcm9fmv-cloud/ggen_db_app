...
function afterBrowseCombineOpChange(ent){if(ent==='char')loadCharacters(1);else if(ent==='unit')loadUnits(1);else if(ent==='supp')loadSupporters(1)}
...
function clearLineageOnly(p){if(p==='char')S.listCharLineage=[];else if(p==='unit')S.listUnitLineage=[];else S.listSuppLineage=[];delete S['_linLblRf'+p];if(S._browseLineageNameHints)S._browseLineageNameHints[p]={};syncLineageCheckboxes(p);updateLineageFilterButtonLabel(p);if(p==='char')loadCharacters(1);else if(p==='unit')loadUnits(1);else loadSupporters(1)}
...
function invalidateBrowseFilterPanels(){S._browseFiltersByEntity=null;S._browseLineageNameHints=null;['char','unit','supp'].forEach(p=>{delete S['_linLblRf'+p];const b=document.getElementById(p+'LineageFilterBody');if(b){b.innerHTML='';delete b.dataset.populated}});['char','unit'].forEach(p=>{const s=document.getElementById(p+'SeriesFilterBody');if(s){s.innerHTML='';delete s.dataset.populated}const gr=document.getElementById(p+'SkillGrid');if(gr){gr.innerHTML='';delete gr.dataset.populated}const pan=document.getElementById(p+'SkillFilterPanel');if(pan)delete pan.dataset.populated});{const ag=document.getElementById('charAbilGrid');if(ag){ag.innerHTML='';delete ag.dataset.populated}const ap=document.getElementById('charAbilFilterPanel');if(ap)delete ap.dataset.populated}}
...
function updateLineageFilterButtonLabel(which){const p=which==='char'?'char':(which==='supp'?'supp':'unit');const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const label=document.getElementById(p+'LineageFilterLabel');const btn=document.getElementById(p+'LineageFilterBtn');if(!label||!btn)return;btn.title=t('list_filter_lineage');const ic=p==='char'?LINEAGE_TAG_ICON_CHAR:LINEAGE_TAG_ICON_UNIT;if(sel.length===0){delete S['_linLblRf'+p];label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(t('filter_source_all'))}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');const ent=S._browseFiltersByEntity;const rows=ent&&ent._lang===S.lang&&ent[p]&&Array.isArray(ent[p].lineages)?ent[p].lineages:null;const hints=(S._browseLineageNameHints&&S._browseLineageNameHints[p])||{};const parts=sel.map(id=>{const w=String(id).trim();if(!w)return '';const nm=rows&&rows.length?browseLineageRowMatchName(rows,w):'';return nm||(hints[w]||'')});if(parts.every(x=>x))delete S['_linLblRf'+p];const miss=parts.some(x=>!x);if(miss&&!S['_linLblRf'+p]){S['_linLblRf'+p]=1;void ensureBrowseFiltersMeta().finally(()=>{syncBrowseLineageNameHints(p);updateLineageFilterButtonLabel(which)})}const ellipsis='\u2026';const txt=parts.map(nm=>nm||ellipsis).join(', ');label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(txt)}</span></span>`}
...
async function toggleLineagePanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':(which==='supp'?'supp':'unit');const panel=document.getElementById(p+'LineageFilterPanel');const btn=document.getElementById(p+'LineageFilterBtn');const body=document.getElementById(p+'LineageFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(body&&!body.dataset.populated){body.innerHTML=buildLineageFilterHtml(p);body.dataset.populated='1'}syncLineageCheckboxes(p);syncBrowseLineageNameHints(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensureFilterPanelFooter(p+'LineageFilterPanel',{onClear:()=>clearLineageOnly(p),combineKey:p==='char'?'browseCombCharLineage':p==='unit'?'browseCombUnitLineage':'browseCombSuppLineage',combineEntity:p==='char'?'char':p==='unit'?'unit':'supp'})}}
...
function onLineageItemChange(p){delete S['_linLblRf'+p];const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');const checked=Array.from(document.querySelectorAll(`input[name="${p}LineageSel"].lineage-filter-item:checked`)).map(i=>i.value);S[key]=checked.length?checked:[];syncBrowseLineageNameHints(p);updateLineageFilterButtonLabel(p);if(p==='char')loadCharacters(1);else if(p==='supp')loadSupporters(1);else loadUnits(1)}
