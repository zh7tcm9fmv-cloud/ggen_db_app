...
async function fetchDetailRankingMetaForSort(type,sortKey,id){const iid=String(id);let page=1;const per=5000;let total=0,maxPages=1;while(page<=maxPages){const r=await fetch(detailRankingFetchUrlFor(type,sortKey,page,per));if(!r.ok)return null;const d=await r.json();if(page===1){total=Number(d&&d.total||0);maxPages=Math.max(1,Math.ceil((total||0)/per)||1)}const rows=Array.isArray(d&&d.rows)?d.rows:[];const idx=rows.findIndex(x=>String(x&&x.id||'')===iid);if(idx>=0)return{rank:(page-1)*per+idx+1,total:total||Number(d&&d.total||0),value:Number(rows[idx]&&rows[idx][sortKey]||0),bounds:d&&d.stat_bounds||null};if(!rows.length)break;page++}return null}
