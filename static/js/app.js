...
function btFeaturedCell(items){if(!items||!items.length)return'<span class="bt-date-cell">—</span>';let h='';items.forEach(it=>{const row={thum:it.thum,rarity:it.rarity||'N',role_icon:it.role_icon||'',acquisition_icon:it.acquisition_icon||'',special_icons:Array.isArray(it.special_icons)?it.special_icons:[],is_ultimate:!!it.is_ultimate};const dt=it.type==='character'?'character':'unit';const thumbKind=it.type==='character'?'char':'unit';h+='<div class="lr-card bt-lr-tile" role="button" tabindex="0" data-detail-type="'+escAttr(dt)+'" data-detail-id="'+escAttr(it.id)+'" onclick="openDetail(\''+dt+'\',\''+escJs(it.id)+'\')"><div class="lr-card-thumb">'+renderListThumb(row,thumbKind,56)+'</div><div class="lr-card-name">'+esc(it.name||'')+'</div></div>'});return'<div class="bt-strip">'+h+'</div>'}
...
function btFmtDuration(b){return btIsSpecialScheduleBanner(b)?'-':(b&&b.duration_label!=null?String(b.duration_label):'—')}
function btFmtScheduleSlot(raw,b){if(btIsSpecialScheduleBanner(b))return'-';return raw!=null&&String(raw).trim()!==''?String(raw):'—'}
function btFeaturedMergedCell(units,chars){return'<td class="bt-feat-stack-cell">'+'<div class="bt-feat-stack">'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(units)+'</div>'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(chars)+'</div>'+'</div>'+'</td>'}
function renderBannerTimelineTable(rows){const thcombo='<thead><tr>'+'<th class="bt-th-banner">'+esc(t('bt_col_banner'))+'</th>'+'<th class="bt-th-combo">'+esc(t('bt_col_featured'))+'</th>'+'<th class="bt-col-head bt-col-head--sched">'+esc(t('bt_col_start'))+'</th>'+'<th class="bt-col-head bt-col-head--sched">'+esc(t('bt_col_end'))+'</th>'+'<th class="bt-col-head bt-col-head--dur">'+esc(t('bt_col_duration'))+'</th>'+'</tr></thead>';let body='<tbody>';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';body+='<tr><td class="bt-banner-cell">'+img+'<div class="bt-banner-title">'+esc(b.name||'')+'</div></td>'+btFeaturedMergedCell(b.featured_units,b.featured_chars)+'<td class="bt-date-cell bt-date-cell--sched">'+esc(btFmtScheduleSlot(b.start_label,b))+'</td><td class="bt-date-cell bt-date-cell--sched">'+esc(btFmtScheduleSlot(b.end_label,b))+'</td><td class="bt-date-cell bt-date-cell--dur">'+esc(btFmtDuration(b))+'</td></tr>'});body+='</tbody>';return'<div class="bt-table-wrap"><table class="data-table bt-table">'+thcombo+body+'</table></div>'}
function renderBannerTimelineRail(rows){let h='<div class="bt-timeline-shell bt-vtl-flow">';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb bt-vtl-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const dsl=btFmtScheduleSlot(b.start_label,b);const del=btFmtScheduleSlot(b.end_label,b);h+='<article class="bt-vtl-card"><div class="bt-vtl-marker" aria-hidden="true"></div><div class="bt-vtl-body">'+'<div class="bt-vtl-head">'+'<h3 class="bt-vtl-title">'+esc(b.name||'')+'</h3>'+img+'<div class="bt-vtl-meta"><div class="bt-vtl-meta-row"><span class="bt-vtl-meta-lbl">'+esc(t('bt_col_start'))+'</span><span class="bt-vtl-meta-val">'+esc(dsl)+'</span></div><div class="bt-vtl-meta-row"><span class="bt-vtl-meta-lbl">'+esc(t('bt_col_end'))+'</span><span class="bt-vtl-meta-val">'+esc(del)+'</span></div><div class="bt-vtl-meta-row bt-vtl-meta-row--dur"><span class="bt-vtl-meta-lbl">'+esc(t('bt_col_duration'))+'</span><span class="bt-vtl-dur-val">'+esc(btFmtDuration(b))+'</span></div></div></div>'+'<div class="bt-vtl-blocks"><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_units'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_units)+'</div></section><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_chars'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_chars)+'</div></section></div></div></article>'});h+='</div>';return h}
