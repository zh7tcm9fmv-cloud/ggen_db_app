...
function invalidateBrowseFilterPanels(){S._browseFiltersByEntity=null;S._browseLineageNameHints=null;function zap(el){if(!el)return;el.innerHTML='';delete el.dataset.populated;delete el.dataset.bfPoolSig}['char','unit','supp'].forEach(p=>{delete S['_linLblRf'+p];zap(document.getElementById(p+'LineageFilterBody'))});['char','unit'].forEach(p=>{zap(document.getElementById(p+'SeriesFilterBody'));zap(document.getElementById(p+'SkillGrid'));const pan=document.getElementById(p+'SkillFilterPanel');if(pan){delete pan.dataset.populated;delete pan.dataset.bfPoolSig}});{zap(document.getElementById('charAbilGrid'));const ap=document.getElementById('charAbilFilterPanel');if(ap){delete ap.dataset.populated;delete ap.dataset.bfPoolSig}}}
...
async function toggleSkillPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'SkillFilterPanel');const btn=document.getElementById(p+'SkillFilterBtn');const grid=document.getElementById(p+'SkillGrid');const search=document.getElementById(p+'SkillFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){panel.hidden=false;btn.setAttribute('aria-expanded','true');await ensureBrowseFiltersEntity(p);if(grid&&browseFilterPanelNeedsRebuild(grid,p)){grid.innerHTML=buildSkillBrowseHtml(p);markBrowseFilterPanelBuilt(grid,p)}if(search)search.placeholder=t('list_filter_search_placeholder');syncSkillBrowseCheckboxes(p);ensureFilterPanelFooter(p+'SkillFilterPanel',{onClear:()=>clearSkillOnly(p),combineKey:p==='char'?'browseCombCharSkill':'browseCombUnitAbil',combineEntity:p})}}
...
async function toggleAbilPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'AbilFilterPanel');const btn=document.getElementById(p+'AbilFilterBtn');const grid=document.getElementById(p+'AbilGrid');const search=document.getElementById(p+'AbilFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){panel.hidden=false;btn.setAttribute('aria-expanded','true');await ensureBrowseFiltersEntity('char');if(grid&&browseFilterPanelNeedsRebuild(grid,'char')){grid.innerHTML=buildAbilBrowseHtml(p);markBrowseFilterPanelBuilt(grid,'char')}if(search)search.placeholder=t('list_filter_search_placeholder');syncAbilBrowseCheckboxes(p);ensureFilterPanelFooter(p+'AbilFilterPanel',{onClear:clearAbilOnly,combineKey:'browseCombCharTrait',combineEntity:'char'})}}
...
async function toggleLineagePanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':(which==='supp'?'supp':'unit');const panel=document.getElementById(p+'LineageFilterPanel');const btn=document.getElementById(p+'LineageFilterBtn');const body=document.getElementById(p+'LineageFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){panel.hidden=false;btn.setAttribute('aria-expanded','true');await ensureBrowseFiltersEntity(p);if(body&&browseFilterPanelNeedsRebuild(body,p)){body.innerHTML=buildLineageFilterHtml(p);markBrowseFilterPanelBuilt(body,p)}syncLineageCheckboxes(p);syncBrowseLineageNameHints(p);ensureFilterPanelFooter(p+'LineageFilterPanel',{onClear:()=>clearLineageOnly(p),combineKey:p==='char'?'browseCombCharLineage':p==='unit'?'browseCombUnitLineage':'browseCombSuppLineage',combineEntity:p==='char'?'char':p==='unit'?'unit':'supp'})}}
async function toggleSeriesPanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':'unit';const panel=document.getElementById(p+'SeriesFilterPanel');const btn=document.getElementById(p+'SeriesFilterBtn');const body=document.getElementById(p+'SeriesFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){panel.hidden=false;btn.setAttribute('aria-expanded','true');await ensureBrowseFiltersEntity(p);if(body&&browseFilterPanelNeedsRebuild(body,p)){body.innerHTML=buildSeriesFilterHtml(p);markBrowseFilterPanelBuilt(body,p)}syncSeriesCheckboxes(p);ensureFilterPanelFooter(p+'SeriesFilterPanel',{onClear:()=>clearSeriesOnly(p),combineKey:p==='char'?'browseCombCharSeries':'browseCombUnitSeries',combineEntity:p})}}
