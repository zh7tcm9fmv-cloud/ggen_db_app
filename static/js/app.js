...
function ensureRankingTopControlEnhancements(){const row=document.querySelector('#panel-ranking .ranking-sort-row');if(!row)return;const panel=document.getElementById('panel-ranking');if(!panel)return;ensureRankingExperimentalState();const dirBtn=document.getElementById('rankingDirBtn');const listSvg=`<svg class="ranking-view-svg" viewBox="0 0 18 18" aria-hidden="true"><rect x="2" y="3.5" width="14" height="2.5" rx="1"/><rect x="2" y="8" width="14" height="2.5" rx="1"/><rect x="2" y="12.5" width="14" height="2.5" rx="1"/></svg>`;const html=`<button type="button" class="ranking-view-btn" data-ranking-view="list" onclick="setRankingViewMode('list')" title="List">${listSvg}<span class="ranking-view-txt">List</span></button><button type="button" class="ranking-view-btn" data-ranking-view="podium" onclick="setRankingViewMode('podium')" title="Dashboard"><img class="ranking-view-icon" src="${imgUrl('/static/images/UI/UI_Home_Campaign_Image_01.webp')}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'"><span class="ranking-view-txt">Dashboard</span></button>`;const exp=`<div class="ranking-exp-switches"><button type="button" class="ranking-exp-switch${S.ranking.showPercentDiff?' active':''}" data-exp-key="pct" onclick="toggleRankingPercentDiffMode()" title="% diff between ranks"><span class="ranking-exp-switch-text">%</span><span class="ranking-exp-switch-track${S.ranking.showPercentDiff?' active':''}"><span class="ranking-exp-switch-thumb"></span></span></button><button type="button" class="ranking-exp-switch${S.ranking.compareMode?' active':''}" data-exp-key="cmp" onclick="toggleRankingCompareMode()" title="% compare mode"><span class="ranking-exp-switch-text">Vs</span><span class="ranking-exp-switch-track${S.ranking.compareMode?' active':''}"><span class="ranking-exp-switch-thumb"></span></span></button></div>`;let tail=[...panel.querySelectorAll('.ranking-tail-controls')].find(el=>dirBtn&&el.contains(dirBtn))||[...panel.querySelectorAll('.ranking-tail-controls')][0];[...panel.querySelectorAll('.ranking-tail-controls')].forEach(el=>{if(el!==tail)el.remove()});if(!tail){tail=document.createElement('div');tail.className='ranking-tail-controls';row.appendChild(tail)}let toggle=panel.querySelector('.ranking-view-toggle');if(!toggle){toggle=document.createElement('div');toggle.className='ranking-view-toggle';toggle.setAttribute('role','group');toggle.setAttribute('aria-label','Ranking view mode')}toggle.innerHTML=html;if(toggle.parentElement!==tail)tail.appendChild(toggle);let ex=panel.querySelector('.ranking-exp-controls');if(!ex){ex=document.createElement('div');ex.className='ranking-exp-controls'}ex.innerHTML=exp;if(ex.parentElement!==tail)tail.appendChild(ex);if(dirBtn&&dirBtn.parentElement!==tail)tail.appendChild(dirBtn);syncRankingTailControlsPlacement()}
