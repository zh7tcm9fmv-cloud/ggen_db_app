...
function renderTowerRewardSection(d){const rows=Array.isArray(d.tower_rewards)?d.tower_rewards:[];if(!rows.length)return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div class="stage-condition-item">${esc(t('none'))}</div></div>`;return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px;">${rows.map(r=>`<div class="ability-item" style="display:flex;align-items:center;gap:10px;margin:0;"><div style="width:36px;height:36px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);border-radius:8px;flex-shrink:0;overflow:hidden;">${r.icon?`<img src="${imgUrl(r.icon)}" alt="${escAttr(r.name||'')}" loading="lazy" decoding="async" style="width:28px;height:28px;object-fit:contain;display:block;" onerror="this.style.display='none'">`:'<span style="font-size:12px;color:var(--text-muted)">ITEM</span>'}</div><div style="min-width:0;display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;"><div style="min-width:0;"><div style="font-size:13px;line-height:1.3;word-break:break-word;">${esc(r.name||'')}</div>${r.description?`<div style="font-size:11px;opacity:.75;line-height:1.2;margin-top:2px;">${esc(r.description)}</div>`:''}</div><span class="stage-meta-badge" style="white-space:nowrap;">x${fmtN(r.count||0)}</span></div></div>`).join('')}</div></div>`}
