...
function renderSortieGroupRestrictions(g){let unitItems=[],charItems=[];(g.restrictions||[]).forEach(r=>{const sides=sortieRestrictionTargetSides(r);const items=collectSortieRestrictionItems(r);if(sides.unit)unitItems=unitItems.concat(items);if(sides.char)charItems=charItems.concat(items)});return`<div class="stage-restriction-split"><div class="stage-restriction-split-col">${renderSortieRestrictionPart('unit',mergeSortieRestrictionItems(unitItems))}</div><div class="stage-restriction-split-col">${renderSortieRestrictionPart('char',mergeSortieRestrictionItems(charItems))}</div></div>`}
