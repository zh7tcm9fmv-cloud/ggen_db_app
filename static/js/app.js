...
function _dcDefNpcSquadUnitBonusAmounts(u){
const o=u&&u.bonus_amounts_squad;
return o&&typeof o==='object'?o:{};
}
function _dcDefNpcSquadMsDefBonus(u){
return Math.max(0,Number(_dcDefNpcSquadUnitBonusAmounts(u).Defense)||0);
}
...
const sqDef=_dcDefNpcSquadMsDefBonus(u);
const inclSq=_dcDefNpcMapBonusesEnabled();
const defVal=inclSq?defTotal:Math.max(0,defTotal-sqDef);
const defBonDisp=inclSq?defBon:Math.max(0,defBon-sqDef);
const defPair={val:defVal,bonusLine:defBonDisp};
...
const pr=s==='Defense'?{val:defVal,bonusLine:defBonDisp}:{val:us[s]||0,bonusLine:Math.max(0,Number(ub[s])||0)};
...
const us=defUnit&&defUnit.stats_raw||{},ub=defUnit&&defUnit.bonus_amounts||{};
const uSq=defUnit&&defUnit.bonus_amounts_squad||{};
const cs=defChar&&defChar.stats_raw||{};
const squadDefBn=Math.max(0,Number(uSq.Defense)||0);
const inclSq=_dcDefNpcMapBonusesEnabled();
let unitDef=Math.max(0,Number(us.Defense)||0);
if(!inclSq)unitDef=Math.max(0,unitDef-squadDefBn);
