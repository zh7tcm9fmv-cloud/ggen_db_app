...
function syncUnitListSspForWeaponEffectFilters(){
const sel=Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff:[];
if(!sel.includes('preemptive')&&!sel.includes('range_6'))return;
if(S.listUnitSsp&&!S.listUnitSp)return;
S.listUnitSsp=true;S.listUnitSp=false;syncListUnitStatToggles();persistListStatToggles()}
function onUnitWeaponDebuffChange(){syncUnitListSspForWeaponEffectFilters();updateUnitWeaponDebuffFilterLabel();loadUnits(1)}
...
function renderBannerTimeline(d){const root=document.getElementById('bannerTimelineRoot');if(!root)return;const rows=d.banners||[];if(!rows.length){root.innerHTML='<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';requestAnimationFrame(updateBannerTimelineScrollFabVisibility);return}root.innerHTML=renderBannerTimelineTable(rows);requestAnimationFrame(()=>updateBannerTimelineScrollFabVisibility())}
async function loadBannerTimeline(){const root=document.getElementById('bannerTimelineRoot');const load=document.getElementById('bannerTimelineLoading');if(!root)return;const cacheKey=S.lang;if(S.btCacheKey===cacheKey&&S.btCacheData){renderBannerTimeline(S.btCacheData);return}if(load)load.style.display='flex';try{const r=await fetch('/api/banner_timeline?lang='+encodeURIComponent(S.lang),{credentials:'same-origin'});if(!r.ok)throw new Error('HTTP '+r.status);const data=await r.json();S.btCacheKey=cacheKey;S.btCacheData=data;renderBannerTimeline(data)}catch(e){S.btCacheKey=null;S.btCacheData=null;root.innerHTML='<div class="empty-state"><div class="empty-state-text">'+esc(String(e))+'</div></div>';requestAnimationFrame(updateBannerTimelineScrollFabVisibility)}finally{if(load)load.style.display='none'}}
