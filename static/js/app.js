...
function renderStageMapSection(d){
  ...
  const controls=S.stageMapExpanded?`<div class="stage-map-controls" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:10px;">
    ...
    <button class="toggle-map-btn ${ro?'active':''}" onclick="toggleStageMapReinforcement()"><span>Reinforcement ${ro?'ON':'OFF'}</span></button>
...
function _stageMapUnitVisible(u){if(!S.stageMapReinforcementOnly)return true;return !!(u&&u.is_initially_placed===false)}
...
function toggleStageMapReinforcement(){
  ...
  if(S.currentDetailType==='stage'){
    document.getElementById('detailStageMapContainer').innerHTML=renderStageMapSection(S.currentDetailData);
    if(S.stageMapExpanded)setTimeout(()=>fitStageMapToUnits(true),80)
  }
}
...
function renderTags(tags){if(!tags||!tags.length)return'';const rows=[...tags].sort((a,b)=>{const an=String((typeof a==='string'?a:(a&&a.name)||'')||'').toLowerCase();const bn=String((typeof b==='string'?b:(b&&b.name)||'')||'').toLowerCase();return an.localeCompare(bn)});return`<div class="detail-tags-row" style="margin-top:12px;">${rows.map(createTagHtml).join('')}</div>`}
