...
Object.assign(T.EN,{tab_banner_timeline:'Banner Timeline',bt_view_table:'Table',bt_view_timeline:'Timeline',bt_col_banner:'Banner',bt_col_featured:'Featured',bt_col_units:'Featured units',bt_col_chars:'Featured characters',bt_col_start:'Start (JST)',bt_col_end:'End (JST)',bt_col_duration:'Duration',bt_empty:'No banner data.'});Object.assign(T.TW,{tab_banner_timeline:'轉蛋時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'橫幅',bt_col_featured:'主打',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無轉蛋資料。'});Object.assign(T.HK,{tab_banner_timeline:'扭蛋時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'橫幅',bt_col_featured:'主打',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無扭蛋資料。'});const STAT_NAME_MAP={EN:{},TW:{'Ranged':'射擊','Melee':'格鬥','Awaken':'覚醒','Defense':'守備','Reaction':'反應','HP':'HP','EN':'EN','Attack':'攻擊','Mobility':'機動','Move':'移動'}};
...
function primeBtTimelineObserve(){disposeBtTimelineObs();const rail=document.querySelector('#bannerTimelineRoot .bt-vtl-flow');if(!rail)return;const reduced=typeof window.matchMedia==='function'&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;const cards=rail.querySelectorAll('.bt-vtl-card');if(reduced||!cards.length){cards.forEach(el=>el.classList.add('bt-vtl-card--visible'));return}S._btTimelineObs=new IntersectionObserver(entries=>{entries.forEach(en=>{if(en.isIntersecting)en.target.classList.add('bt-vtl-card--visible')})},{root:null,threshold:0.06,rootMargin:'0px'});cards.forEach(c=>S._btTimelineObs.observe(c))}
...
function btFeaturedCell(items,tpx){let px=tpx!=null&&tpx!==undefined?Number(tpx):52;if(Number.isNaN(px)||px<32)px=52;if(!items||!items.length)return'<span class="bt-strip-empty">—</span>';let h='';items.forEach(it=>{const row={thum:it.thum,rarity:it.rarity||'N',role_icon:it.role_icon||'',acquisition_icon:it.acquisition_icon||'',special_icons:Array.isArray(it.special_icons)?it.special_icons:[],is_ultimate:!!it.is_ultimate};const dt=it.type==='character'?'character':'unit';const thumbKind=it.type==='character'?'char':'unit';h+='<div class="lr-card bt-lr-tile" role="button" tabindex="0" data-detail-type="'+escAttr(dt)+'" data-detail-id="'+escAttr(it.id)+'" onclick="openDetail(\''+dt+'\',\''+escJs(it.id)+'\')"><div class="lr-card-thumb">'+renderListThumb(row,thumbKind,Math.round(px))+'</div><div class="lr-card-name">'+esc(it.name||'')+'</div></div>'});return'<div class="bt-strip">'+h+'</div>'}
...
function bindBtTimelineWheel(){disposeBtTimelineWheel();const shell=document.querySelector('#bannerTimelineRoot .bt-timeline-shell');if(!shell)return;function onWheel(e){if(shell.scrollWidth<=shell.clientWidth+2)return;if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){if(e.ctrlKey)return;e.preventDefault();shell.scrollLeft+=e.deltaY}}shell.addEventListener('wheel',onWheel,{passive:false});S._btWheelShell=shell;S._btWheelHandler=onWheel}
...
function btFeaturedMergedCell(units,chars){return'<td class="bt-feat-stack-cell">'+'<div class="bt-feat-stack">'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(units,62)+'</div>'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(chars,62)+'</div>'+'</div>'+'</td>'}
function renderBannerTimelineTable(rows){const thead='<thead><tr><th class="bt-th-banner">'+esc(t('bt_col_banner'))+'</th><th class="bt-th-combo">'+esc(t('bt_col_featured'))+'</th></tr></thead>';let body='<tbody>';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';body+='<tr><td class="bt-banner-cell">'+img+'<div class="bt-banner-title">'+esc(b.name||'')+'</div>'+btBannerScheduleBlock(b)+'</td>'+btFeaturedMergedCell(b.featured_units,b.featured_chars)+'</tr>'});body+='</tbody>';return'<div class="bt-table-wrap"><table class="data-table bt-table">'+thead+body+'</table></div>'}
function btRenderVtlArticle(b,pos){const thPx=56;const img=b.banner_url?'<div class="bt-banner-thumb bt-vtl-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const dsl=btFmtScheduleSlot(b.start_label,b);const del=btFmtScheduleSlot(b.end_label,b);const dur=btFmtDuration(b);const meta='<div class="bt-vtl-meta-compact">'+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_start'))+'</span> '+esc(dsl)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_end'))+'</span> '+esc(del)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_duration'))+'</span> <span class="bt-vtl-dur-inline">'+esc(dur)+'</span></span></div>';const head='<div class="bt-vtl-head"><h3 class="bt-vtl-title">'+esc(b.name||'')+'</h3>'+img+meta+'</div>';const blocks='<div class="bt-vtl-blocks"><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_units'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_units,thPx)+'</div></section><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_chars'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_chars,thPx)+'</div></section></div>';const body='<div class="bt-vtl-body">'+head+blocks+'</div>';const tip=btFmtTooltipMonth(b.start_ms);const tipA=escAttr(tip);const marker='<div class="bt-vtl-marker" title="'+tipA+'" aria-label="'+tipA+'"></div>';if(pos==='east')return'<article class="bt-vtl-card bt-vtl-card--east">'+body+marker+'</article>';if(pos==='west')return'<article class="bt-vtl-card bt-vtl-card--west">'+marker+body+'</article>';return'<article class="bt-vtl-card bt-vtl-card--solo">'+marker+body+'</article>'}
function renderBannerTimelineRail(rows){let h='<div class="bt-timeline-shell bt-vtl-flow">';const buckets=btRailBuckets(rows);buckets.forEach(bu=>{if(bu.key!=='__'){h+='<div class="bt-vtl-monthlabel">'+esc(btMonthHeadingFromKey(bu.key))+'</div>'}const list=bu.items.slice().sort((a,b)=>{const sa=a&&a.start_ms?+a.start_ms:0,sb=b&&b.start_ms?+b.start_ms:0;if(sa!==sb)return sa-sb;return String(a.gasha_id||'').localeCompare(String(b.gasha_id||''))});let i=0;while(i<list.length){const left=list[i];const right=list[i+1];const dkL=btJstDayKey(left&&left.start_ms);const dkR=right?btJstDayKey(right.start_ms):'';const canPair=right&&dkL&&dkR&&dkL===dkR;if(canPair){h+='<div class="bt-vtl-pair">';h+='<div class="bt-vtl-slot bt-vtl-slot--left">'+btRenderVtlArticle(left,'east')+'</div>';h+='<div class="bt-vtl-spine-gutter" aria-hidden="true"></div>';h+='<div class="bt-vtl-slot bt-vtl-slot--right">'+btRenderVtlArticle(right,'west')+'</div>';h+='</div>';i+=2}else{h+='<div class="bt-vtl-solo-wrap">'+btRenderVtlArticle(left,'solo')+'</div>';i+=1}}});h+='</div>';return h}
function renderBannerTimeline(d){const root=document.getElementById('bannerTimelineRoot');if(!root)return;disposeBtTimelineWheel();disposeBtTimelineObs();const rows=d.banners||[];if(!rows.length){root.innerHTML='<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';return}root.innerHTML=S.bannerTimelineView==='table'?renderBannerTimelineTable(rows):renderBannerTimelineRail(rows);if(S.bannerTimelineView==='timeline')requestAnimationFrame(()=>setTimeout(()=>{primeBtTimelineObserve();bindBtTimelineWheel()},50));else disposeBtTimelineWheel();syncBtViewToggleButtons()}
