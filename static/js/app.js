...
function ensureRankUnitWeaponRangeFilterDom(){const host=document.getElementById('rankingUnitToolbarHost');if(!host)return;const debWrap=document.getElementById('rankUnitWeaponDebuffWrap');if(!debWrap)return;if(document.getElementById('rankUnitWeaponRangeWrap'))return;const h=`<div id="rankUnitWeaponRangeWrap" class="debuff-filter-wrap"><button type="button" class="rarity-filter-btn role-filter-dd-btn" id="rankUnitWeaponRangeFilterBtn" onclick="toggleUnitWeaponRangePanel(event)" aria-expanded="false" aria-haspopup="true" title="Weapon Range"><span id="rankUnitWeaponRangeFilterLabel"><span class="source-filter-btn-plain">All Weapon Ranges</span></span> ▾</button><div id="rankUnitWeaponRangeFilterPanel" class="terrain-filter-panel unit-terrain-filter-panel" hidden><div class="filter-panel-scroll-inner"><div id="rankUnitWeaponRangeFilterBody"></div></div></div></div>`;debWrap.insertAdjacentHTML('afterend',h);updateUnitWeaponRangeFilterLabel('rankUnit')}
...
function normalizeCharAbilityBrowseRows(rows){const out=[];(rows||[]).forEach(row=>{const id=String(row&&row.id||'').trim();const nm=String(row&&row.name||'').trim();if(!id||!nm)return;const nml=nm.toLowerCase();if(id===SUPPORT_DEF_X2_FILTER_ID||id===SUPPORT_ATK_X2_FILTER_ID){out.push(row);return}const isPlainSupportDef=nml==='support defense'||nml==='支援防禦'||nml==='支援防御';const isPlainSupportAtk=((nml.includes('support attack')&&nml.includes('counter'))||nml==='支援攻擊／反擊'||nml==='支援攻擊/反擊'||nml==='支援攻撃／反撃'||nml==='支援攻撃/反撃');if(isPlainSupportDef||isPlainSupportAtk)return;out.push(row)});return out}
