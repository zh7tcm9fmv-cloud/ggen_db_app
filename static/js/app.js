...
function tStat(name){const m=STAT_NAME_MAP[S.lang];return(m&&m[name])||name}
function charGridStatLabel(k){const colMap={Ranged:'col_ranged',Melee:'col_melee',Awaken:'col_awaken',Defense:'col_defense',Reaction:'col_reaction'};if(S.lang==='TW'||S.lang==='HK'||S.lang==='JA'||S.lang==='JP'){const ck=colMap[k];return ck?t(ck):tStat(k)}const m={Ranged:'char_grid_stat_ranged',Melee:'char_grid_stat_melee',Awaken:'char_grid_stat_awaken',Defense:'char_grid_stat_defense',Reaction:'char_grid_stat_reaction'};const key=m[k];return key?t(key):tStat(k)}
...
document.addEventListener('DOMContentLoaded',async()=>{try{const _btv=sessionStorage.getItem('ggen_bt_view');if(_btv==='timeline'||_btv==='table')S.bannerTimelineView=_btv}catch(_){}try{const _bsd=sessionStorage.getItem('ggen_bt_sort');if(_bsd==='asc'||_bsd==='desc')S.btBannerSortDir=_bsd}catch(_){}loadPersistedListView();loadPersistedListGridVariant();applyModEffectDropdownIcons();await loadLangs();loadPersistedListStatToggles();applyLang();['characters','units','supporters','stages','modifications'].forEach(t=>{syncListViewToggleUI(t);applyListViewVisibility(t)});syncGridVariantBadge('char');syncGridVariantBadge('unit');buildTableHeaders();initSearchHints();syncBrowseSearchWidths();let _bwResize;window.addEventListener('resize',()=>{clearTimeout(_bwResize);_bwResize=setTimeout(()=>syncBrowseSearchWidths(),120)});initSeriesIconNav();bindSearchRecallObserver();syncListCharToggle();syncListUnitStatToggles();syncListCondToggles();primeBrowseTabIfNeeded('characters');setupKeys();initBannerTimelineExtras();initCmpSafeArea();syncCmpMobilePickChrome();initDetailPrefetchIntentHandlers();wireTbPickerBodyClicks();wireStageMapNpcClicks();const _qs=new URLSearchParams(location.search);if(!urlTabParamBlocksBrowseShortPath(_qs.get('tab'))&&applyBrowseShortPathOnLoad()){}else{_dcCheckUrlParams();_tbCheckUrlParams();stagesOrModsTabFromQueryOnLoad()}});
...
const UNIT_WEAPON_DEBUFF_DEFS=[{key:'atk_dn',labelKey:'unit_filter_wb_atk_dn'},{key:'def_dn',labelKey:'unit_filter_wb_def_dn'},{key:'enemy_def_atk',labelKey:'unit_filter_wb_enemy_def_atk'},{key:'mob_dn',labelKey:'unit_filter_wb_mob_dn'},{key:'acc_dn',labelKey:'unit_filter_wb_acc_dn'},{key:'eva_dn',labelKey:'unit_filter_wb_eva_dn'},{key:'dmg_phys',labelKey:'unit_filter_wb_dmg_phys'},{key:'dmg_beam',labelKey:'unit_filter_wb_dmg_beam'},{key:'dmg_spec',labelKey:'unit_filter_wb_dmg_spec'},{key:'wp_phys',labelKey:'unit_filter_wb_wp_phys'},{key:'wp_beam',labelKey:'unit_filter_wb_wp_beam'},{key:'wp_spec',labelKey:'unit_filter_wb_wp_spec'},{key:'range_beam',labelKey:'unit_filter_wb_range_beam'},{key:'range_phys',labelKey:'unit_filter_wb_range_phys'},{key:'range_all',labelKey:'unit_filter_wb_range_all'},{key:'mp_1',labelKey:'unit_filter_wb_mp_1'},{key:'mp_2',labelKey:'unit_filter_wb_mp_2'},{key:'mp_3',labelKey:'unit_filter_wb_mp_3'},{key:'preemptive',labelKey:'unit_filter_wb_preemptive'},{key:'map_weapon',labelKey:'unit_filter_wb_map_weapon'}];
function getUnitWeaponDebuffDefsForUi(){const defs=UNIT_WEAPON_DEBUFF_DEFS;const ok=S.weaponDebuffPresentKeys;if(ok===undefined||ok===null)return defs;if(!Array.isArray(ok))return defs;if(ok.length===0)return[];const allow=new Set(ok.map(String));return defs.filter(d=>allow.has(d.key))}
function applyWeaponDebuffPresentFromApi(d){if(!d||!Object.prototype.hasOwnProperty.call(d,'weapon_debuff_present_keys'))return false;const keys=d.weapon_debuff_present_keys;if(!Array.isArray(keys))return false;S.weaponDebuffPresentKeys=keys;const allow=new Set(keys.map(String));let sel=Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff:[];const pruned=sel.filter(k=>allow.has(String(k)));const changed=pruned.length!==sel.length;if(changed)S.listUnitWeaponDebuff=pruned;const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();updateUnitWeaponDebuffFilterLabel();syncUnitListSspForPreemptiveWeaponFilter();return changed}
function applyMechanismPresentFromApi(d){if(!d||!Object.prototype.hasOwnProperty.call(d,'mechanism_present'))return false;const rows=d.mechanism_present;if(!Array.isArray(rows))return false;S.mechanismPresentRows=rows;const allow=new Set(rows.map(r=>String(r&&r.id||'')));let sel=Array.isArray(S.listUnitMechanism)?S.listUnitMechanism:[];const pruned=sel.filter(k=>allow.has(String(k)));const changed=pruned.length!==sel.length;if(changed)S.listUnitMechanism=pruned;const body=document.getElementById('unitMechanismFilterBody');if(body)body.innerHTML=buildUnitMechanismFilterHtml();updateUnitMechanismFilterLabel();return changed}
...
function rarityKeysFor(which){return which==='unit'?['ULT',...RARITY_LIST_KEYS]:RARITY_LIST_KEYS}
function ensureUnitUltRarityRow(){if(document.getElementById('unitRarityULT'))return;const ltRow=document.getElementById('unitRarityLT');if(!ltRow)return;const wrap=ltRow.closest('.rarity-filter-row');if(!wrap)return;const row=document.createElement('div');row.className='rarity-filter-row';row.innerHTML=`<input type="checkbox" id="unitRarityULT" checked onchange="onRarityStarCheckboxChange('unit','ULT',event)"><span class="rarity-row-hit" role="button" tabindex="0" onclick="onRarityRowClick('unit','ULT',event)" onkeydown="onRarityRowKey(event,'unit','ULT')"><span class="rarity-row-one-icon" id="unitRarityRowULT"></span></span>`;wrap.parentNode.insertBefore(row,wrap)}
...
function initToolbarFilterIcons(){ensureUnitUltRarityRow();['char','unit','supp'].forEach(p=>fillRarityPanelIcons(p));['char','unit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit'].forEach(p=>fillSourcePanel(p));['char','unit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar()}
function rarityPrefix(which){return which==='char'?'char':which==='unit'?'unit':'supp'}
function rolePrefix(which){return which==='char'?'char':'unit'}
function closeAllRarityPanels(){['char','unit','supp'].forEach(p=>{const panel=document.getElementById(p+'RarityFilterPanel');const btn=document.getElementById(p+'RarityFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
function closeAllRolePanels(){['char','unit'].forEach(p=>{const panel=document.getElementById(p+'RoleFilterPanel');const btn=document.getElementById(p+'RoleFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
...
function closeAllSourcePanels(){['char','unit'].forEach(p=>{const panel=document.getElementById(p+'SourceFilterPanel');const btn=document.getElementById(p+'SourceFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
function closeAllTerrainPanels(){const panel=document.getElementById('unitTerrainFilterPanel');const btn=document.getElementById('unitTerrainFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')}
function closeAllUnitWeaponDebuffPanels(){const panel=document.getElementById('unitWeaponDebuffFilterPanel');const btn=document.getElementById('unitWeaponDebuffFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')}
function closeAllUnitMechanismPanels(){const panel=document.getElementById('unitMechanismFilterPanel');const btn=document.getElementById('unitMechanismFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')}
function closeAllLineagePanels(){['char','unit','supp'].forEach(p=>{const panel=document.getElementById(p+'LineageFilterPanel');const btn=document.getElementById(p+'LineageFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
function closeAllSeriesPanels(){['char','unit'].forEach(p=>{const panel=document.getElementById(p+'SeriesFilterPanel');const btn=document.getElementById(p+'SeriesFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
function closeAllSkillPanels(){['char','unit'].forEach(p=>{const panel=document.getElementById(p+'SkillFilterPanel');const btn=document.getElementById(p+'SkillFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})}
function closeAllAbilPanels(){const panel=document.getElementById('charAbilFilterPanel');const btn=document.getElementById('charAbilFilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')}
function closeAllFilterPanels(){closeAllRarityPanels();closeAllRolePanels();closeStageDiffPanel();closeAllSourcePanels();closeAllTerrainPanels();closeAllUnitWeaponDebuffPanels();closeAllUnitMechanismPanels();closeAllLineagePanels();closeAllSeriesPanels();closeAllSkillPanels();closeAllAbilPanels();closeModEffectPanel()}
function ensurePanelClearButton(panelId,onClear){const panel=document.getElementById(panelId);if(!panel)return;let wrap=panel.querySelector('.filter-panel-clear-wrap');if(!wrap){wrap=document.createElement('div');wrap.className='filter-panel-clear-wrap';const btn=document.createElement('button');btn.type='button';btn.className='filter-panel-clear-btn';wrap.appendChild(btn);panel.appendChild(wrap)}const btn=wrap.querySelector('button');if(!btn)return;btn.textContent=t('browse_filters_clear');btn.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();try{onClear&&onClear()}catch(_){}}}
...
function clearSourceOnly(which){const p=which==='char'?'char':'unit';const keys=['assembly','development','other'];keys.forEach(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));if(el)el.checked=true});S[p==='char'?'listCharSource':'listUnitSource']='ALL';updateSourceFilterButtonLabel(which);if(which==='char')loadCharacters(1);else loadUnits(1)}
function clearLineageOnly(p){if(p==='char')S.listCharLineage=[];else if(p==='unit')S.listUnitLineage=[];else S.listSuppLineage=[];syncLineageCheckboxes(p);updateLineageFilterButtonLabel(p);if(p==='char')loadCharacters(1);else if(p==='unit')loadUnits(1);else loadSupporters(1)}
function clearSeriesOnly(p){if(p==='char')S.listCharSeries=[];else S.listUnitSeries=[];syncSeriesCheckboxes(p);updateSeriesFilterButtonLabel(p==='char'?'char':'unit');if(p==='char')loadCharacters(1);else loadUnits(1)}
function clearSkillOnly(p){if(p==='char')S.listCharSkills=[];else S.listUnitAbilities=[];syncSkillBrowseCheckboxes(p);updateSkillBrowseFilterLabel(p);if(p==='char')loadCharacters(1);else loadUnits(1)}
function clearAbilOnly(){S.listCharAbilities=[];syncAbilBrowseCheckboxes('char');updateAbilBrowseFilterLabel('char');loadCharacters(1)}
function clearStageDiffOnly(){S.stages.difficultyFilter='ALL';fillStageDiffPanel();updateStageDifficultyFilterButtons();loadStages(1)}
function clearModEffectOnly(){S.modifications.effectFilter='ALL';fillModEffectPanel();updateModEffectFilterLabel();loadModifications(1)}
function clearUnitTerrainOnly(){S.listUnitTerrain=[];const body=document.getElementById('unitTerrainFilterBody');if(body)body.innerHTML=buildUnitTerrainFilterHtml();updateUnitTerrainFilterLabel();loadUnits(1)}
...
function onModEffectRadioChange(){const inp=document.querySelector('input[name="modEffectSel"]:checked');if(!inp)return;S.modifications.effectFilter=inp.value;updateModEffectFilterLabel();closeModEffectPanel();loadModifications(1)}
function invalidateBrowseFilterPanels(){S._browseFiltersByEntity=null;['char','unit','supp'].forEach(p=>{const b=document.getElementById(p+'LineageFilterBody');if(b){b.innerHTML='';delete b.dataset.populated}});['char','unit'].forEach(p=>{const s=document.getElementById(p+'SeriesFilterBody');if(s){s.innerHTML='';delete s.dataset.populated}const gr=document.getElementById(p+'SkillGrid');if(gr){gr.innerHTML='';delete gr.dataset.populated}const pan=document.getElementById(p+'SkillFilterPanel');if(pan)delete pan.dataset.populated});{const ag=document.getElementById('charAbilGrid');if(ag){ag.innerHTML='';delete ag.dataset.populated}const ap=document.getElementById('charAbilFilterPanel');if(ap)delete ap.dataset.populated}}
function fillRarityPanelIcons(p){rarityKeysFor(p==='unit'?'unit':(p==='char'?'char':'supp')).forEach(k=>{const row=document.getElementById(p+'RarityRow'+k);if(row)row.innerHTML=rarityFilterIconsHtml([k])});const rowLt=document.getElementById(p+'RarityRowLT');if(rowLt)rowLt.innerHTML=rarityLtFilterRowHtml(p)}
...
function onStageDiffChange(){const inp=document.querySelector('input[name="stageDiffSel"]:checked');if(!inp)return;const v=inp.value;S.stages.difficultyFilter=v==='ALL'?'ALL':v;updateStageDifficultyFilterButtons();loadStages(1)}
...
function fillSourcePanel(which){const p=which==='char'?'char':'unit';const cur=S[p==='char'?'listCharSource':'listUnitSource']||'ALL';const keys=['assembly','development','other'];const set=new Set(cur==='ALL'?keys:cur.split(',').map(x=>x.trim()).filter(Boolean));if(cur!=='ALL'&&set.size===0)keys.forEach(k=>set.add(k));keys.forEach(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));if(el)el.checked=cur==='ALL'||set.has(k)});const la=document.getElementById(p+'SourceLblAssembly');if(la)la.textContent=t(which==='char'?'filter_source_assembly_char':'filter_source_assembly');const ld=document.getElementById(p+'SourceLblDevelopment');if(ld)ld.textContent=t(which==='char'?'filter_source_development_char':'filter_source_development');const lo=document.getElementById(p+'SourceLblOther');if(lo)lo.textContent=t('filter_source_other')}
function updateSourceFilterButtonLabel(which){const p=which==='char'?'char':'unit';const cur=S[p==='char'?'listCharSource':'listUnitSource']||'ALL';const label=document.getElementById(p+'SourceFilterLabel');const btn=document.getElementById(p+'SourceFilterBtn');if(!label||!btn)return;btn.title=t('filter_source');const ic=SOURCE_FILTER_UI_ICONS;const lbl=(k)=>k==='assembly'?t(which==='char'?'filter_source_assembly_char':'filter_source_assembly'):k==='development'?t(which==='char'?'filter_source_development_char':'filter_source_development'):t('filter_source_other');if(cur==='ALL'){label.innerHTML=`<span class="source-filter-btn-all-icons"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic.assembly)}" alt="" role="presentation"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic.development)}" alt="" role="presentation"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic.other)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(' - ' + t('filter_source_all'))}</span></span>`;btn.classList.remove('active')}else{const parts=cur.split(',').map(x=>x.trim()).filter(Boolean).sort();const imgs=parts.map(pk=>`<img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic[pk])}" alt="" role="presentation">`).join('');const tx=parts.map(lbl).join(', ');label.innerHTML=imgs+`<span class="source-filter-btn-plain" style="margin-left:4px;font-size:13px;font-weight:600">${esc(tx)}</span>`;btn.classList.add('active')}}
function toggleSourcePanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':'unit';const panel=document.getElementById(p+'SourceFilterPanel');const btn=document.getElementById(p+'SourceFilterBtn');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){panel.hidden=false;btn.setAttribute('aria-expanded','true');fillSourcePanel(which);ensurePanelClearButton(p+'SourceFilterPanel',()=>clearSourceOnly(which))}}
function clearBrowseFilters(which){const p=which==='char'?'char':'unit';const qEl=document.getElementById(which==='char'?'charFilter':'unitFilter');if(qEl){qEl.value='';updateSearchHintVisibility(which==='char'?'charFilter':'unitFilter');if(qEl.classList.contains('filter-input--organic'))syncBrowseSearchWidth(qEl.id)}if(which==='char'){S.listCharSource='ALL';S.listCharLineage=[];S.listCharSeries=[];S.listCharSkills=[];S.listCharAbilities=[];S.listCharSp=false;S.listCharCond=false}else{S.listUnitSource='ALL';S.listUnitLineage=[];S.listUnitSeries=[];S.listUnitAbilities=[];S.listUnitTerrain=[];S.listUnitWeaponDebuff=[];S.listUnitMechanism=[];S.listUnitSp=false;S.listUnitSsp=false;S.listUnitCond=false}const rp=rarityPrefix(which);const ltEl=document.getElementById(p+'RarityLT');rarityKeysFor(which).forEach(k=>{const el=document.getElementById(rp+'Rarity'+k);if(el)el.checked=true});if(ltEl)ltEl.checked=true;ROLE_LIST_IDS.forEach(id=>{const el=document.getElementById(p+'Role'+id);if(el)el.checked=true});fillSourcePanel(which);syncLineageCheckboxes(p);syncSeriesCheckboxes(p);syncSkillBrowseCheckboxes(p);syncAbilBrowseCheckboxes(p);if(which==='unit')syncUnitTerrainCheckboxes();if(which==='unit')syncUnitWeaponDebuffCheckboxes();if(which==='unit')syncUnitMechanismCheckboxes();updateRarityFilterButtonLabel(which);updateRoleFilterButtonLabel(which);updateSourceFilterButtonLabel(which);if(which==='unit')updateUnitTerrainFilterLabel();if(which==='unit')updateUnitWeaponDebuffFilterLabel();if(which==='unit')updateUnitMechanismFilterLabel();updateLineageFilterButtonLabel(which==='supp'?'supp':p);updateSeriesFilterButtonLabel(p);if(which==='char'||which==='unit')updateSkillBrowseFilterLabel(p);if(which==='char')updateAbilBrowseFilterLabel(p);syncListCharToggle();syncListUnitStatToggles();syncListCondToggles();persistListStatToggles();closeAllFilterPanels();invalidateBrowseFilterPanels();if(which==='char')loadCharacters(1);else loadUnits(1)}
function applySourceFromDom(which){const p=which==='char'?'char':'unit';const keys=['assembly','development','other'];const picked=keys.filter(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));return el&&el.checked});if(picked.length===0){keys.forEach(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));if(el)el.checked=true});S[p==='char'?'listCharSource':'listUnitSource']='ALL'}else if(picked.length===3){S[p==='char'?'listCharSource':'listUnitSource']='ALL'}else{S[p==='char'?'listCharSource':'listUnitSource']=picked.slice().sort().join(',')}updateSourceFilterButtonLabel(which);if(which==='char')loadCharacters(1);else loadUnits(1)}
function onSourceCheckboxChange(which){applySourceFromDom(which)}
function onSourceRowClick(which,key,ev){if(ev){ev.preventDefault();ev.stopPropagation()}const p=which==='char'?'char':'unit';const keys=['assembly','development','other'];if(!keys.includes(key))return;keys.forEach(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));if(el)el.checked=(k===key)});applySourceFromDom(which)}
function onSourceRowKey(ev,which,key){if(ev.key!=='Enter'&&ev.key!==' ')return;ev.preventDefault();onSourceRowClick(which,key,ev)}
function getSourceQuerySuffix(which){const p=which==='char'?'char':'unit';const cur=S[p==='char'?'listCharSource':'listUnitSource']||'ALL';if(!cur||cur==='ALL')return '';return '&source='+encodeURIComponent(cur)}
...
function buildUnitTerrainFilterHtml(){const selSet=new Set((Array.isArray(S.listUnitTerrain)?S.listUnitTerrain:[]).map(v=>String(v)));const names=['Space','Atmospheric','Ground','Sea','Underwater'];let h='<div class="unit-terrain-grid">';names.forEach(name=>{const mk=(lv)=>{const token=`${name}:${lv}`;const active=selSet.has(token)?' is-active':'';const typeIc=UNIT_TERRAIN_TYPE_ICONS[name]||'';const lvIc=UNIT_TERRAIN_LEVEL_ICONS[String(lv)]||'';const lvWrap=lv===1?'unit-terrain-lv-wrap unit-terrain-lv-wrap--hyphen':'unit-terrain-lv-wrap';const lvHtml=lvIc?`<span class="${lvWrap}"><img class="source-filter-opt-icon" src="${imgUrl(lvIc)}" alt=""></span>`:'';return`<button type="button" class="unit-terrain-item${active}" onclick="onUnitTerrainRowClick('${token}',event)" onkeydown="onUnitTerrainRowKey(event,'${token}')">${typeIc?`<img class="source-filter-opt-icon" src="${imgUrl(typeIc)}" alt="">`:''}<span>${esc(unitTerrainLabel(name))}</span>${lvHtml}</button>`};h+=`<div class="unit-terrain-grid-row">${mk(1)}${mk(2)}${mk(3)}</div>`});h+='</div>';return h}
function syncUnitTerrainCheckboxes(){const body=document.getElementById('unitTerrainFilterBody');if(body)body.innerHTML=buildUnitTerrainFilterHtml()}
function toggleUnitTerrainPanel(ev){if(ev)ev.stopPropagation();const panel=document.getElementById('unitTerrainFilterPanel');const btn=document.getElementById('unitTerrainFilterBtn');const body=document.getElementById('unitTerrainFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){if(body)body.innerHTML=buildUnitTerrainFilterHtml();panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton('unitTerrainFilterPanel',clearUnitTerrainOnly)}}
function onUnitTerrainChange(){updateUnitTerrainFilterLabel();loadUnits(1)}
function onUnitTerrainRowClick(token,ev){if(ev){ev.preventDefault();ev.stopPropagation()}let sel=Array.isArray(S.listUnitTerrain)?S.listUnitTerrain.slice():[];if(sel.includes(token))sel=sel.filter(x=>x!==token);else sel.push(token);S.listUnitTerrain=sel;syncUnitTerrainCheckboxes();onUnitTerrainChange()}
...
function buildUnitWeaponDebuffFilterHtml(){const selSet=new Set((Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff:[]).map(v=>String(v)));const defs=getUnitWeaponDebuffDefsForUi();let h='<div class="unit-debuff-grid">';for(let i=0;i<defs.length;i+=2){const a=defs[i],b=defs[i+1];const mk=(d)=>{if(!d)return'<span class="unit-debuff-fill" aria-hidden="true"></span>';const active=selSet.has(d.key)?' is-active':'';return`<button type="button" class="unit-debuff-item${active}" onclick="onUnitWeaponDebuffRowClick('${d.key}',event)" onkeydown="onUnitWeaponDebuffRowKey(event,'${d.key}')"><span>${esc(t(d.labelKey))}</span></button>`};h+=`<div class="unit-debuff-grid-row">${mk(a)}${mk(b)}</div>`}h+='</div>';return h}
function syncUnitWeaponDebuffCheckboxes(){const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml()}
function toggleUnitWeaponDebuffPanel(ev){if(ev)ev.stopPropagation();const panel=document.getElementById('unitWeaponDebuffFilterPanel');const btn=document.getElementById('unitWeaponDebuffFilterBtn');const body=document.getElementById('unitWeaponDebuffFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton('unitWeaponDebuffFilterPanel',clearUnitWeaponDebuffOnly)}}
function syncUnitListSspForPreemptiveWeaponFilter(){const sel=Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff:[];if(!sel.includes('preemptive'))return;if(S.listUnitSsp&&!S.listUnitSp)return;S.listUnitSsp=true;S.listUnitSp=false;syncListUnitStatToggles();persistListStatToggles()}
function onUnitWeaponDebuffChange(){syncUnitListSspForPreemptiveWeaponFilter();updateUnitWeaponDebuffFilterLabel();loadUnits(1)}
function onUnitWeaponDebuffRowClick(key,ev){if(ev){ev.preventDefault();ev.stopPropagation()}let sel=Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff.slice():[];if(sel.includes(key))sel=sel.filter(x=>x!==key);else sel.push(key);S.listUnitWeaponDebuff=sel;syncUnitWeaponDebuffCheckboxes();onUnitWeaponDebuffChange()}
...
function updateUnitWeaponDebuffFilterLabel(){const label=document.getElementById('unitWeaponDebuffFilterLabel');const btn=document.getElementById('unitWeaponDebuffFilterBtn');if(!label||!btn)return;btn.title=t('unit_filter_weapon_debuff');let sel=S.listUnitWeaponDebuff;if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_weapon_debuff_all'))}</span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){const def=UNIT_WEAPON_DEBUFF_DEFS.find(d=>d.key===sel[0]);const txt=def?t(def.labelKey):String(sel[0]);label.innerHTML=`<span class="source-filter-btn-plain" style="font-size:13px;font-weight:600">${esc(txt)}</span>`;return}label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_weapon_debuff_multi').replace('{n}',String(sel.length)))}</span>`}
...
function clearUnitWeaponDebuffOnly(){S.listUnitWeaponDebuff=[];const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();updateUnitWeaponDebuffFilterLabel();loadUnits(1)}
function buildUnitMechanismFilterHtml(){const rows=Array.isArray(S.mechanismPresentRows)?S.mechanismPresentRows:[];const selSet=new Set((Array.isArray(S.listUnitMechanism)?S.listUnitMechanism:[]).map(v=>String(v)));let h='<div class="unit-debuff-grid">';for(let i=0;i<rows.length;i+=2){const a=rows[i],b=rows[i+1];const mk=(row)=>{if(!row)return'<span class="unit-debuff-fill" aria-hidden="true"></span>';const id=String(row.id||'');const active=selSet.has(id)?' is-active':'';const ic=row.icon?`<img class="source-filter-opt-icon" src="${imgUrl(row.icon)}" alt="">`:'';return`<button type="button" class="unit-debuff-item${active}" onclick='onUnitMechanismRowClick(${JSON.stringify(id)},event)' onkeydown='onUnitMechanismRowKey(event,${JSON.stringify(id)})'>${ic}<span>${esc(row.name||id)}</span></button>`};h+=`<div class="unit-debuff-grid-row">${mk(a)}${mk(b)}</div>`}h+='</div>';return h}
function syncUnitMechanismCheckboxes(){const body=document.getElementById('unitMechanismFilterBody');if(body)body.innerHTML=buildUnitMechanismFilterHtml()}
function toggleUnitMechanismPanel(ev){if(ev)ev.stopPropagation();const panel=document.getElementById('unitMechanismFilterPanel');const btn=document.getElementById('unitMechanismFilterBtn');const body=document.getElementById('unitMechanismFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){if(body)body.innerHTML=buildUnitMechanismFilterHtml();panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton('unitMechanismFilterPanel',clearUnitMechanismOnly)}}
function onUnitMechanismChange(){updateUnitMechanismFilterLabel();loadUnits(1)}
function onUnitMechanismRowClick(id,ev){if(ev){ev.preventDefault();ev.stopPropagation()}const sid=String(id||'');let sel=Array.isArray(S.listUnitMechanism)?S.listUnitMechanism.slice():[];if(sel.includes(sid))sel=sel.filter(x=>x!==sid);else sel.push(sid);S.listUnitMechanism=sel;syncUnitMechanismCheckboxes();onUnitMechanismChange()}
function onUnitMechanismRowKey(ev,id){if(ev.key!=='Enter'&&ev.key!==' ')return;ev.preventDefault();onUnitMechanismRowClick(id,ev)}
function updateUnitMechanismFilterLabel(){const label=document.getElementById('unitMechanismFilterLabel');const btn=document.getElementById('unitMechanismFilterBtn');if(!label||!btn)return;btn.title=t('unit_filter_mechanism_tt');let sel=S.listUnitMechanism;if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_mechanism_all'))}</span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){const rows=Array.isArray(S.mechanismPresentRows)?S.mechanismPresentRows:[];const found=rows.find(r=>String(r&&r.id||'')===String(sel[0]));const txt=found?(found.name||sel[0]):String(sel[0]);const ic=found&&found.icon?`<img class="filter-inline-icon role-filter-chip" src="${imgUrl(found.icon)}" alt="" role="presentation">`:'';label.innerHTML=`${ic}<span class="source-filter-btn-plain" style="margin-left:4px;font-size:13px;font-weight:600">${esc(txt)}</span>`;return}label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_mechanism_multi').replace('{n}',String(sel.length)))}</span>`}
...
function clearUnitMechanismOnly(){S.listUnitMechanism=[];const body=document.getElementById('unitMechanismFilterBody');if(body)body.innerHTML=buildUnitMechanismFilterHtml();updateUnitMechanismFilterLabel();loadUnits(1)}
function getLineageQuerySuffix(which){const p=which==='char'?'char':(which==='supp'?'supp':'unit');const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&lineage_id='+encodeURIComponent(sel.join(','))}
function getSeriesQuerySuffix(which){const p=which==='char'?'char':'unit';let sel=S[p==='char'?'listCharSeries':'listUnitSeries'];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel)){if(String(sel).trim())sel=[String(sel)];else sel=[]}if(!sel.length)return '';return '&series_id='+encodeURIComponent(sel.join(','))}
function getSkillOrAbilityQuerySuffix(which){const p=which==='char'?'char':'unit';const key=p==='char'?'listCharSkills':'listUnitAbilities';let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';const param=p==='char'?'skill_id':'ability_id';return '&'+param+'='+encodeURIComponent(sel.join(','))}
...
function buildBrowsePoolQuery(which){if(which==='supp'){const qEl=document.getElementById('suppFilter');const q=(qEl&&qEl.value.trim())||'';const rq=getRarityQuerySuffix('supp');const linQ=getLineageQuerySuffix('supp');return `q=${encodeURIComponent(q)}${rq}${linQ}`}const qEl=document.getElementById(which==='char'?'charFilter':'unitFilter');const qRaw=(qEl&&qEl.value.trim())||'';const q=which==='unit'?expandUnitSearchQuery(qRaw):qRaw;const roleQ=getRoleQuerySuffix(which);const rq=getRarityQuerySuffix(which);const stQ=which==='unit'?getListStatQuery('unit'):'';const srcQ=getSourceQuerySuffix(which);const terrQ=which==='unit'?getUnitTerrainQuerySuffix():'';const debQ=which==='unit'?getUnitWeaponDebuffQuerySuffix():'';const mechQ=which==='unit'?getUnitMechanismQuerySuffix():'';const linQ=getLineageQuerySuffix(which);const serQ=getSeriesQuerySuffix(which);const skAb=getSkillOrAbilityQuerySuffix(which);const abQ=which==='char'?getAbilityQuerySuffix(which):'';return `q=${encodeURIComponent(q)}${roleQ}${rq}${stQ}${srcQ}${terrQ}${debQ}${mechQ}${linQ}${serQ}${skAb}${abQ}`}
function buildBrowsePoolSig(which){if(which==='supp'){const qEl=document.getElementById('suppFilter');const q=(qEl&&qEl.value.trim().toLowerCase())||'';const rq=getRarityQuerySuffix('supp');const linQ=getLineageQuerySuffix('supp');return [q,rq,linQ].join('|')}const qEl=document.getElementById(which==='char'?'charFilter':'unitFilter');const q=(qEl&&qEl.value.trim().toLowerCase())||'';const roleQ=getRoleQuerySuffix(which);const rq=getRarityQuerySuffix(which);const stQ=which==='unit'?getListStatQuery('unit'):'';const srcQ=getSourceQuerySuffix(which);const terrQ=which==='unit'?getUnitTerrainQuerySuffix():'';const debQ=which==='unit'?getUnitWeaponDebuffQuerySuffix():'';const mechQ=which==='unit'?getUnitMechanismQuerySuffix():'';const linQ=getLineageQuerySuffix(which);const serQ=getSeriesQuerySuffix(which);const skAb=getSkillOrAbilityQuerySuffix(which);const abQ=which==='char'?getAbilityQuerySuffix(which):'';return [q,roleQ,rq,stQ,srcQ,terrQ,debQ,mechQ,linQ,serQ,skAb,abQ].join('|')}
function buildSkillBrowseHtml(p){const pack=getBrowsePack(p)||{};const rows=groupBrowseRowsNoLevel(p==='char'?(pack.skills||[]):(pack.abilities||[]));const nameAttr=p==='char'?'charSkillSel':'unitSkillSel';let h='';rows.forEach((row,idx)=>{const ft=String(row.name||'').toLowerCase();if((p==='char'||p==='unit')&&row.icon){h+=`<label class="list-filter-tag-item skill-browse-row" data-skill-order="${idx}" data-filter-text="${escAttr(ft)}"><input type="checkbox" name="${nameAttr}" class="list-filter-sr skill-browse-item" value="${escAttr(String(row.id))}" onchange="onSkillBrowseItemChange('${p}')"><span class="tag-composite list-filter-tag-composite skill-browse-row-inner"><span class="tag-part-icon"><img class="skill-browse-ic" src="${imgUrl(row.icon)}" alt="" loading="lazy"></span><span class="tag-part-value">${esc(row.name)}</span></span></label>`}else{h+=`<label class="list-filter-tag-item skill-browse-row" data-skill-order="${idx}" data-filter-text="${escAttr(ft)}"><input type="checkbox" name="${nameAttr}" class="list-filter-sr skill-browse-item" value="${escAttr(String(row.id))}" onchange="onSkillBrowseItemChange('${p}')"><span class="tag-part-value">${esc(row.name)}</span></label>`}});return h}
...
function filterSkillDropdown(p,q){const grid=document.getElementById(p+'SkillGrid');if(!grid)return;const qn=(q||'').trim().toLowerCase();const allRow=grid.querySelector('.skill-browse-all-row');const labels=Array.from(grid.querySelectorAll('.list-filter-tag-item'));if(!qn){labels.forEach(el=>{el.style.display=''});const rest=labels.filter(l=>l!==allRow).sort((a,b)=>parseInt(a.getAttribute('data-skill-order')||'999999',10)-parseInt(b.getAttribute('data-skill-order')||'999999',10));rest.forEach(el=>grid.appendChild(el));if(allRow)grid.insertBefore(allRow,grid.firstChild);return}labels.forEach(el=>{const t=el.getAttribute('data-filter-text')||'';el.style.display=!qn||t.includes(qn)?'':'none'});if(p!=='char'&&p!=='unit')return;const rp=p==='char'?'char':'unit';const rarityKeys=rarityKeysFor(rp==='unit'?'unit':'char');const skillRows=labels.filter(l=>l.classList.contains('skill-browse-row')&&l.style.display!=='none');const byStem=new Map();skillRows.forEach(el=>{const lab=el.querySelector('.tag-part-value');const nm=lab?lab.textContent:'';const {stem,lv}=skillBrowseStemAndLv(nm);const stemKey=stem||String(nm).toLowerCase();const prev=byStem.get(stemKey);if(!prev||lv>prev.lv)byStem.set(stemKey,{el,lv})});labels.filter(l=>l.classList.contains('skill-browse-row')).forEach(el=>{el.style.display='none'});byStem.forEach(({el})=>{el.style.display=''});const rq=getRarityQuerySuffix(rp);const boxes=rarityKeys.map(k=>document.getElementById(rp+'Rarity'+k));const sel=rarityKeys.filter((k,i)=>boxes[i]&&boxes[i].checked);const ltEl=document.getElementById(rp+'RarityLT');const ltOn=ltEl&&ltEl.checked;const urOnly=sel.length===1&&sel[0]==='UR'&&!ltOn;const hideLowLv=rq!==''&&rq!=='&rarity=__NONE__'&&urOnly;if(hideLowLv){[...byStem.entries()].forEach(([k,v])=>{if(v.lv>0&&v.lv<=3){v.el.style.display='none';byStem.delete(k)}})}const ordered=[...byStem.entries()].filter(([,x])=>x.el.style.display!=='none').sort((a,b)=>b[1].lv-a[1].lv||a[0].localeCompare(b[0]));ordered.forEach(([,{el}])=>grid.appendChild(el));const rest=labels.filter(l=>!l.classList.contains('skill-browse-row')&&!l.classList.contains('skill-browse-all-row')&&l.style.display!=='none');rest.sort((a,b)=>parseInt(a.getAttribute('data-skill-order')||'0',10)-parseInt(b.getAttribute('data-skill-order')||'0',10));rest.forEach(el=>grid.appendChild(el));if(allRow)grid.insertBefore(allRow,grid.firstChild)}
function syncSkillBrowseCheckboxes(p){const key=p==='char'?'listCharSkills':'listUnitAbilities';let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const items=document.querySelectorAll(`input[name="${p==='char'?'charSkillSel':'unitSkillSel'}"].skill-browse-item`);if(!sel.length){items.forEach(i=>i.checked=false)}else{items.forEach(i=>{i.checked=sel.some(s=>String(s)===String(i.value))})}}
function onSkillBrowseItemChange(p){const key=p==='char'?'listCharSkills':'listUnitAbilities';const checked=Array.from(document.querySelectorAll(`input[name="${p==='char'?'charSkillSel':'unitSkillSel'}"].skill-browse-item:checked`)).map(i=>i.value);S[key]=checked.length?checked:[];updateSkillBrowseFilterLabel(p);if(p==='char')loadCharacters(1);else loadUnits(1)}
function updateSkillBrowseFilterLabel(p){const key=p==='char'?'listCharSkills':'listUnitAbilities';let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const label=document.getElementById(p+'SkillFilterLabel');const btn=document.getElementById(p+'SkillFilterBtn');if(!label||!btn)return;btn.title=p==='char'?t('list_filter_skill'):t('list_filter_ability');const pack=getBrowsePack(p);const rows=groupBrowseRowsNoLevel(p==='char'?(pack&&pack.skills||[]):(pack&&pack.abilities||[]));const allLbl=p==='char'?t('list_filter_all_skills'):t('list_filter_all_abilities');const toolbarLead=p==='char'?`<img class="skill-filter-toolbar-ic" src="${imgUrl(SKILL_BROWSE_ALL_ICON_LOCK)}" alt="" role="presentation">`:`<img class="skill-filter-toolbar-ic" src="${imgUrlPreferCdn(UNIT_SKILL_BROWSE_TOOLBAR_IC)}" alt="" role="presentation">`;if(sel.length===0){label.innerHTML=`<span class="skill-filter-toolbar-label">${toolbarLead}<span class="source-filter-btn-plain">${esc(allLbl)}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){let name=sel[0];const found=rows.find(x=>String(x.id)===String(sel[0]));if(found)name=found.name;const ic=found&&found.icon?`<img class="skill-filter-toolbar-ic" src="${imgUrl(found.icon)}" alt="" role="presentation">`:'';label.innerHTML=`<span class="skill-filter-toolbar-label">${ic}<span class="source-filter-btn-plain">${esc(name)}</span></span>`;return}label.innerHTML=`<span class="skill-filter-toolbar-label"><span class="source-filter-btn-plain">${esc(t('list_filter_lineage_multi').replace('{n}',String(sel.length)))}</span></span>`}
async function toggleSkillPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'SkillFilterPanel');const btn=document.getElementById(p+'SkillFilterBtn');const grid=document.getElementById(p+'SkillGrid');const search=document.getElementById(p+'SkillFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(grid&&!grid.dataset.populated){grid.innerHTML=buildSkillBrowseHtml(p);grid.dataset.populated='1'}if(search)search.placeholder=t('list_filter_search_placeholder');syncSkillBrowseCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton(p+'SkillFilterPanel',()=>clearSkillOnly(p))}}
function buildAbilBrowseHtml(p){const pack=getBrowsePack(p)||{};const rows=groupBrowseRowsNoLevel(pack.abilities||[]);const nameAttr=p+'AbilSel';let h='';rows.forEach((row,idx)=>{const ft=String(row.name||'').toLowerCase();if(row.icon){h+=`<label class="list-filter-tag-item skill-browse-row" data-skill-order="${idx}" data-filter-text="${escAttr(ft)}"><input type="checkbox" name="${nameAttr}" class="list-filter-sr skill-browse-item" value="${escAttr(String(row.id))}" onchange="onAbilBrowseItemChange('${p}')"><span class="tag-composite list-filter-tag-composite skill-browse-row-inner"><span class="tag-part-icon"><img class="skill-browse-ic" src="${imgUrl(row.icon)}" alt="" loading="lazy"></span><span class="tag-part-value">${esc(row.name)}</span></span></label>`}else{h+=`<label class="list-filter-tag-item skill-browse-row" data-skill-order="${idx}" data-filter-text="${escAttr(ft)}"><input type="checkbox" name="${nameAttr}" class="list-filter-sr skill-browse-item" value="${escAttr(String(row.id))}" onchange="onAbilBrowseItemChange('${p}')"><span class="tag-part-value">${esc(row.name)}</span></label>`}});return h}
function syncAbilBrowseCheckboxes(p){let sel=S.listCharAbilities;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const items=document.querySelectorAll(`input[name="${p}AbilSel"].skill-browse-item`);if(!sel.length){items.forEach(i=>i.checked=false)}else{items.forEach(i=>{i.checked=sel.some(s=>String(s)===String(i.value))})}}
function onAbilBrowseItemChange(p){const checked=Array.from(document.querySelectorAll(`input[name="${p}AbilSel"].skill-browse-item:checked`)).map(i=>i.value);S.listCharAbilities=checked.length?checked:[];updateAbilBrowseFilterLabel(p);loadCharacters(1)}
function updateAbilBrowseFilterLabel(p){let sel=S.listCharAbilities;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const label=document.getElementById(p+'AbilFilterLabel');const btn=document.getElementById(p+'AbilFilterBtn');if(!label||!btn)return;btn.title=t('list_filter_ability_char');const pack=getBrowsePack(p);const rows=groupBrowseRowsNoLevel((pack&&pack.abilities)||[]);const allLbl=t('list_filter_all_abilities_char');const toolbarLead=`<img class="skill-filter-toolbar-ic" src="${imgUrl(CHAR_ABILITY_BROWSE_ALL_ICON)}" alt="" role="presentation">`;if(sel.length===0){label.innerHTML=`<span class="skill-filter-toolbar-label">${toolbarLead}<span class="source-filter-btn-plain">${esc(allLbl)}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){let name=sel[0];const found=rows.find(x=>String(x.id)===String(sel[0]));if(found)name=found.name;const ic=found&&found.icon?`<img class="skill-filter-toolbar-ic" src="${imgUrl(found.icon)}" alt="" role="presentation">`:'';label.innerHTML=`<span class="skill-filter-toolbar-label">${ic}<span class="source-filter-btn-plain">${esc(name)}</span></span>`;return}label.innerHTML=`<span class="skill-filter-toolbar-label"><span class="source-filter-btn-plain">${esc(t('list_filter_lineage_multi').replace('{n}',String(sel.length)))}</span></span>`}
...
async function toggleAbilPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'AbilFilterPanel');const btn=document.getElementById(p+'AbilFilterBtn');const grid=document.getElementById(p+'AbilGrid');const search=document.getElementById(p+'AbilFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(grid&&!grid.dataset.populated){grid.innerHTML=buildAbilBrowseHtml(p);grid.dataset.populated='1'}if(search)search.placeholder=t('list_filter_search_placeholder');syncAbilBrowseCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton(p+'AbilFilterPanel',clearAbilOnly)}}
function getAbilityQuerySuffix(which){let sel=S.listCharAbilities;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&ability_id='+encodeURIComponent(sel.join(','))}
...
function updateLineageFilterButtonLabel(which){const p=which==='char'?'char':(which==='supp'?'supp':'unit');const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const label=document.getElementById(p+'LineageFilterLabel');const btn=document.getElementById(p+'LineageFilterBtn');if(!label||!btn)return;btn.title=t('list_filter_lineage');const ic=p==='char'?LINEAGE_TAG_ICON_CHAR:LINEAGE_TAG_ICON_UNIT;const pack=getBrowsePack(p);if(sel.length===0){label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(t('filter_source_all'))}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){let name=sel[0];if(pack&&pack.lineages){const found=pack.lineages.find(x=>String(x.id)===String(sel[0]));if(found)name=found.name}label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(name)}</span></span>`;return}if(sel.length===2){const n0=pack&&pack.lineages?pack.lineages.find(x=>String(x.id)===String(sel[0])):null;const n1=pack&&pack.lineages?pack.lineages.find(x=>String(x.id)===String(sel[1])):null;const t0=n0?n0.name:sel[0];const t1=n1?n1.name:sel[1];label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(t0+', '+t1)}</span></span>`;return}label.innerHTML=`<span class="list-filter-btn-mini"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(ic)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(t('list_filter_lineage_multi').replace('{n}',String(sel.length)))}</span></span>`}
function updateSeriesFilterButtonLabel(which){const p=which==='char'?'char':'unit';let sel=S[p==='char'?'listCharSeries':'listUnitSeries'];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel)){if(String(sel).trim())sel=[String(sel)];else sel=[]}const label=document.getElementById(p+'SeriesFilterLabel');const btn=document.getElementById(p+'SeriesFilterBtn');if(!label||!btn)return;btn.title=t('list_filter_series');const pack=getBrowsePack(p);if(sel.length===0){label.innerHTML=`<span class="list-filter-btn-mini series-filter-btn-all" title="${escAttr(t('series_filter_all_brand'))}"><img class="filter-inline-icon role-filter-chip" src="${imgUrl(SERIES_FILTER_ALL_LOGO)}" alt="" role="presentation"><span class="source-filter-btn-plain">${esc(t('filter_source_all'))}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){let name=sel[0];let icon='';if(pack&&pack.series){const found=pack.series.find(x=>String(x.id)===String(sel[0]));if(found){name=found.name;icon=found.icon||''}}if(icon){label.innerHTML=`<img class="filter-inline-icon role-filter-chip" src="${imgUrl(icon)}" alt="" role="presentation"><span class="source-filter-btn-plain" style="margin-left:4px">${esc(name)}</span>`}else{label.innerHTML=`<span class="source-filter-btn-plain">${esc(name)}</span>`}return}if(sel.length===2){const n0=pack&&pack.series?pack.series.find(x=>String(x.id)===String(sel[0])):null;const n1=pack&&pack.series?pack.series.find(x=>String(x.id)===String(sel[1])):null;const t0=n0?n0.name:sel[0];const t1=n1?n1.name:sel[1];label.innerHTML=`<span class="source-filter-btn-plain">${esc(t0+', '+t1)}</span>`;return}label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('list_filter_series_multi').replace('{n}',String(sel.length)))}</span>`}
async function toggleLineagePanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':(which==='supp'?'supp':'unit');const panel=document.getElementById(p+'LineageFilterPanel');const btn=document.getElementById(p+'LineageFilterBtn');const body=document.getElementById(p+'LineageFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(body&&!body.dataset.populated){body.innerHTML=buildLineageFilterHtml(p);body.dataset.populated='1'}syncLineageCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton(p+'LineageFilterPanel',()=>clearLineageOnly(p))}}
async function toggleSeriesPanel(which,ev){if(ev)ev.stopPropagation();const p=which==='char'?'char':'unit';const panel=document.getElementById(p+'SeriesFilterPanel');const btn=document.getElementById(p+'SeriesFilterBtn');const body=document.getElementById(p+'SeriesFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(body&&!body.dataset.populated){body.innerHTML=buildSeriesFilterHtml(p);body.dataset.populated='1'}syncSeriesCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensurePanelClearButton(p+'SeriesFilterPanel',()=>clearSeriesOnly(p))}}
function onSeriesItemChange(p){const key=p==='char'?'listCharSeries':'listUnitSeries';const checked=Array.from(document.querySelectorAll(`input[name="${p}SeriesSel"].series-filter-item:checked`)).map(i=>i.value);S[key]=checked.length?checked:[];updateSeriesFilterButtonLabel(p);if(p==='char')loadCharacters(1);else loadUnits(1)}
function onLineageItemChange(p){const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');const checked=Array.from(document.querySelectorAll(`input[name="${p}LineageSel"].lineage-filter-item:checked`)).map(i=>i.value);S[key]=checked.length?checked:[];updateLineageFilterButtonLabel(p);if(p==='char')loadCharacters(1);else if(p==='supp')loadSupporters(1);else loadUnits(1)}
...
function buildCharactersListUrl(p,pp,sp,cond){const s=S.characters;const q=document.getElementById('charFilter').value.trim();const roleQ=getRoleQuerySuffix('char');const rq=getRarityQuerySuffix('char');const stQ=listStatQChar(!!sp,!!cond);const srcQ=getSourceQuerySuffix('char');const linQ=getLineageQuerySuffix('char');const serQ=getSeriesQuerySuffix('char');const skillQ=getSkillOrAbilityQuerySuffix('char');const abilQ=getAbilityQuerySuffix('char');const gsQ=getListViewMode('characters')==='grid'&&getListGridVariant('characters')===2?'&grid_skills=1':'';return`/api/characters?lang=${S.lang}&page=${p}&per_page=${pp}&sort=${s.sort}&dir=${s.dir}&q=${encodeURIComponent(q)}${roleQ}${rq}${stQ}${srcQ}${linQ}${serQ}${skillQ}${abilQ}${gsQ}`}
function buildUnitsListApiUrl(p,pp,sp,ssp,cond){const s=S.units;const qRaw=document.getElementById('unitFilter').value.trim();const qApi=expandUnitSearchQuery(qRaw);const roleQ=getRoleQuerySuffix('unit');const rq=getRarityQuerySuffix('unit');const stQ=listStatQUnit(!!sp,!!ssp,!!cond);const srcQ=getSourceQuerySuffix('unit');const terrQ=getUnitTerrainQuerySuffix();const debQ=getUnitWeaponDebuffQuerySuffix();const mechQ=getUnitMechanismQuerySuffix();const linQ=getLineageQuerySuffix('unit');const serQ=getSeriesQuerySuffix('unit');const abQ=getSkillOrAbilityQuerySuffix('unit');const gsU=getListViewMode('units')==='grid'?'&grid_skills=1':'';return`/api/units?lang=${S.lang}&page=${p}&per_page=${pp}&sort=${s.sort}&dir=${s.dir}&q=${encodeURIComponent(qApi)}${roleQ}${rq}${stQ}${srcQ}${terrQ}${debQ}${mechQ}${linQ}${serQ}${abQ}${gsU}`}
...
function toggleListCharSp(){S.listCharSp=!S.listCharSp;syncListCharToggle();persistListStatToggles();loadCharacters(S.characters.page||1)}
function toggleListCharCond(){S.listCharCond=!S.listCharCond;syncListCondToggles();persistListStatToggles();loadCharacters(S.characters.page||1)}
function toggleListUnitSp(){if(S.listUnitSp){S.listUnitSp=false}else{S.listUnitSp=true;S.listUnitSsp=false}syncListUnitStatToggles();persistListStatToggles();loadUnits(S.units.page||1)}
function toggleListUnitSsp(){if(S.listUnitSsp){S.listUnitSsp=false}else{S.listUnitSsp=true;S.listUnitSp=false}syncListUnitStatToggles();persistListStatToggles();loadUnits(S.units.page||1)}
function toggleListUnitCond(){S.listUnitCond=!S.listUnitCond;syncListCondToggles();persistListStatToggles();loadUnits(S.units.page||1)}
...
function onRoleFilterChange(which){updateRoleFilterButtonLabel(which);if(which==='char')loadCharacters(1);else loadUnits(1)}
...
function getTabSearchPlaceholderKey(){const m={characters:'search_char',units:'search_unit',supporters:'search_supporter',stages:'search_stage',modifications:'search_mod'};return m[S.currentTab]||'search_char'}
function getTabNameForSpotlight(){const m={characters:'tab_char',units:'tab_unit',supporters:'tab_supporter',stages:'tab_stage',modifications:'tab_mod'};return t(m[S.currentTab]||'tab_char')}
function onSearchSpotlightInput(){const real=getActiveSearchInput();const inp=document.getElementById('searchSpotlightInput');if(!real||!inp)return;real.value=inp.value;updateSearchHintVisibility(real.id);if(real.classList.contains('filter-input--organic'))syncBrowseSearchWidth(real.id);const tabById={charFilter:'characters',unitFilter:'units',suppFilter:'supporters',stageFilter:'stages',modFilter:'modifications'};const tab=tabById[real.id];if(tab)debounceLoad(tab);debounceSpotlightResults()}
...
function bindSearchRecallObserver(){const fab=document.getElementById('searchRecallFab');if(S._searchRecallObs){S._searchRecallObs.disconnect();S._searchRecallObs=null}if(!fab)return;if(S.currentTab==='latest_release'||S.currentTab==='banner_timeline'||S.currentTab==='calculator'||S.currentTab==='team_builder'){fab.classList.remove('visible');return}const panel=document.getElementById('panel-'+S.currentTab);if(!panel)return;const tb=panel.querySelector('.list-toolbar');if(!tb)return;S._searchRecallObs=new IntersectionObserver(entries=>{const e=entries[0];fab.classList.toggle('visible',!!e&&!e.isIntersecting)},{root:null,threshold:0,rootMargin:'-64px 0px 0px 0px'});S._searchRecallObs.observe(tb)}
...
function syncBtViewToggleButtons(){const a=document.getElementById('btViewTableBtn'),b=document.getElementById('btViewTimelineBtn');if(a)a.classList.toggle('active',S.bannerTimelineView==='table');if(b)b.classList.toggle('active',S.bannerTimelineView==='timeline')}
function disposeBtTimelineObs(){if(S._btTimelineObs){try{S._btTimelineObs.disconnect()}catch(_){}S._btTimelineObs=null}}
function primeBtTimelineObserve(){disposeBtTimelineObs();const rail=document.querySelector('#bannerTimelineRoot .bt-ig-shell');if(!rail)return;if(rail.classList.contains('bt-ig-shell--horizontal'))return;const reduced=typeof window.matchMedia==='function'&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;const cards=rail.querySelectorAll('.bt-ig-step.bt-vtl-card');if(reduced||!cards.length){cards.forEach(el=>el.classList.add('bt-vtl-card--visible'));return}S._btTimelineObs=new IntersectionObserver(entries=>{entries.forEach(en=>{if(en.isIntersecting)en.target.classList.add('bt-vtl-card--visible')})},{root:null,threshold:0.06,rootMargin:'0px'});cards.forEach(c=>S._btTimelineObs.observe(c))}
function setBannerTimelineView(mode){const m=mode==='timeline'?'timeline':'table';S.bannerTimelineView=m;try{sessionStorage.setItem('ggen_bt_view',m)}catch(_){}syncBtViewToggleButtons();if(S.btCacheData){renderBannerTimeline(S.btCacheData)}else if(S.currentTab==='banner_timeline')loadBannerTimeline()}
...
const BT_SPECIAL_SCHEDULE_ID='9999990001';function disposeBtTimelineWheel(){if(S._btWheelShell&&S._btWheelHandler){try{S._btWheelShell.removeEventListener('wheel',S._btWheelHandler)}catch(_){}S._btWheelShell=null;S._btWheelHandler=null}}
function bindBtTimelineWheel(){disposeBtTimelineWheel()}
function btIsSpecialScheduleBanner(b){const sid=String(b&&(b.schedule_id!=null)?b.schedule_id:'').trim();return sid===BT_SPECIAL_SCHEDULE_ID}
...
function btFmtTooltipMonth(ms){if(!(ms>0))return'';const lc=(S.lang||'EN').toUpperCase();const loc=lc==='JA'||lc==='JP'?'ja':lc==='TW'||lc==='HK'?'zh-TW':'en';try{return new Intl.DateTimeFormat(loc,{timeZone:'Asia/Tokyo',month:'short',year:'numeric'}).format(new Date(ms))}catch(_){return''}}
function btFmtBannerNodeTooltip(ms){if(!(ms>0))return'';const lc=(S.lang||'EN').toUpperCase();const loc=lc==='JA'||lc==='JP'?'ja':lc==='TW'||lc==='HK'?'zh-TW':'en';try{return new Intl.DateTimeFormat(loc,{timeZone:'Asia/Tokyo',year:'numeric',month:'short',day:'numeric'}).format(new Date(ms))}catch(_){return''}}
function btJstStartDayNum(ms){if(!(ms>0))return null;try{const p=new Intl.DateTimeFormat('en',{timeZone:'Asia/Tokyo',day:'numeric'}).formatToParts(new Date(ms));const d=p.find(x=>x.type==='day');return d?parseInt(d.value,10):null}catch(_){return null}}
function btYmKey(ms){if(!(ms>0))return'__';try{const p=new Intl.DateTimeFormat('en',{timeZone:'Asia/Tokyo',year:'numeric',month:'numeric'}).formatToParts(new Date(ms));const y=p.find(x=>x.type==='year').value;const mo=p.find(x=>x.type==='month').value;return y+'-'+String(parseInt(String(mo),10)||1).padStart(2,'0')}catch(_){return'__'}}
function btJstDayKey(ms){if(!(ms>0))return'';try{return new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Tokyo',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date(ms))}catch(_){return''}}
function btMonthHeadingFromKey(k){if(!k||k==='__')return'';try{const a=k.split('-');const mo=parseInt(a[1],10)-1;if(mo<0)return k;const lc=(S.lang||'EN').toUpperCase();const loc=lc==='JA'||lc==='JP'?'ja':lc==='TW'||lc==='HK'?'zh-TW':'en';return new Intl.DateTimeFormat(loc,{month:'long',year:'numeric',timeZone:'UTC'}).format(new Date(Date.UTC(parseInt(a[0],10),mo,1)))}catch(_){return k}}
function btGroupBannersByJstDay(sorted){const out=[];const arr=sorted||[];let i=0;while(i<arr.length){const b=arr[i];if(b&&b.start_ms>0){const dk=btJstDayKey(b.start_ms);const grp=[b];i++;while(i<arr.length){const bx=arr[i];if(bx&&bx.start_ms>0&&btJstDayKey(bx.start_ms)===dk){grp.push(bx);i++}else break}out.push(grp)}else{out.push([arr[i]]);i++}}return out}
function btRailBuckets(rows){const order=[];const map=new Map();rows.forEach(b=>{const k=btYmKey(b.start_ms);if(!map.has(k)){map.set(k,[]);order.push(k)}map.get(k).push(b)});order.sort((a,b)=>{if(a==='__')return 1;if(b==='__')return -1;return b.localeCompare(a)});return order.map(k=>({key:k,items:map.get(k)}))}
...
function btBannerTimelineCardBodyHtml(b){const thPx=56;const img=b.banner_url?'<div class="bt-banner-thumb bt-vtl-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const dsl=btFmtScheduleSlot(b.start_label,b);const del=btFmtScheduleSlot(b.end_label,b);const dur=btFmtDuration(b);const meta='<div class="bt-vtl-meta-compact">'+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_start'))+'</span> '+esc(dsl)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_end'))+'</span> '+esc(del)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_duration'))+'</span> <span class="bt-vtl-dur-inline">'+esc(dur)+'</span></span></div>';const head='<div class="bt-vtl-head"><h3 class="bt-vtl-title">'+esc(b.name||'')+'</h3>'+img+meta+'</div>';let blocks='<div class="bt-vtl-blocks"><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_units'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_units,thPx)+'</div></section><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_chars'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_chars,thPx)+'</div></section>';const sups=b.featured_supporters;if(sups&&sups.length)blocks+='<section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_supporters'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(sups,thPx)+'</div></section>';blocks+='</div>';return'<div class="bt-vtl-body">'+head+blocks+'</div>'}
function btPreferHorizontalTimeline(){try{return typeof window.matchMedia==='function'&&window.matchMedia('(min-width: 1180px)').matches}catch(_){return false}}
function renderBannerTimelineRailHz(rows){const buckets=btRailBuckets(rows||[]);let h='<div class="bt-timeline-shell bt-ig-shell bt-ig-shell--horizontal"><div class="bt-hz-track" role="region">';buckets.forEach(bk=>{const items=(bk.items||[]).slice().sort((a,x)=>(+(x.start_ms||0))-(+(a.start_ms||0)));if(!items.length)return;h+='<section class="bt-hz-col">';h+='<header class="bt-hz-month"><span class="bt-hz-month-inner">'+esc(btMonthHeadingFromKey(bk.key))+'</span></header>';h+='<div class="bt-hz-banners">';items.forEach((b,ii)=>{const pal=ii%7;const dn=b&&b.start_ms>0?btJstStartDayNum(b.start_ms):null;const num=dn!=null?String(dn):'—';const tipRaw=b&&b.start_ms>0?btFmtBannerNodeTooltip(b.start_ms):(b&&(b.name||''))||'';h+='<article class="bt-hz-banner bt-vtl-card bt-vtl-card--visible bt-ig-step--pal-'+pal+'">';h+='<div class="bt-hz-banner-node bt-ig-node bt-ig-node--pal-'+pal+'" title="'+escAttr(tipRaw)+'" aria-hidden="true"><span class="bt-ig-num">'+esc(num)+'</span></div>';h+='<div class="bt-hz-banner-body">'+btBannerTimelineCardBodyHtml(b)+'</div>';h+='</article>'});h+='</div></section>'});h+='</div></div>';return h}
function renderBannerTimelineRail(rows){const sorted=btSortBannerRows(rows||[]);const groups=btGroupBannersByJstDay(sorted);let h='<div class="bt-timeline-shell bt-ig-shell"><div class="bt-ig-spine-line" aria-hidden="true"></div><div class="bt-ig-list">';let lastYm='';groups.forEach((items,idx)=>{const b0=items[0];const ym=btYmKey(b0&&b0.start_ms);if(ym!==lastYm&&ym&&ym!=='__'){lastYm=ym;h+='<div class="bt-ig-monthlabel">'+esc(btMonthHeadingFromKey(ym))+'</div>'}const isRight=idx%2===0;const dn=b0&&b0.start_ms>0?btJstStartDayNum(b0.start_ms):null;let tipRaw='';if(b0&&b0.start_ms>0){tipRaw=btFmtBannerNodeTooltip(b0.start_ms);if(items.length>1)tipRaw=tipRaw+' ('+items.length+')'}else tipRaw=(b0&&(b0.name||''))||'';const tipA=escAttr(tipRaw),num=dn!=null?String(dn):'—',pal=idx%7;const skins=items.map(b=>'<div class="bt-ig-card-skin">'+btBannerTimelineCardBodyHtml(b)+'</div>').join('');const cardInner=items.length>1?'<div class="bt-ig-cards-row">'+skins+'</div>':skins;const emp='<div class="bt-ig-zone bt-ig-zone--blank" aria-hidden="true"></div>';const left=isRight?emp:'<div class="bt-ig-zone bt-ig-zone--card bt-ig-zone--card-row">'+cardInner+'</div>';const right=isRight?'<div class="bt-ig-zone bt-ig-zone--card bt-ig-zone--card-row">'+cardInner+'</div>':emp;const node='<div class="bt-ig-rail-slot"><div class="bt-ig-node bt-ig-node--pal-'+pal+'" title="'+tipA+'" aria-label="'+tipA+'"><span class="bt-ig-num">'+esc(num)+'</span></div></div>';h+='<div class="bt-ig-step '+(isRight?'bt-ig-step--right':'bt-ig-step--left')+' bt-ig-step--pal-'+pal+' bt-vtl-card">'+left+node+right+'</div>'});h+='</div></div>';return h}
function renderBannerTimeline(d){const root=document.getElementById('bannerTimelineRoot');if(!root)return;disposeBtTimelineWheel();disposeBtTimelineObs();const rows=d.banners||[];if(!rows.length){root.innerHTML='<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';requestAnimationFrame(updateBannerTimelineScrollFabVisibility);return}const railHtml=S.bannerTimelineView==='table'?renderBannerTimelineTable(rows):(btPreferHorizontalTimeline()?renderBannerTimelineRailHz(rows):renderBannerTimelineRail(rows));root.innerHTML=railHtml;if(S.bannerTimelineView==='timeline')requestAnimationFrame(()=>setTimeout(()=>{primeBtTimelineObserve();bindBtTimelineWheel()},50));else disposeBtTimelineWheel();syncBtViewToggleButtons();requestAnimationFrame(()=>updateBannerTimelineScrollFabVisibility())}
async function loadBannerTimeline(){const root=document.getElementById('bannerTimelineRoot');const load=document.getElementById('bannerTimelineLoading');if(!root)return;const cacheKey=S.lang;if(S.btCacheKey===cacheKey&&S.btCacheData){renderBannerTimeline(S.btCacheData);return}if(load)load.style.display='flex';disposeBtTimelineWheel();disposeBtTimelineObs();try{const r=await fetch('/api/banner_timeline?lang='+encodeURIComponent(S.lang),{credentials:'same-origin'});if(!r.ok)throw new Error('HTTP '+r.status);const data=await r.json();S.btCacheKey=cacheKey;S.btCacheData=data;renderBannerTimeline(data)}catch(e){S.btCacheKey=null;S.btCacheData=null;root.innerHTML='<div class="empty-state"><div class="empty-state-text">'+esc(String(e))+'</div></div>';requestAnimationFrame(updateBannerTimelineScrollFabVisibility)}finally{if(load)load.style.display='none'}}
function scrollBannerTimelineToTop(){try{window.scrollTo({top:0,behavior:'smooth'})}catch(_){window.scrollTo(0,0)}}
function updateBannerTimelineScrollFabVisibility(){const b=document.getElementById('bannerTimelineScrollTopFab');if(!b)return;const st=window.scrollY||document.documentElement.scrollTop;const show=S.currentTab==='banner_timeline'&&st>180;b.hidden=!show}
function onBannerTimelineAdjacentWheel(e){if(S.currentTab!=='banner_timeline')return;const ae=document.activeElement;if(ae&&(ae.tagName==='INPUT'||ae.tagName==='TEXTAREA'||(typeof ae.closest==='function'&&ae.closest('[contenteditable="true"]'))))return;const doc=document.documentElement;const st=window.scrollY||doc.scrollTop;const maxScroll=Math.max(0,doc.scrollHeight-window.innerHeight);const margin=20;const dy=e.deltaY;if(dy<=-0.5&&st<=margin){e.preventDefault();switchTab('latest_release');return}if(dy>=0.5&&st>=maxScroll-margin){const g=document.getElementById('navGameNewsTab');const href=g&&g.getAttribute('href');if(href){e.preventDefault();location.assign(href)}}}
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});document.addEventListener('wheel',onBannerTimelineAdjacentWheel,{passive:false});const mql=window.matchMedia('(min-width: 1180px)');function onMqlChange(){if(S.currentTab!=='banner_timeline'||!S.btCacheData||S.bannerTimelineView!=='timeline')return;renderBannerTimeline(S.btCacheData)}if(typeof mql.addEventListener==='function')mql.addEventListener('change',onMqlChange);else if(typeof mql.addListener==='function')mql.addListener(onMqlChange)}
...
function debounceLoad(tab){clearTimeout(S.ft);S.ft=setTimeout(()=>{if(tab==='characters')loadCharacters(1);else if(tab==='units')loadUnits(1);else if(tab==='supporters')loadSupporters(1);else if(tab==='stages')loadStages(1);else if(tab==='modifications')loadModifications(1)},300)}
...
function syncBrowseSearchWidths(){syncBrowseSearchWidth('charFilter');syncBrowseSearchWidth('unitFilter')}
...
function renderStageTable(data){document.getElementById('stageBody').innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const ph=locked?'<div class="row-thum-placeholder stage-thum-locked">🔒</div>':(r.portrait?imgTag(r.portrait,{cls:'row-thum',webp:true,onerror:"this.parentElement.innerHTML='<div class=\\'row-thum-placeholder\\'>🗺️</div>'"}) :'<div class="row-thum-placeholder">🗺️</div>');const diffCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:isSa?'<span class="stat-value"></span>':`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span>`;const numCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:`<span class="stat-value">${fmtN(r.stage_number)}</span>`;const nameCell=locked?`<span class="name-text er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</span>`:`<span class="name-text">${esc(r.name)}</span>`;const cpDisp=locked?'—':fmtN(r.recommended_cp);const terDisp=locked?'—':esc(r.terrain);return`<tr data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><td class="col-thum"><div class="stage-thum-wrap">${ph}</div></td><td class="col-diff">${diffCell}</td><td class="col-number">${numCell}</td><td class="col-name"><div class="name-cell">${nameCell}</div></td><td class="col-cp"><span class="stat-value">${cpDisp}</span></td><td class="col-terrain"><span class="stat-value">${terDisp}</span></td></tr>`}).join('')}
function renderStageGrid(data){const grid=document.getElementById('stageGrid');grid.innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const ph=locked?'<div class="row-thum-placeholder stage-thum-locked">🔒</div>':(r.portrait?imgTag(r.portrait,{cls:'row-thum',webp:true,onerror:"this.parentElement.innerHTML='<div class=\\'row-thum-placeholder\\'>🗺️</div>'"}) :'<div class="row-thum-placeholder">🗺️</div>');const metaLine=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:isSa?`<span class="stat-value">#${fmtN(r.stage_number)}</span>`:`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span> <span class="stat-value">#${fmtN(r.stage_number)}</span>`;const nameLine=locked?`<div class="list-grid-card-name er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</div>`:`<div class="list-grid-card-name">${esc(r.name)}</div>`;const cpPart=locked?'—':fmtN(r.recommended_cp);const terPart=locked?'—':esc(r.terrain);return`<div class="list-grid-card list-grid-card--stage${locked?' list-grid-card--locked':''}" data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><div class="list-grid-card-thumb"><div class="stage-thum-wrap">${ph}</div></div><div class="list-grid-card-body"><div class="list-grid-card-meta">${metaLine}</div>${nameLine}<div class="list-grid-card-stats">${t('col_stage_cp')} ${cpPart} · ${t('terrain')} ${terPart}</div></div></div>`}).join('')}
...
function syncStageSourceToolbar(){const src=S.stages.source||'eternal';const diff=document.getElementById('stageDiffWrap');if(diff)diff.style.display=src==='eternal'?'':'none';const bE=document.getElementById('stageSourceEternalBtn');const bS=document.getElementById('stageSourceScoreBtn');if(bE){bE.classList.toggle('active',src==='eternal');bE.setAttribute('aria-pressed',src==='eternal'?'true':'false')}if(bS){bS.classList.toggle('active',src==='score_attack');bS.setAttribute('aria-pressed',src==='score_attack'?'true':'false')}}
function setStageBrowseSource(mode){if(mode!=='eternal'&&mode!=='score_attack')return;if((S.stages.source||'eternal')===mode)return;S.stages.source=mode;syncStageSourceToolbar();loadStages(1)}
function renderPag(pfx,data){const el=document.getElementById(`${pfx}Pagination`);const{page,total_pages}=data;if(total_pages<=1){el.innerHTML='';return}let fn;if(pfx==='char')fn='loadCharacters';else if(pfx==='unit')fn='loadUnits';else if(pfx==='supp')fn='loadSupporters';else if(pfx==='mod')fn='loadModifications';else fn='loadStages';let h='';h+=`<button class="page-btn ${page<=1?'disabled':''}" ${page>1?`onclick="${fn}(${page-1})"`:''}>◀</button>`;const mx=7;let sp=Math.max(1,page-Math.floor(mx/2));let ep=Math.min(total_pages,sp+mx-1);if(ep-sp<mx-1)sp=Math.max(1,ep-mx+1);if(sp>1){h+=`<button class="page-btn" onclick="${fn}(1)">1</button>`;if(sp>2)h+=`<span class="page-info">…</span>`}for(let i=sp;i<=ep;i++)h+=`<button class="page-btn ${i===page?'active':''}" onclick="${fn}(${i})">${i}</button>`;if(ep<total_pages){if(ep<total_pages-1)h+=`<span class="page-info">…</span>`;h+=`<button class="page-btn" onclick="${fn}(${total_pages})">${total_pages}</button>`}h+=`<button class="page-btn ${page>=total_pages?'disabled':''}" ${page<total_pages?`onclick="${fn}(${page+1})"`:''}>▶</button>`;el.innerHTML=h}
function toggleStatState(type){if(type==='sp'){if(S.spActive){S.spActive=false}else{S.spActive=true;S.sspActive=false}}else if(type==='ssp'){if(S.sspActive){S.sspActive=false}else{S.sspActive=true;S.spActive=false}}const spBtn=document.getElementById('spToggleBtn');const spBtnChar=document.getElementById('spToggleBtnChar');const sspBtn=document.getElementById('sspToggleBtn');if(spBtn)spBtn.classList.toggle('active',S.spActive);if(spBtnChar)spBtnChar.classList.toggle('active',S.spActive);if(sspBtn)sspBtn.classList.toggle('active',S.sspActive);updateDetailDynamicSections(S.currentDetailType)}
...
const MAIN_TAB_PATH_SHORT={latest_release:'/new',stages:'/st',calculator:'/cal',team_builder:'/tb',modifications:'/op',banner_timeline:'/tl'};
...
const v=opts&&opts.variantTag?String(opts.variantTag):'';
return String(S.lang||'EN')+':'+String(type)+':'+String(id)+':'+v
}
function detailApiUrl(type,id,opts){
const eid=encodeURIComponent(id),lang=encodeURIComponent(S.lang);
if(type==='character')return`/api/character/${eid}?lang=${lang}`;
if(type==='unit')return`/api/unit/${eid}?lang=${lang}`;
if(type==='supporter')return`/api/supporter/${eid}?lang=${lang}&level=100&lb_tier=3`;
if(type==='option_part'){
const vt=opts&&opts.variantTag?`&variant_tag=${encodeURIComponent(String(opts.variantTag))}`:'';
...
function updateDetailDynamicSections(type){const d=S.currentDetailData;if(type!=='supporter'&&type!=='stage')document.getElementById('detailStatsWrapper').innerHTML=renderStatsWrapper(d,type);if(type==='character'){document.getElementById('detailAbilitiesContainer').innerHTML=renderAbilsDynamic(d.abilities,t('sec_abilities'),false,S.spActive);document.getElementById('detailSkillsContainer').innerHTML=renderSkills(d.skills,S.spActive)}else if(type==='unit'){document.getElementById('detailAbilitiesContainer').innerHTML=renderAbilsDynamic(d.abilities,t('sec_abilities'),S.sspActive,false);document.getElementById('detailUnitSkillsContainer').innerHTML=renderSkills(d.skills||[],false);document.getElementById('detailWeaponsContainer').innerHTML=renderWeaponsDynamic(d.weapons,S.sspActive,d);document.getElementById('detailMechanismsContainer').innerHTML=renderMechanisms(d.mechanisms);const tr=document.getElementById('detailUnitTerrainRow');if(tr){const base=d.terrain||[];const ter=S.sspActive&&(d.terrain_ssp&&d.terrain_ssp.length)?d.terrain_ssp:base;const hasTerrainEnh=!!(S.sspActive&&(d.has_terrain_enhancement===true||(d.terrain_ssp&&d.terrain&&d.terrain_ssp.some((ts,i)=>ts.level!==(d.terrain[i]?.level??-1)))));tr.innerHTML=renderHeaderTerrain(ter,S.sspActive,hasTerrainEnh)}}else if(type==='supporter'){let lh='';if(d.leader_skills&&d.leader_skills.length){lh=`<div class="detail-section"><div class="section-title">${t('sec_leader_skill')}</div><div class="ability-list">${d.leader_skills.map(ls=>{let ts2=ls.tags.map(t=>t.name).join(',');return`<div class="ability-item" style="flex-direction:column;gap:10px;cursor:pointer;" onclick="openTagModal('${esc(ts2)}','${ls.separator}')"><div class="ability-detail" style="margin:0;">${esc(ls.desc)}</div>${ls.tags&&ls.tags.length?`<div class="detail-tags-row" style="margin-top:0;align-items:center;">${renderSkillTags([{tags:ls.tags,separator:ls.separator}])}</div>`:''}</div>`}).join('')}</div></div>`}document.getElementById('detailLeaderSkillContainer').innerHTML=lh;let ah='';if(d.active_skills&&d.active_skills.length){ah=`<div class="detail-section"><div class="section-title">${t('sec_active_skills')}</div><div class="ability-list">${d.active_skills.map(sk=>`<div class="ability-item"><div class="ability-icon-wrap">${renderAbilIcon({icon:sk.icon})}</div><div class="ability-info"><div class="ability-name">${esc(sk.name)}</div><div class="ability-detail" style="margin:0;white-space:pre-wrap;">${esc(sk.desc)}</div></div></div>`).join('')}</div></div>`}document.getElementById('detailAbilitiesContainer').innerHTML=ah}else if(type==='stage'){document.getElementById('detailStageRestrictionsContainer').innerHTML=renderStageRestrictions(d);document.getElementById('detailStageMapContainer').innerHTML=renderStageMapSection(d);document.getElementById('detailNpcContainer').innerHTML=renderNpcDetails(d.npc_details||[],d.id)}}
function renderStatsWrapper(d,type){let hcf=type==='character'?(d.has_conditional_passive!=null?d.has_conditional_passive:d.has_ex_stats):d.has_cond_stats;const cp=hcf&&S.conditionalPassiveActive;let sr,bs;if(type==='unit'){const td=(d.lb_data&&d.lb_data[S.currentLbTier])||(d.stats&&{stats_no_cond:d.stats,stats_with_cond:d.stats,sp_stats_no_cond:d.stats,sp_stats_with_cond:d.stats,ssp_stats_no_cond:d.stats,ssp_stats_with_cond:d.stats});if(!td)return'';if(d.has_sp){if(S.sspActive){sr=cp?td.ssp_stats_with_cond:td.ssp_stats_no_cond;bs=td.ssp_stats_no_cond}else if(S.spActive){sr=cp?td.sp_stats_with_cond:td.sp_stats_no_cond;bs=td.sp_stats_no_cond}else{sr=cp?td.stats_with_cond:td.stats_no_cond;bs=td.stats_no_cond}}else{sr=cp?td.stats_with_cond:td.stats_no_cond;bs=td.stats_no_cond}}else{const exTiers=d.ex_supercharged_tiers;if(cp&&exTiers&&exTiers.length>1){const ti=Math.min(Math.max(0,S.charSuperchargedExTier|0),exTiers.length-1);sr=exTiers[ti].stats;bs=d.stats}else if(d.has_sp){if(S.spActive){sr=cp?d.sp_stats_with_ex:d.sp_stats;bs=d.sp_stats}else{sr=cp?d.stats_with_ex:d.stats;bs=d.stats}}else{sr=cp?d.stats_with_ex:d.stats;bs=d.stats}}let th='';if(hcf){const cplab=type==='unit'?_dcUnitConditionalToggleLabel(d):t('conditional_passive');th=`<div class="conditional-toggle" style="justify-content:flex-end;margin-bottom:8px;"><div class="toggle-clickable ${S.conditionalPassiveActive?'active':''}" role="button" tabindex="0" onclick="toggleConditionalPassive(!S.conditionalPassiveActive)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleConditionalPassive(!S.conditionalPassiveActive)}"><span class="toggle-switch"></span><span class="toggle-label">${esc(cplab)}</span></div></div>`}let exRow='';if(type==='character'&&cp&&d.ex_supercharged_tiers&&d.ex_supercharged_tiers.length>1){const ti=Math.min(Math.max(0,S.charSuperchargedExTier|0),d.ex_supercharged_tiers.length-1);exRow=`<div class="detail-ex-tier-row" style="display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end;margin:0 0 8px 0">`+d.ex_supercharged_tiers.map((t,idx)=>`<button type="button" class="dc-lb-btn${idx===ti?' active':''}" style="font-size:11px;line-height:1.25;max-width:100%;white-space:normal;text-align:center;padding:6px 8px" onclick="setCharSuperchargedExTier(${idx})">${esc(t.label||('EX '+t.tier))}</button>`).join('')+`</div>`}return`${th}${exRow}<div class="stats-grid">${sr.map((s,i)=>{const b=bs[i];const hcb=cp&&s.total>b.total;let eb='';if(s.bonus>0){const showSsp=(type==='unit'&&S.sspActive&&s.name==='Move');eb=`<div class="stat-card-bonus">${showSsp?`<img src="${imgUrl('/static/images/UI/UI_Common_Icon_Ssp.webp')}" alt="SSP" style="height:14px;vertical-align:-2px;margin-right:4px;" onerror="this.style.display='none'">`:''}+${fmtN(s.bonus)}</div>`}return`<div class="stat-card ${hcb?'has-cond-bonus':''}"><div class="stat-card-label">${esc(tStat(s.name))}</div><div class="stat-card-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`}).join('')}</div>`}
function toggleConditionalPassive(c){S.conditionalPassiveActive=c;if(!c)S.charSuperchargedExTier=0;updateDetailDynamicSections(S.currentDetailType)}
...
function renderAbilityDetailChunks(d){let obj;if(typeof d==='string')obj={text:d};else if(d&&typeof d==='object')obj=d;else return'';const raw0=String(obj.text||'').trim();const raw=normalizeAbilityDetailTextForSplit(raw0);const cond=renderAbilityConditionGroups(obj);const hasCond=!!cond;const validCg=abilityValidConditionGroupsSorted(obj);function wrapBlock(inner){return`<div class="ability-detail-chunk"><div class="ability-detail-block"><div class="ability-detail-block-inner">${inner}</div></div></div>`}if(!raw)return hasCond?wrapBlock(cond):'';let groups=splitAbilityTextIntoBlocks(raw);if(validCg.length>=2&&groups.length===1){const g0=groups[0]||'';const sub=g0.split(/\.\s+(?=When\s+Vigor\b|\b(?:Increase|Increases)\b)/i).map(x=>x.trim()).filter(x=>x.length);if(sub.length>=2&&sub.length===validCg.length)groups=sub}const useParallel=validCg.length>=2&&validCg.length===groups.length;if(useParallel)return groups.map((g,i)=>{const textHtml=g?`<div class="ability-detail-text">${esc(g)}</div>`:'';const condHtml=renderAbilityConditionGroupAt(obj,i);return wrapBlock(textHtml+condHtml)}).join('');const condIdx=blockIndexForConditions(groups,hasCond);return groups.map((g,i)=>{const textHtml=g?`<div class="ability-detail-text">${esc(g)}</div>`:'';const condHtml=i===condIdx?cond:'';return wrapBlock(textHtml+condHtml)}).join('')}
...
const allStats=getCompareStats();
if(!allStats.length||!allStats[0].length)return;
const labels=allStats[0].map(s=>tStat(s.name));
...
/** 00 Raiser (1370005950): in-game labels this slot Conditional Passive, not the generic EX-ability title. */
function _dcUnitConditionalToggleLabel(ud){
if(!ud||String(ud.id)!=='1370005950')return t('conditional_passive');
const lc=String(S.lang||'EN').toUpperCase();
if(lc==='TW'||lc==='HK')return'條件被動';
if(lc==='JA'||lc==='JP')return'条件パッシブ';
...
charCpToggle=`<div class="dc-picked-controls"><div class="conditional-toggle dc-dc-cond-toggle"><div class="toggle-clickable${S.dc.charCondPassive?' active':''}" role="button" tabindex="0" title="${escAttr(_cpl)}" onclick="toggleDcCharCondPassive()" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleDcCharCondPassive()}"><span class="toggle-switch"></span><span class="toggle-label">${esc(_cpl)}</span></div></div></div>`;
}
let charExTierRow='';
...
return`<div class="${cardCls}"><div class="stat-card-label">${esc(tStat(statKey))}</div><div class="stat-card-value ${hl}">${fmtN(val)}</div>${bon}</div>`;
}
...
unitGrid+=_dcDefEternalStatCard(s,pr.val,pr.bonusLine);
}
...
charGrid+=_dcDefEternalStatCard(s,pr.val,pr.bonusLine);
}
