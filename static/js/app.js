...
Object.assign(T.EN,{tab_banner_timeline:'Banner Timeline',bt_view_table:'Table',bt_view_timeline:'Timeline',bt_col_banner:'Banner',bt_col_units:'Featured units',bt_col_chars:'Featured characters',bt_col_start:'Start (JST)',bt_col_end:'End (JST)',bt_col_duration:'Duration',bt_empty:'No banner data.'});Object.assign(T.TW,{tab_banner_timeline:'轉蛋時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'橫幅',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無轉蛋資料。'});Object.assign(T.HK,{tab_banner_timeline:'扭蛋時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'橫幅',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無扭蛋資料。'});const STAT_NAME_MAP={EN:{},TW:{'Ranged':'射擊','Melee':'格鬥','Awaken':'覚醒','Defense':'守備','Reaction':'反應','HP':'HP','EN':'EN','Attack':'攻擊','Mobility':'機動','Move':'移動'}};
...
function primeBtTimelineObserve(){disposeBtTimelineObs();const rail=document.querySelector('#bannerTimelineRoot .bt-timeline-rail');if(!rail)return;const reduced=typeof window.matchMedia==='function'&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;const cards=rail.querySelectorAll('.bt-tl-card');if(reduced||!cards.length){cards.forEach(el=>el.classList.add('bt-tl-card--visible'));return}S._btTimelineObs=new IntersectionObserver(entries=>{entries.forEach(en=>{if(en.isIntersecting)en.target.classList.add('bt-tl-card--visible')})},{root:null,threshold:0.06,rootMargin:'0px'});cards.forEach(c=>S._btTimelineObs.observe(c))}
...
function renderBannerTimelineTable(rows){const thcombo='<thead><tr>'+'<th class="bt-th-banner">'+esc(t('bt_col_banner'))+'</th>'+'<th class="bt-th-combo"><div class="bt-th-stack"><div class="bt-th-lines"><span>'+esc(t('bt_col_units'))+'</span><span class="bt-th-muted">'+esc(t('bt_col_chars'))+'</span></div></div></th>'+'<th>'+esc(t('bt_col_start'))+'</th>'+'<th>'+esc(t('bt_col_end'))+'</th>'+'<th>'+esc(t('bt_col_duration'))+'</th>'+'</tr></thead>';let body='<tbody>';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';body+='<tr><td class="bt-banner-cell">'+img+'<div class="bt-banner-title">'+esc(b.name||'')+'</div></td>'+btFeaturedMergedCell(b.featured_units,b.featured_chars)+'<td class="bt-date-cell">'+esc(btFmtScheduleSlot(b.start_label,b))+'</td><td class="bt-date-cell">'+esc(btFmtScheduleSlot(b.end_label,b))+'</td><td class="bt-date-cell">'+esc(btFmtDuration(b))+'</td></tr>'});body+='</tbody>';return'<div class="bt-table-wrap"><table class="data-table bt-table">'+thcombo+body+'</table></div>'}
function renderBannerTimelineRail(rows){let h='<div class="bt-timeline-shell"><div class="bt-timeline-rail">';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const dsl=btFmtScheduleSlot(b.start_label,b);const del=btFmtScheduleSlot(b.end_label,b);h+='<article class="bt-tl-card"><div class="bt-banner-title">'+esc(b.name||'')+'</div>'+img+'<div class="bt-tl-dates"><span>'+esc(t('bt_col_start'))+': '+esc(dsl)+'</span> · <span>'+esc(t('bt_col_end'))+': '+esc(del)+'</span></div><div class="bt-tl-dur">'+esc(btFmtDuration(b))+'</div><div class="bt-tl-feats"><div class="bt-tl-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(b.featured_units)+'<div class="bt-tl-sub bt-tl-sub--pad">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(b.featured_chars)+'</div></article>'});h+='</div></div>';return h}
