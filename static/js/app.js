...
Object.assign(T.EN,{tab_banner_timeline:'Unit Assembly Timeline',bt_view_table:'Table',bt_view_timeline:'Timeline',bt_col_banner:'Unit Assembly',bt_col_featured:'Featured',bt_col_units:'Featured units',bt_col_chars:'Featured characters',bt_col_supporters:'Featured supporters',bt_col_start:'Start (JST)',bt_col_end:'End (JST)',bt_col_duration:'Duration',bt_empty:'No schedule data.',bt_sort_start_hint:'Click to sort by start date (▼ newest first · ▲ oldest first).'});Object.assign(T.TW,{tab_banner_timeline:'機體補給時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'機體補給',bt_col_featured:'精選',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_supporters:'主打支援',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無轉蛋資料。',bt_sort_start_hint:'按開始日期排序（▼ 由新至舊 · ▲ 由舊至新）。'});Object.assign(T.HK,{tab_banner_timeline:'機體補給時間軸',bt_view_table:'表格',bt_view_timeline:'時間軸',bt_col_banner:'機體補給',bt_col_featured:'精選',bt_col_units:'主打機體',bt_col_chars:'主打角色',bt_col_supporters:'主打支援',bt_col_start:'開始（日本時間）',bt_col_end:'結束（日本時間）',bt_col_duration:'期間',bt_empty:'尚無扭蛋資料。',bt_sort_start_hint:'按開始日期排序（▼ 由新至舊 · ▲ 由舊至新）。'});const STAT_NAME_MAP={EN:{},TW:{'Ranged':'射擊','Melee':'格鬥','Awaken':'覚醒','Defense':'守備','Reaction':'反應','HP':'HP','EN':'EN','Attack':'攻擊','Mobility':'機動','Move':'移動'}};
...
function primeBtTimelineObserve(){disposeBtTimelineObs();const rail=document.querySelector('#bannerTimelineRoot .bt-ig-shell');if(!rail)return;const reduced=typeof window.matchMedia==='function'&&window.matchMedia('(prefers-reduced-motion: reduce)').matches;const cards=rail.querySelectorAll('.bt-ig-step.bt-vtl-card');if(reduced||!cards.length){cards.forEach(el=>el.classList.add('bt-vtl-card--visible'));return}S._btTimelineObs=new IntersectionObserver(entries=>{entries.forEach(en=>{if(en.isIntersecting)en.target.classList.add('bt-vtl-card--visible')})},{root:null,threshold:0.06,rootMargin:'0px'});cards.forEach(c=>S._btTimelineObs.observe(c))}
