...
function syncRankingTailControlsPlacement(){const row=document.querySelector('#panel-ranking .ranking-sort-row');if(!row)return;const tail=document.querySelector('#panel-ranking .ranking-tail-controls');if(!tail)return;if(S.currentTab!=='ranking')return;const mode=S.ranking&&S.ranking.mode;const charsHost=document.getElementById('rankCharListStatToggles');const unitsHost=document.getElementById('rankUnitListStatToggles');const targetHost=(mode==='characters'?charsHost:unitsHost)||null;const dock=rankingTailMobileRelocationActive()&&targetHost;if(dock){targetHost.appendChild(tail)}else if(tail.parentElement!==row){row.appendChild(tail)}}
