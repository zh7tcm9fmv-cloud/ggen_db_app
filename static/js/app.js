...
function rankingBarLimit(){try{return window.matchMedia&&window.matchMedia('(max-width:768px)').matches?10:20}catch(_){return 20}}
function ensureRankingTopControlEnhancements(){const row=document.querySelector('#panel-ranking .ranking-sort-row');if(!row)return;const dirBtn=document.getElementById('rankingDirBtn');if(!row.querySelector('.ranking-view-toggle')){const h=`<div class="ranking-view-toggle" role="group" aria-label="Ranking view mode"><button type="button" class="ranking-view-btn" data-ranking-view="list" onclick="setRankingViewMode('list')">List</button><button type="button" class="ranking-view-btn" data-ranking-view="bar" onclick="setRankingViewMode('bar')">Bar</button></div>`;if(dirBtn)dirBtn.insertAdjacentHTML('beforebegin',h);else row.insertAdjacentHTML('beforeend',h)}}
function syncRankingViewModeUi(){ensureRankingTopControlEnhancements();const vm=(S.ranking&&S.ranking.viewMode)||'list';document.querySelectorAll('.ranking-view-btn').forEach(b=>b.classList.toggle('active',b.dataset.rankingView===vm));const pg=document.querySelector('#panel-ranking .pagination-bar-wrap');if(pg)pg.style.display=vm==='bar'?'none':''}
function setRankingViewMode(mode){if(mode!=='list'&&mode!=='bar')return;if(S.ranking.viewMode===mode)return;S.ranking.viewMode=mode;syncRankingViewModeUi();const cur=S.ranking.mode==='characters'?'characters':'units';const payload=_rankingLastPayloadByMode[cur];if(payload)renderRankingList(payload);else void loadRankingList(mode==='bar'?1:(cur==='characters'?S.ranking.pageChar:S.ranking.pageUnit))}
...
function detailRankingSortKeyForStat(type,statName){if(type==='character'){const m={Ranged:'Ranged',Melee:'Melee',Awaken:'Awaken',Defense:'Defense',Reaction:'Reaction'};return m[statName]||''}const m={HP:'HP',EN:'EN',Attack:'ATK',Defense:'DEF',Mobility:'MOB',Move:'MOV'};return m[statName]||''}
...
async function ensureDetailRankingStats(type,id){if((type!=='character'&&type!=='unit')||!id)return null;const ck=detailRankingCacheKey(type,id);if(_detailRankingStatsCache.has(ck))return _detailRankingStatsCache.get(ck);if(_detailRankingStatsInflight.has(ck))return await _detailRankingStatsInflight.get(ck);const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB','MOV'];const iid=String(id);const p=(async()=>{const byStat={};await Promise.all(sortKeys.map(async sk=>{try{const r=await fetch(detailRankingFetchUrlFor(type,sk));if(!r.ok)return;const d=await r.json();const rows=Array.isArray(d&&d.rows)?d.rows:[];const idx=rows.findIndex(x=>String(x&&x.id||'')===iid);if(idx<0)return;byStat[sk]={rank:idx+1,total:Number(d.total||0),value:Number(rows[idx]&&rows[idx][sk]||0),bounds:d.stat_bounds||null}}catch(_){}}));_detailRankingStatsCache.set(ck,byStat);return byStat})().finally(()=>_detailRankingStatsInflight.delete(ck));_detailRankingStatsInflight.set(ck,p);return await p}
...
function detailRankingPosText(meta){return meta&&meta.rank&&meta.total?`${fmtN(meta.rank)}位/${fmtN(meta.total)}`:'-'}
...
if(!isRankingView)return`<div class="stat-card ${hcb?'has-cond-bonus':''}"><div class="stat-card-label">${esc(tStat(s.name,type))}</div><div class="stat-card-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`;
