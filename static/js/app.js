...
function renderSortieGroupRestrictions(g){let unitItems=[],charItems=[];(g.restrictions||[]).forEach(r=>{const sides=sortieRestrictionTargetSides(r);const items=collectSortieRestrictionItems(r);if(sides.unit)unitItems=unitItems.concat(items);if(sides.char)charItems=charItems.concat(items)});return renderSortieRestrictionPart('unit',mergeSortieRestrictionItems(unitItems))+renderSortieRestrictionPart('char',mergeSortieRestrictionItems(charItems))}
