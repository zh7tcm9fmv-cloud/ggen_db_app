...
function armBannerTimelineScrollFabBaseline(){if(S.currentTab!=='banner_timeline')return;const st=Math.max(window.scrollY||0,document.documentElement.scrollTop||0,document.body.scrollTop||0);S._btFabLastScrollSt=st;S._btFabShown=false;const b=document.getElementById('bannerTimelineScrollTopFab');if(b)b.hidden=true}
function updateBannerTimelineScrollFabVisibility(){const b=document.getElementById('bannerTimelineScrollTopFab');if(!b)return;if(S.currentTab!=='banner_timeline'){b.hidden=true;return}const st=Math.max(window.scrollY||0,document.documentElement.scrollTop||0,document.body.scrollTop||0);const topTol=56,jit=10;if(typeof S._btFabLastScrollSt!=='number'){S._btFabLastScrollSt=st;b.hidden=true;return}const d=st-S._btFabLastScrollSt;S._btFabLastScrollSt=st;if(st<=topTol)S._btFabShown=false;else{if(d>jit)S._btFabShown=true;else if(d<-jit)S._btFabShown=false}b.hidden=S._btFabShown!==true}
