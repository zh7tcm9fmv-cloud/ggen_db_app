...
function closeAllUnitWeaponRangePanels(){['unit','rankUnit'].forEach(pfx=>{['WeaponRange','WeaponRangeNonMap'].forEach(mid=>{const panel=document.getElementById(pfx+mid+'FilterPanel');const btn=document.getElementById(pfx+mid+'FilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})})}
...
function updateGameNewsNavNotice(show){const tab=document.getElementById('navGameNewsTab');if(!tab)return;tab.classList.toggle('has-ui-notice',!!show);const lbl=tab.querySelector('.nav-tab-label');if(lbl){const base=String(lbl.textContent||'').replace(/\*$/, '').trim()||t('tab_game_news');lbl.textContent=show?base+'*':base}}
