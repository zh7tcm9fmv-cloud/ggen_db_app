...
STAT_NAME_MAP.JA={'Ranged':'射撃','Melee':'格闘','Awaken':'覚醒','Defense':'防御力','Reaction':'反応','HP':'HP','EN':'EN','Attack':'攻撃力','Mobility':'機動力','Move':'移動力'};STAT_NAME_MAP.JP=STAT_NAME_MAP.JA;
...
function getUnitWeaponDebuffDefsForUi(){const defs=UNIT_WEAPON_DEBUFF_DEFS;const hk=S.lang==='HK';let list;const ok=S.weaponDebuffPresentKeys;if(ok===undefined||ok===null)list=defs.slice();else if(!Array.isArray(ok))list=defs.slice();else if(ok.length===0)list=[];else{const allow=new Set(ok.map(String));list=defs.filter(d=>allow.has(d.key))}if(hk)list=list.filter(d=>d.key!=='range_beam');return list}
function pruneUnitWeaponDebuffSelectionForLocale(){if(S.lang!=='HK'||!Array.isArray(S.listUnitWeaponDebuff))return false;const next=S.listUnitWeaponDebuff.filter(k=>String(k)!=='range_beam');if(next.length===S.listUnitWeaponDebuff.length)return false;S.listUnitWeaponDebuff=next;const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();return true}
function applyWeaponDebuffPresentFromApi(d){if(!d||!Object.prototype.hasOwnProperty.call(d,'weapon_debuff_present_keys'))return false;const keys=d.weapon_debuff_present_keys;if(!Array.isArray(keys))return false;S.weaponDebuffPresentKeys=keys;const allow=new Set(keys.map(String));let sel=Array.isArray(S.listUnitWeaponDebuff)?S.listUnitWeaponDebuff:[];let pruned=sel.filter(k=>allow.has(String(k)));if(S.lang==='HK')pruned=pruned.filter(k=>String(k)!=='range_beam');const changed=pruned.length!==sel.length;if(changed)S.listUnitWeaponDebuff=pruned;const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();updateUnitWeaponDebuffFilterLabel();syncUnitListSspForWeaponEffectFilters();return changed}
