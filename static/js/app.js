...
function renderBannerTimeline(d){const root=document.getElementById('bannerTimelineRoot');if(!root)return;const rows=d.banners||[];const rf=()=>requestAnimationFrame(()=>{syncListTheadStickyTop();armBannerTimelineScrollFabBaseline();updateBannerTimelineScrollFabVisibility();updateBtVoteNotices()});if(!rows.length){root.innerHTML='<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';rf();return}root.innerHTML=renderBannerTimelineTable(rows);btSyncVoteModeChrome();rf()}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};btBindVoteDelegation();initNavTabsOverflowHints();bootstrapContentNoticesAll()}
