...
function renderStageTable(data){document.getElementById('stageBody').innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const ph=locked?'<div class="row-thum-placeholder stage-thum-locked">🔒</div>':(r.portrait?imgTag(r.portrait,{cls:'row-thum',webp:true,onerror:"this.parentElement.innerHTML='<div class=\\'row-thum-placeholder\\'>🗺️</div>'"}) :'<div class="row-thum-placeholder">🗺️</div>');const diffCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSa||isSs||isTs)?'<span class="stat-value"></span>':`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span>`;const numCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'<span class="stat-value"></span>':`<span class="stat-value">${fmtN(r.stage_number)}</span>`;const nameCell=locked?`<span class="name-text er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</span>`:`<span class="name-text">${esc(r.name)}</span>`;const cpDisp=locked?'—':fmtN(r.recommended_cp);const terDisp=locked?'—':esc(r.terrain);return`<tr data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><td class="col-thum"><div class="stage-thum-wrap">${ph}</div></td><td class="col-diff">${diffCell}</td><td class="col-number">${numCell}</td><td class="col-name"><div class="name-cell">${nameCell}</div></td><td class="col-cp"><span class="stat-value">${cpDisp}</span></td><td class="col-terrain"><span class="stat-value">${terDisp}</span></td></tr>`}).join('')}
function renderStageGrid(data){const grid=document.getElementById('stageGrid');grid.innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const ph=locked?'<div class="row-thum-placeholder stage-thum-locked">🔒</div>':(r.portrait?imgTag(r.portrait,{cls:'row-thum',webp:true,onerror:"this.parentElement.innerHTML='<div class=\\'row-thum-placeholder\\'>🗺️</div>'"}) :'<div class="row-thum-placeholder">🗺️</div>');const metaLine=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'':isSa?`<span class="stat-value">#${fmtN(r.stage_number)}</span>`:`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span> <span class="stat-value">#${fmtN(r.stage_number)}</span>`;const metaRow=!locked&&(isSs||isTs)?'':`<div class="list-grid-card-meta">${metaLine}</div>`;const nameLine=locked?`<div class="list-grid-card-name er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</div>`:`<div class="list-grid-card-name">${esc(r.name)}</div>`;const cpPart=locked?'—':fmtN(r.recommended_cp);const terPart=locked?'—':esc(r.terrain);return`<div class="list-grid-card list-grid-card--stage${locked?' list-grid-card--locked':''}" data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><div class="list-grid-card-thumb"><div class="stage-thum-wrap">${ph}</div></div><div class="list-grid-card-body">${metaRow}${nameLine}<div class="list-grid-card-stats">${t('col_stage_cp')} ${cpPart} · ${t('terrain')} ${terPart}</div></div></div>`}).join('')}
