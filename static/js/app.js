...
function renderBannerTimelineRail(rows){const sorted=btSortBannerRows(rows||[]);let h='<div class="bt-timeline-shell bt-ig-shell"><div class="bt-ig-spine-line" aria-hidden="true"></div><div class="bt-ig-list">';let lastYm='';sorted.forEach((b,idx)=>{const ym=btYmKey(b&&b.start_ms);if(ym!==lastYm&&ym&&ym!=='__'){lastYm=ym;h+='<div class="bt-ig-monthlabel">'+esc(btMonthHeadingFromKey(ym))+'</div>'}const isRight=idx%2===0;const dn=btJstStartDayNum(b&&b.start_ms);const tipRaw=b&&b.start_ms>0?btFmtBannerNodeTooltip(b.start_ms):(b&&(b.name||''))||'';const tipA=escAttr(tipRaw),num=dn!=null?String(dn):'—',pal=idx%7;const card='<div class="bt-ig-card-skin">'+btBannerTimelineCardBodyHtml(b)+'</div>';const emp='<div class="bt-ig-zone bt-ig-zone--blank" aria-hidden="true"></div>';const left=isRight?emp:'<div class="bt-ig-zone bt-ig-zone--card">'+card+'</div>';const right=isRight?'<div class="bt-ig-zone bt-ig-zone--card">'+card+'</div>':emp;const node='<div class="bt-ig-rail-slot"><div class="bt-ig-node bt-ig-node--pal-'+pal+'" title="'+tipA+'" aria-label="'+tipA+'"><span class="bt-ig-num">'+esc(num)+'</span></div></div>';h+='<div class="bt-ig-step '+(isRight?'bt-ig-step--right':'bt-ig-step--left')+' bt-ig-step--pal-'+pal+' bt-vtl-card">'+left+node+right+'</div>'});h+='</div></div>';return h}
