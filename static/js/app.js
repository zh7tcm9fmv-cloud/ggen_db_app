...
function clearRankingBrowseFilters(which){if(which==='rankChar'){S.listRankCharSource='ALL';S.listRankCharLineage=[];S.listRankCharSeries=[];S.listRankCharSkills=[];S.listRankCharAbilities=[];S.listRankCharSp=false;S.listRankCharCond=false;S.browseCombRankCharLineage='and';S.browseCombRankCharSeries='or';S.browseCombRankCharSkill='and';S.browseCombRankCharTrait='and';const qEl=document.getElementById('rankCharFilter');if(qEl){qEl.value='';updateSearchHintVisibility('rankCharFilter');syncBrowseSearchWidth('rankCharFilter')}const rp=rarityPrefix('rankChar');const ltEl=document.getElementById(rp+'RarityLT');rarityKeysFor('rankChar').forEach(k=>{const el=document.getElementById(rp+'Rarity'+k);if(el)el.checked=true});if(ltEl)ltEl.checked=true;ROLE_LIST_IDS.forEach(id=>{const el=document.getElementById('rankCharRole'+id);if(el)el.checked=true});fillSourcePanel('rankChar');syncLineageCheckboxes('rankChar');syncSeriesCheckboxes('rankChar');syncSkillBrowseCheckboxes('rankChar');syncAbilBrowseCheckboxes('rankChar');updateRarityFilterButtonLabel('rankChar');updateRoleFilterButtonLabel('rankChar');updateSourceFilterButtonLabel('rankChar');updateLineageFilterButtonLabel('rankChar');updateSeriesFilterButtonLabel('rankChar');updateSkillBrowseFilterLabel('rankChar');updateAbilBrowseFilterLabel('rankChar');syncRankListStatToggleUi();closeAllFilterPanels();invalidateRankingBrowseFilterPanels()}else{S.listRankUnitSource='ALL';S.listRankUnitLineage=[];S.listRankUnitSeries=[];S.listRankUnitAbilities=[];S.listRankUnitTerrain=[];S.listRankUnitWeaponDebuff=[];S.listRankUnitWeaponRange=[];S.listRankUnitWeaponRangeNonMap=[];S.listRankUnitMechanism=[];S.listRankUnitWeaponSspExToggle=false;S.listRankUnitSp=false;S.listRankUnitSsp=false;S.listRankUnitCond=false;S._rankUnitWeaponRangeSspAutoArmed=false;S.browseCombRankUnitLineage='and';S.browseCombRankUnitSeries='or';S.browseCombRankUnitAbil='and';S.browseCombRankTerrain='and';S.browseCombRankWb='and';S.browseCombRankWr='and';S.browseCombRankWrNm='and';S.browseCombRankMech='and';const qEl=document.getElementById('rankUnitFilter');if(qEl){qEl.value='';updateSearchHintVisibility('rankUnitFilter');syncBrowseSearchWidth('rankUnitFilter')}const rp=rarityPrefix('rankUnit');const ltEl=document.getElementById(rp+'RarityLT');rarityKeysFor('rankUnit').forEach(k=>{const el=document.getElementById(rp+'Rarity'+k);if(el)el.checked=true});if(ltEl)ltEl.checked=true;ROLE_LIST_IDS.forEach(id=>{const el=document.getElementById('rankUnitRole'+id);if(el)el.checked=true});fillSourcePanel('rankUnit');syncLineageCheckboxes('rankUnit');syncSeriesCheckboxes('rankUnit');syncSkillBrowseCheckboxes('rankUnit');const b=document.getElementById('rankUnitTerrainFilterBody');if(b)b.innerHTML=buildUnitTerrainFilterHtml();syncUnitWeaponDebuffCheckboxes();syncUnitWeaponRangeCheckboxes();syncUnitWeaponRangeNonMapCheckboxes();syncUnitMechanismCheckboxes();const sspTb=document.getElementById('rankUnitWeaponSspExToggleBtn');if(sspTb)sspTb.setAttribute('aria-pressed',S.listRankUnitWeaponSspExToggle?'true':'false');updateRarityFilterButtonLabel('rankUnit');updateRoleFilterButtonLabel('rankUnit');updateSourceFilterButtonLabel('rankUnit');updateLineageFilterButtonLabel('rankUnit');updateSeriesFilterButtonLabel('rankUnit');updateSkillBrowseFilterLabel('rankUnit');updateUnitTerrainFilterLabel();updateRankUnitWeaponFilterChrome();updateUnitWeaponRangeFilterLabel('rankUnit');updateUnitWeaponRangeNonMapFilterLabel();updateUnitMechanismFilterLabel();updateLineageFilterButtonLabel('rankUnit');syncRankListStatToggleUi();closeAllFilterPanels();invalidateRankingBrowseFilterPanels()}void loadRankingList(1)}
function refreshRankingFilterLabels(){updateRarityFilterButtonLabel('rankChar');updateRoleFilterButtonLabel('rankChar');updateSourceFilterButtonLabel('rankChar');updateLineageFilterButtonLabel('rankChar');updateSeriesFilterButtonLabel('rankChar');updateSkillBrowseFilterLabel('rankChar');updateAbilBrowseFilterLabel('rankChar');updateRarityFilterButtonLabel('rankUnit');updateRoleFilterButtonLabel('rankUnit');updateSourceFilterButtonLabel('rankUnit');updateLineageFilterButtonLabel('rankUnit');updateSeriesFilterButtonLabel('rankUnit');updateSkillBrowseFilterLabel('rankUnit');updateUnitTerrainFilterLabel();updateRankUnitWeaponFilterChrome();updateUnitWeaponRangeNonMapFilterLabel();updateUnitMechanismFilterLabel();fillSourcePanel('rankChar');fillSourcePanel('rankUnit');syncRankListStatToggleUi()}
function ensureRankUnitWeaponRangeFilterDom(){if(document.getElementById('rankUnitWeaponRangeWrap'))return;const tpl=document.getElementById('rankUnitWeaponRangeToolbarDupSource');const anc=document.getElementById('rankUnitWeaponDebuffWrap');if(!tpl||!anc)return;const imp=document.importNode(tpl.content,true);const wrap=imp.querySelector('#rankUnitWeaponRangeWrap');if(!wrap)return;wrap.style.display='none';wrap.setAttribute('aria-hidden','true');anc.insertAdjacentElement('afterend',wrap);updateUnitWeaponRangeFilterLabel('rankUnit')}
function rankUnitWeaponFilterIsSspMode(){return!!(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units'&&S.listRankUnitWeaponSspExToggle)}
function toggleRankUnitWeaponSspExMode(ev){if(ev){ev.preventDefault();ev.stopPropagation()}closeAllFilterPanels();S.listRankUnitWeaponSspExToggle=!S.listRankUnitWeaponSspExToggle;const b=document.getElementById('rankUnitWeaponSspExToggleBtn');if(b)b.setAttribute('aria-pressed',S.listRankUnitWeaponSspExToggle?'true':'false');updateRankUnitWeaponFilterChrome()}
function toggleRankUnitWeaponFilterPanel(ev){if(rankUnitWeaponFilterIsSspMode())return toggleRankUnitWeaponRangePanel(ev);return toggleUnitWeaponDebuffPanel(ev)}
function toggleRankUnitWeaponRangePanel(ev){if(ev)ev.stopPropagation();const panel=document.getElementById('rankUnitWeaponRangeMergedPanel');const btn=document.getElementById('rankUnitWeaponDebuffFilterBtn');const body=document.getElementById('rankUnitWeaponRangeMergedBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){if(body)body.innerHTML=buildUnitWeaponRangeFilterHtml('listRankUnitWeaponRange');panel.hidden=false;btn.setAttribute('aria-expanded','true');ensureFilterPanelFooter('rankUnitWeaponRangeMergedPanel',{onClear:clearUnitWeaponRangeOnly,combineKey:'browseCombRankWr',combineEntity:'rankUnit'})}}
function updateRankUnitWeaponFilterChrome(){if(!document.getElementById('rankUnitWeaponDebuffFilterBtn'))return;if(S.listRankUnitWeaponSspExToggle){const label=document.getElementById('rankUnitWeaponDebuffFilterLabel');const btn=document.getElementById('rankUnitWeaponDebuffFilterBtn');if(!label||!btn)return;const sk='listRankUnitWeaponRange';const p=weaponRangeLabelPrefix();btn.title=weaponRangeFilterName();let sel=S[sk];if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeAllText())}</span>`;btn.classList.remove('active');return}btn.classList.add('active');label.innerHTML=`<span class="source-filter-btn-plain" style="font-size:13px;font-weight:600">${esc(p+' '+String(sel[0]))}</span>`}else updateUnitWeaponDebuffFilterLabel('rankUnit')}
...
function initToolbarFilterIcons(){ensureUnitUltRarityRow('unit');ensureUnitUltRarityRow('rankUnit');ensureUnitWeaponRangeFilterDom();ensureUnitWeaponRangeNonMapFilterDom();ensureRankUnitWeaponRangeFilterDom();ensureRankUnitWeaponRangeNonMapFilterDom();['char','unit','supp','rankChar','rankUnit'].forEach(p=>fillRarityPanelIcons(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillSourcePanel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar();['char','rankChar'].forEach(p=>updateAbilBrowseFilterLabel(p));updateUnitWeaponRangeFilterLabel('unit');updateUnitWeaponRangeFilterLabel('rankUnit')}
...
function closeAllUnitWeaponRangePanels(){['unit','rankUnit'].forEach(pfx=>{['WeaponRange','WeaponRangeNonMap'].forEach(mid=>{const panel=document.getElementById(pfx+mid+'FilterPanel');const btn=document.getElementById(pfx+mid+'FilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')});if(pfx==='rankUnit'){const mp=document.getElementById('rankUnitWeaponRangeMergedPanel');if(mp)mp.hidden=true;const sh=document.getElementById('rankUnitWeaponDebuffFilterBtn');if(sh)sh.setAttribute('aria-expanded','false')}})}
...
function onUnitWeaponDebuffChange(){syncUnitListSspForWeaponEffectFilters();if(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units')updateRankUnitWeaponFilterChrome();else updateUnitWeaponDebuffFilterLabel();if(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units')scheduleRankingListReload();else scheduleBrowseListReload('units')}
...
function syncUnitWeaponRangeCheckboxes(){const pfx=unitWeaponRangeDomPrefix();let bid=pfx+'WeaponRangeFilterBody';if(pfx==='rankUnit'){const mb=document.getElementById('rankUnitWeaponRangeMergedBody');if(mb)bid='rankUnitWeaponRangeMergedBody'}const body=document.getElementById(bid);if(body)body.innerHTML=buildUnitWeaponRangeFilterHtml()}
...
function onUnitWeaponRangeChange(){syncUnitListSspForWeaponRangeFilters();if(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units')updateRankUnitWeaponFilterChrome();else updateUnitWeaponRangeFilterLabel();if(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units')scheduleRankingListReload();else scheduleBrowseListReload('units')}
...
function clearUnitWeaponRangeOnly(){const sk=unitWeaponRangeStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];S[rk?'_rankUnitWeaponRangeSspAutoArmed':'_unitWeaponRangeSspAutoArmed']=false;const pfx=unitWeaponRangeDomPrefix();let bid=pfx+'WeaponRangeFilterBody';if(pfx==='rankUnit'){const mb=document.getElementById('rankUnitWeaponRangeMergedBody');if(mb)bid='rankUnitWeaponRangeMergedBody'}const body=document.getElementById(bid);if(body)body.innerHTML=buildUnitWeaponRangeFilterHtml();if(rk)updateRankUnitWeaponFilterChrome();else updateUnitWeaponRangeFilterLabel();syncUnitListSspForWeaponRangeFilters();if(rk)void loadRankingList(1);else loadUnits(1)}
...
function updateUnitWeaponRangeNonMapFilterLabel(forcePfx){const pfx=forcePfx||unitWeaponRangeDomPrefix();const sk=pfx==='rankUnit'?'listRankUnitWeaponRangeNonMap':'listUnitWeaponRangeNonMap';const label=document.getElementById(pfx+'WeaponRangeNonMapFilterLabel');const btn=document.getElementById(pfx+'WeaponRangeNonMapFilterBtn');if(!label||!btn)return;const p=weaponRangeLabelPrefix();btn.title=weaponRangeNonMapFilterName();let sel=S[sk];if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeNonMapAllText())}</span>`;btn.classList.remove('active');return}btn.classList.add('active');label.innerHTML=`<span class="source-filter-btn-plain" style="font-size:13px;font-weight:600">${esc(p+' '+String(sel[0]))}</span>`}
function clearUnitWeaponRangeNonMapOnly(){const sk=unitWeaponRangeNonMapStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];const pfx=unitWeaponRangeDomPrefix();const body=document.getElementById(pfx+'WeaponRangeNonMapFilterBody');if(body)body.innerHTML=buildUnitWeaponRangeNonMapFilterHtml();updateUnitWeaponRangeNonMapFilterLabel();syncUnitListSspForWeaponRangeFilters();if(rk)void loadRankingList(1);else loadUnits(1)}
function ensureRankUnitWeaponRangeNonMapFilterDom(){const host=document.getElementById('rankingUnitToolbarHost');if(!host)return;const base=document.getElementById('rankUnitWeaponRangeWrap');if(!base)return;if(document.getElementById('rankUnitWeaponRangeNonMapWrap'))return;const clone=base.cloneNode(true);clone.id='rankUnitWeaponRangeNonMapWrap';const btn=clone.querySelector('#rankUnitWeaponRangeFilterBtn');if(btn){btn.id='rankUnitWeaponRangeNonMapFilterBtn';btn.setAttribute('onclick','toggleUnitWeaponRangeNonMapPanel(event)');btn.setAttribute('title',weaponRangeNonMapFilterName());btn.setAttribute('aria-expanded','false')}const lbl=clone.querySelector('#rankUnitWeaponRangeFilterLabel');if(lbl){lbl.id='rankUnitWeaponRangeNonMapFilterLabel';lbl.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeNonMapAllText())}</span>`}const panel=clone.querySelector('#rankUnitWeaponRangeFilterPanel');if(panel){panel.id='rankUnitWeaponRangeNonMapFilterPanel';panel.hidden=true}const body=clone.querySelector('#rankUnitWeaponRangeFilterBody');if(body){body.id='rankUnitWeaponRangeNonMapFilterBody';body.innerHTML=''}base.insertAdjacentElement('afterend',clone);updateUnitWeaponRangeNonMapFilterLabel('rankUnit')}
function ensureUnitWeaponRangeNonMapFilterDom(){const host=document.getElementById('panel-units');if(!host)return;const base=document.getElementById('unitWeaponRangeWrap');if(!base)return;if(document.getElementById('unitWeaponRangeNonMapWrap'))return;const clone=base.cloneNode(true);clone.id='unitWeaponRangeNonMapWrap';const btn=clone.querySelector('#unitWeaponRangeFilterBtn');if(btn){btn.id='unitWeaponRangeNonMapFilterBtn';btn.setAttribute('onclick','toggleUnitWeaponRangeNonMapPanel(event)');btn.setAttribute('title',weaponRangeNonMapFilterName());btn.setAttribute('aria-expanded','false')}const lbl=clone.querySelector('#unitWeaponRangeFilterLabel');if(lbl){lbl.id='unitWeaponRangeNonMapFilterLabel';lbl.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeNonMapAllText())}</span>`}const panel=clone.querySelector('#unitWeaponRangeFilterPanel');if(panel){panel.id='unitWeaponRangeNonMapFilterPanel';panel.hidden=true}const body=clone.querySelector('#unitWeaponRangeFilterBody');if(body){body.id='unitWeaponRangeNonMapFilterBody';body.innerHTML=''}base.insertAdjacentElement('afterend',clone);updateUnitWeaponRangeNonMapFilterLabel('unit')}
...
function renderAbilIcon(ab,srcRow){const base=srcRow||ab;const hintUrl=(ab&&ab.strategy_hint_icon)||(base&&base.strategy_hint_icon);const hintSrc=hintUrl?imgUrlWebp(imgUrlPreferCdn(hintUrl)):'';const hintHtml=hintSrc?`<img class="strategy-hint-badge" src="${hintSrc}" alt="" loading="lazy" onerror="this.style.display='none'">`:'';if(!ab||!ab.icon)return hintHtml+'<div class="ability-icon-placeholder">★</div>';let h=`<img class="ability-icon" src="${imgUrl(ab.icon)}" alt="" onerror="this.parentElement.innerHTML='<div class=\\'ability-icon-placeholder\\'>★</div>'">`;if(ab.frame_overlay&&ab.is_ex)h+=`<img class="ability-frame-overlay" src="${imgUrl(ab.frame_overlay)}" alt="" onerror="this.style.display='none'">`;return hintHtml+h}
