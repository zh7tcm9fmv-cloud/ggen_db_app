...
function buildUnitMapWeaponRangeFilterHtml(skOpt){const sk=skOpt||unitMapWeaponRangeStateKey();const selArr=Array.isArray(S[sk])?S[sk]:[];const selSet=new Set(selArr.map(v=>String(v)));let h='<div class="map-wrt-filter-list">';MAP_WEAPON_RANGE_TYPE_DEFS.forEach(d=>{const active=selSet.has(String(d.value))?' is-active':'';h+=`<button type="button" class="rarity-filter-row map-wrt-filter-row${active}" onclick="onUnitMapWeaponRangeRowClick('${d.value}',event)" onkeydown="onUnitMapWeaponRangeRowKey(event,'${d.value}')"><span class="map-wrt-filter-lbl">${esc(t(d.labelKey))}</span></button>`});h+='</div>';return h}
