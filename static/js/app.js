...
function readGameNewsSeen(lang){const k=String(lang||'EN').trim()||'EN';try{const all=JSON.parse(localStorage.getItem(GAME_NEWS_SEEN_KEY)||'{}');const row=all[k];return row&&typeof row==='object'?{at:Number(row.at)||0,fp:String(row.fp||'')}:{at:0,fp:''}}catch(_){return{at:0,fp:''}}}
function writeGameNewsSeen(lang,patch){const k=String(lang||'EN').trim()||'EN';try{const all=JSON.parse(localStorage.getItem(GAME_NEWS_SEEN_KEY)||'{}');const prev=all[k]&&typeof all[k]==='object'?all[k]:{};all[k]={at:Number(patch&&patch.at!=null?patch.at:prev.at)||0,fp:String(patch&&patch.fp!=null?patch.fp:prev.fp||'')};localStorage.setItem(GAME_NEWS_SEEN_KEY,JSON.stringify(all))}catch(_){}}
...
async function bootstrapGameNewsNotices(){ensureNavGameNewsNoticeFlare();const lang=S.lang||'EN';const seen=readGameNewsSeen(lang);try{const r=await fetch('/api/game_news/status?lang='+encodeURIComponent(lang)+'&last_seen_at='+encodeURIComponent(String(seen.at||0)),{credentials:'same-origin',cache:'no-store'});if(!r.ok){updateGameNewsNavNotice(false);return}const d=await r.json();const hasNew=!!d.has_new||(!!d.fingerprint&&d.fingerprint!==seen.fp&&Number(d.latest_at||0)>Number(seen.at||0));S._gameNewsStatus=d;updateGameNewsNavNotice(hasNew)}catch(_){updateGameNewsNavNotice(false)}}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};ensureNavBannerTimelineNoticeFlare();void bootstrapBtVoteNotices();ensureNavGameNewsNoticeFlare();void bootstrapGameNewsNotices();initNavTabsOverflowHints()}
