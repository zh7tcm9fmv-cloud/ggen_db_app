...
function pruneUnitMapWeaponRangeSelectionForUi(){const drop=MAP_WEAPON_RANGE_TYPE_UI_HIDDEN;function pr(k){const arr=Array.isArray(S[k])?S[k]:[];const next=arr.filter(x=>!drop.has(String(x)));if(next.length===arr.length)return false;S[k]=next;return true}const changed=pr('listUnitMapWeaponRange')||pr('listRankUnitMapWeaponRange');if(changed){['unit','rankUnit'].forEach(pfx=>updateUnitMapWeaponRangeFilterLabel(pfx));const pfx=unitWeaponRangeDomPrefix();const panel=document.getElementById(pfx+'MapWeaponRangeFilterPanel');if(panel&&!panel.hidden)populateUnitMapWeaponRangePanel(pfx)}return changed}
...
function buildUnitMapWeaponRangeFilterHtml(skOpt){const sk=skOpt||unitMapWeaponRangeStateKey();const selArr=Array.isArray(S[sk])?S[sk]:[];const selSet=new Set(selArr.map(v=>String(v)));let h='';getMapWeaponRangeTypeDefsForUi().forEach(d=>{const checked=selSet.has(String(d.value))?' checked':'';h+=`<label class="rarity-filter-row map-wrt-filter-row"><input type="checkbox" value="${d.value}"${checked} onchange="onUnitMapWeaponRangeCheckboxChange('${d.value}',event)"><span>${esc(t(d.labelKey))}</span></label>`});return h}
...
function populateUnitMapWeaponRangePanel(pfx){const panel=document.getElementById(pfx+'MapWeaponRangeFilterPanel');if(!panel)return;panel.innerHTML=buildUnitMapWeaponRangeFilterHtml();ensureFilterPanelFooter(pfx+'MapWeaponRangeFilterPanel',mapWeaponRangeFooterOpts(pfx));syncMapWeaponRangeCombineFooter(pfx)}
function syncUnitMapWeaponRangeCheckboxes(){const pfx=unitWeaponRangeDomPrefix();const panel=document.getElementById(pfx+'MapWeaponRangeFilterPanel');if(!panel)return;const sk=unitMapWeaponRangeStateKey();const selSet=new Set((Array.isArray(S[sk])?S[sk]:[]).map(v=>String(v)));panel.querySelectorAll('.map-wrt-filter-row input[type=checkbox]').forEach(inp=>{inp.checked=selSet.has(String(inp.value))});syncMapWeaponRangeCombineFooter(pfx)}
function toggleUnitMapWeaponRangePanel(ev){if(ev)ev.stopPropagation();const pfx=unitWeaponRangeDomPrefix();if(!mapWeaponDebuffActiveForPfx(pfx))return;const panel=document.getElementById(pfx+'MapWeaponRangeFilterPanel');const btn=document.getElementById(pfx+'MapWeaponRangeFilterBtn');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){populateUnitMapWeaponRangePanel(pfx);panel.hidden=false;btn.setAttribute('aria-expanded','true')}}
...
function onUnitMapWeaponRangeCheckboxChange(v,ev){if(ev)ev.stopPropagation();onUnitMapWeaponRangeRowClick(v,ev)}
...
function clearUnitMapWeaponRangeOnly(){const sk=unitMapWeaponRangeStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];const pfx=unitWeaponRangeDomPrefix();populateUnitMapWeaponRangePanel(pfx);updateUnitMapWeaponRangeFilterLabel();if(rk)void loadRankingList(1);else loadUnits(1)}
...
function setupMapWeaponRangeFilterDom(pfx,debWrap){if(!debWrap||document.getElementById(pfx+'MapWeaponRangeWrap'))return;const wrap=document.createElement('div');wrap.id=pfx+'MapWeaponRangeWrap';wrap.className='rarity-filter-wrap map-wrt-filter-wrap';wrap.innerHTML=`<button type="button" class="rarity-filter-btn role-filter-dd-btn" id="${pfx}MapWeaponRangeFilterBtn" onclick="toggleUnitMapWeaponRangePanel(event)" aria-expanded="false" aria-haspopup="true"><span id="${pfx}MapWeaponRangeFilterLabel" class="filter-btn-label-inner"></span><span class="rarity-filter-caret">▾</span></button><div class="rarity-filter-panel filter-dropdown-panel map-wrt-filter-panel" id="${pfx}MapWeaponRangeFilterPanel" role="menu" hidden onclick="event.stopPropagation()"></div>`;debWrap.insertAdjacentElement('afterend',wrap);updateUnitMapWeaponRangeFilterLabel(pfx)}
