...
function _dcOnDcStageSearchInput(){_dcRebuildDcStageUi()}
...
function renderModTable(data){document.getElementById('modBody').innerHTML=data.rows.map(r=>{const icon=r.thum?imgTag(r.thum,{cls:'row-thum mod-icon',webp:true,onerror:"this.outerHTML='<div class=\\'row-thum-placeholder\\'>⚙</div>'"}) :'<div class="row-thum-placeholder">⚙</div>';return`<tr class="mod-browse-row" data-detail-type="option_part" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('option_part','${escJs(String(r.id))}')"><td class="col-thum">${icon}</td><td class="col-name"><div class="name-cell"><span class="name-text mod-name">${esc(r.name)}</span></div></td><td class="col-tag">${renderModLineageTagCell(r.tags)}</td><td class="col-details"><div class="mod-details-cell">${esc(r.details||'')}</div></td></tr>`}).join('')}
function renderModGrid(data){const grid=document.getElementById('modGrid');grid.innerHTML=data.rows.map(r=>{const icon=r.thum?imgTag(r.thum,{cls:'row-thum mod-icon',webp:true,onerror:"this.outerHTML='<div class=\\'row-thum-placeholder\\'>⚙</div>'"}) :'<div class="row-thum-placeholder">⚙</div>';const tagRow=r.tags&&r.tags.length?`<div class="detail-tags-row" style="margin-top:4px;align-items:center;flex-wrap:wrap;justify-content:flex-start;">${r.tags.map(tg=>createTagHtml(tg,{defaultTarget:'unit'})).join('')}</div>`:`<div style="margin-top:4px;font-size:11px;color:var(--text-muted)">-</div>`;return`<div class="list-grid-card list-grid-card--mod mod-browse-row" data-detail-type="option_part" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('option_part','${escJs(String(r.id))}')"><div class="list-grid-card-thumb">${icon}</div><div class="list-grid-card-body"><div class="list-grid-card-name mod-name">${esc(r.name)}</div>${tagRow}<div class="mod-details-cell">${esc(r.details||'')}</div></div></div>`}).join('')}
...
function detailPrefetchKey(type,id){return String(S.lang||'EN')+':'+String(type)+':'+String(id)}
function detailApiUrl(type,id){
...
if(type==='option_part')return`/api/option_part/${eid}?lang=${lang}`;
...
async function fetchDetailPayload(type,id){
const ck=detailPrefetchKey(type,id);
...
const url=detailApiUrl(type,id);
const p=(async()=>{
try{
...
return`<img class="dc-wpn-atk-type-icon" src="${imgUrl(src)}" alt="" loading="lazy" title="${tip}" onerror="this.style.display='none'">`;
}).join('');
}
...
if(S.dc.atkCharData&&!S.dc.atkCharData._manual)_dcRecalcPilotBonuses();
onDcParamChange();
}
...
if(_dcStagesCache)return _dcStagesCache;
try{const r=await fetch(`/api/dc_targets?lang=${S.lang}`);const d=await r.json();_dcStagesCache=Array.isArray(d)?d:[];return _dcStagesCache}catch(e){return[]}}
...
unitStatModeHtml=`<div class="dc-lb-row dc-unit-stat-mode-row"><button type="button" class="dc-lb-btn${uModeRow==='normal'?' active':''}" onclick="setDcUnitStatMode('normal')">Normal</button><button type="button" class="dc-lb-btn${uModeRow==='sp'?' active':''}" onclick="setDcUnitStatMode('sp')">SP</button><button type="button" class="dc-lb-btn${uModeRow==='ssp'?' active':''}" onclick="setDcUnitStatMode('ssp')">SSP</button></div>`;
}
...
const advAtkPct=_dcAdvantageTagAtkPctFromAbilities(ud,S.dc.defNpc);
const atkMid=_dcApplyAdvantageTagAtkToUnitAtk(_dcApplyCounterOwnAtkToUnitAtk(atkAfterPair),advAtkPct,uEff.unitAtkExSquadBase);
...
function setDcCharCondPassive(on){S.dc.charCondPassive=!!on;if(!on)S.dc.dcSuperchargedExTier=0;renderDcAtkUnit();renderDcAtkChar();const autoV=S.dc.atkCharData&&_dcShouldAutoSuperchargedVigorOnCharCp(S.dc.atkCharData);if(on&&autoV)setDcMp('super');else if(!on&&autoV)_dcAutoSetVigor();else onDcParamChange()}
function setDcSuperchargedExTier(i){const cd=S.dc.atkCharData,arr=cd&&cd.ex_supercharged_tiers;if(!arr||arr.length<2)return;const n=arr.length;S.dc.dcSuperchargedExTier=Math.max(0,Math.min(Number(i)||0,n-1));renderDcAtkUnit();renderDcAtkChar();onDcParamChange()}
...
if(m){const v=parseInt(m[1],10);b.dmgDealt+=isUnmet?0:v;b.items.push({label:`Damage Dealt +${v}%`,val:v,key:'dmgDealt',cond:isUnmet,autoMet:hasCond&&condMet,name:dispName,abilityHasCond:hasCond,spCharGate,src,alwaysActive:!hasCond&&!isUnmet&&!spCharGate})}
...
h+=`<label class="dc-pilot-bonus-row${rowOff?' dc-pilot-bonus--off':''}"><input type="checkbox" id="${togId}" ${checked?'checked':''} onchange="_dcRecalcPilotBonuses()"><span class="dc-pilot-bonus-line"><span class="dc-pilot-bonus-main">${esc(it.label)}${condTag}</span><span class="dc-pilot-bonus-detail">${esc(it.name)}</span></span></label>`;
});
h+=`</div>`;
area.innerHTML+=h;
}
function _dcRecalcPilotBonuses(){
...
let h='';
if(u){
const us=u.stats_raw||{},ub=u.bonus_amounts||{};
...
unitGrid+=_dcDefEternalStatCard(s,us[s]||0,ub[s]||0);
}
...
charGrid+=_dcDefEternalStatCard(s,cs[s]||0,cb[s]||0);
}
...
if(S.dc.atkCharData)_dcRecalcPilotBonuses();
else onDcParamChange();
}
...
return{unitAtk,unitHp,unitDefVal,unitMob,unitMove,atkSupport,leaderPct,unitAtkExSquadBase,unitAtkGrowthAfterOptions,unitDefExSquadBase,deltaExAtk};
}
...
unitAtk=_dcApplyAdvantageTagAtkToUnitAtk(unitAtk,advantageTagAtkPct,uMod.unitAtkExSquadBase);
...
const npcHp=defUnit?(defUnit.stats_raw?.HP||0):0;
const accuracy=lvData.accuracy;
const critical=lvData.critical;

const atkCharReaction=_dcFindStat(atkCharStats,'Reaction');
const defUnitMob=defUnit?(defUnit.stats_raw?.Mobility||0):0;
const defCharReaction=defChar?(defChar.stats_raw?.Reaction||0):0;
...
<button type="button" class="dc-ctrl-btn" onclick="copyDcResultText()" style="font-size:12px;padding:6px 14px">📋 Copy Results</button>
...
multi.forEach(m=>{
const rr=m.result,ud=m.slot.atkUnitData,cd=m.slot.atkCharData;
...
const m=document.getElementById('dcShareMsg');if(m){m.textContent='Copied to clipboard!';setTimeout(()=>m.textContent='',2000)}
});
}
