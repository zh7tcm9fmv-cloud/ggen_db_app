...
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const showBtn=show&&!S.btVoteMode;const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);['btVoteModeBtn','btVoteModeBtnMobile'].forEach(id=>{const btn=document.getElementById(id);if(!btn)return;const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',showBtn)});syncNavTabsOverflowHints()}
...
function ensureNavTabsEdgeFlare(){const wrap=document.getElementById('navTabsEdgeFlareWrap');if(!wrap||wrap.querySelector('.ui-notice-flare'))return;wrap.insertAdjacentHTML('beforeend',uiNoticeFlareHtml())}
function syncNavTabsOverflowHints(){const scroller=navTabScrollerEl(),shell=document.getElementById('navTabsShell'),hintR=document.getElementById('navTabsEdgeHintRight'),flareWrap=document.getElementById('navTabsEdgeFlareWrap');if(!scroller||!shell||!hintR)return;const canScroll=scroller.scrollWidth>scroller.clientWidth+2;const hasRight=canScroll&&scroller.scrollLeft+scroller.clientWidth<scroller.scrollWidth-2;const hiddenNotices=navTabsHiddenRightNoticeTabs(scroller);shell.classList.toggle('nav-tabs-shell--scroll-right',hasRight);shell.classList.toggle('nav-tabs-shell--hidden-notice',hiddenNotices.length>0);hintR.hidden=!hasRight;if(flareWrap)flareWrap.classList.toggle('has-ui-notice',hiddenNotices.length>0);hintR.dataset.noticeTarget=hiddenNotices[0]?hiddenNotices[0].id:''}
function scrollNavTabsToHiddenNotice(){const scroller=navTabScrollerEl();if(!scroller)return;const hidden=navTabsHiddenRightNoticeTabs(scroller);const target=hidden[0]||null;if(target)target.scrollIntoView({behavior:'smooth',inline:'nearest',block:'nearest'});else scroller.scrollBy({left:Math.max(120,scroller.clientWidth*.65),behavior:'smooth'})}
function initNavTabsOverflowHints(){if(S._navTabsOverflowInit)return;S._navTabsOverflowInit=1;ensureNavTabsEdgeFlare();const scroller=navTabScrollerEl(),hintR=document.getElementById('navTabsEdgeHintRight');if(!scroller||!hintR)return;let raf=0;const sync=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;syncNavTabsOverflowHints()})};scroller.addEventListener('scroll',sync,{passive:true});window.addEventListener('resize',sync,{passive:true});hintR.addEventListener('click',scrollNavTabsToHiddenNotice);sync()}
...
function btVoteMobileBarHtml(){return'<div class="bt-vote-mobile-bar"><div class="bt-vote-mobile-bar-main"><span class="bt-vote-mobile-bar-k">'+esc(t('bt_col_featured'))+'</span>'+btVoteModeCaptionMarkup('btVoteModeCaptionMobile')+'</div>'+btVoteModeBtnMarkup('btVoteModeBtnMobile')+'</div>'}
