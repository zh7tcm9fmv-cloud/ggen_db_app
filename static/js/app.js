...
function afterBrowseCombineOpChange(ent){if(ent==='char')scheduleBrowseListReload('characters');else if(ent==='unit')scheduleBrowseListReload('units');else if(ent==='supp')scheduleBrowseListReload('supporters')}
...
function clearSourceOnly(which){const p=which==='char'?'char':'unit';const keys=['assembly','development','other'];keys.forEach(k=>{const el=document.getElementById(p+'SourceChk'+k.charAt(0).toUpperCase()+k.slice(1));if(el)el.checked=true});S[p==='char'?'listCharSource':'listUnitSource']='ALL';updateSourceFilterButtonLabel(which);if(which==='char'){cancelBrowseListReloadTimer('characters');loadCharacters(1)}else{cancelBrowseListReloadTimer('units');loadUnits(1)}}
function clearLineageOnly(p){if(p==='char')S.listCharLineage=[];else if(p==='unit')S.listUnitLineage=[];else S.listSuppLineage=[];delete S['_linLblRf'+p];if(S._browseLineageNameHints)S._browseLineageNameHints[p]={};syncLineageCheckboxes(p);updateLineageFilterButtonLabel(p);if(p==='char'){cancelBrowseListReloadTimer('characters');loadCharacters(1)}else if(p==='unit'){cancelBrowseListReloadTimer('units');loadUnits(1)}else{cancelBrowseListReloadTimer('supporters');loadSupporters(1)}}
function clearSeriesOnly(p){if(p==='char')S.listCharSeries=[];else S.listUnitSeries=[];syncSeriesCheckboxes(p);updateSeriesFilterButtonLabel(p==='char'?'char':'unit');if(p==='char'){cancelBrowseListReloadTimer('characters');loadCharacters(1)}else{cancelBrowseListReloadTimer('units');loadUnits(1)}}
function clearSkillOnly(p){if(p==='char')cancelBrowseListReloadTimer('characters');else cancelBrowseListReloadTimer('units');if(p==='char')S.listCharSkills=[];else S.listUnitAbilities=[];syncSkillBrowseCheckboxes(p);updateSkillBrowseFilterLabel(p);if(p==='char')loadCharacters(1);else loadUnits(1)}
...
function clearUnitTerrainOnly(){cancelBrowseListReloadTimer('units');S.listUnitTerrain=[];const body=document.getElementById('unitTerrainFilterBody');if(body)body.innerHTML=buildUnitTerrainFilterHtml();updateUnitTerrainFilterLabel();loadUnits(1)}
...
function onUnitTerrainChange(){updateUnitTerrainFilterLabel();scheduleBrowseListReload('units')}
...
function onUnitWeaponDebuffChange(){syncUnitListSspForWeaponEffectFilters();updateUnitWeaponDebuffFilterLabel();scheduleBrowseListReload('units')}
...
function clearUnitWeaponDebuffOnly(){cancelBrowseListReloadTimer('units');S.listUnitWeaponDebuff=[];const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();updateUnitWeaponDebuffFilterLabel();loadUnits(1)}
...
function onUnitMechanismChange(){updateUnitMechanismFilterLabel();scheduleBrowseListReload('units')}
...
function clearUnitMechanismOnly(){cancelBrowseListReloadTimer('units');S.listUnitMechanism=[];const body=document.getElementById('unitMechanismFilterBody');if(body)body.innerHTML=buildUnitMechanismFilterHtml();updateUnitMechanismFilterLabel();loadUnits(1)}
...
function onSkillBrowseItemChange(p){const key=p==='char'?'listCharSkills':'listUnitAbilities';const checked=Array.from(document.querySelectorAll(`input[name="${p==='char'?'charSkillSel':'unitSkillSel'}"].skill-browse-item:checked`)).map(i=>i.value);S[key]=checked.length?checked:[];updateSkillBrowseFilterLabel(p);scheduleBrowseListReload(p==='char'?'characters':'units')}
...
async function toggleSkillPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'SkillFilterPanel');const btn=document.getElementById(p+'SkillFilterBtn');const grid=document.getElementById(p+'SkillGrid');const search=document.getElementById(p+'SkillFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersEntity(p);if(grid&&!grid.dataset.populated){grid.innerHTML=buildSkillBrowseHtml(p);grid.dataset.populated='1'}if(search)search.placeholder=t('list_filter_search_placeholder');syncSkillBrowseCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensureFilterPanelFooter(p+'SkillFilterPanel',{onClear:()=>clearSkillOnly(p),combineKey:p==='char'?'browseCombCharSkill':'browseCombUnitAbil',combineEntity:p})}}
...
function onAbilBrowseItemChange(p){const checked=Array.from(document.querySelectorAll(`input[name="${p}AbilSel"].skill-browse-item:checked`)).map(i=>i.value);S.listCharAbilities=checked.length?checked:[];updateAbilBrowseFilterLabel(p);scheduleBrowseListReload('characters')}
...
async function toggleAbilPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'AbilFilterPanel');const btn=document.getElementById(p+'AbilFilterBtn');const grid=document.getElementById(p+'AbilGrid');const search=document.getElementById(p+'AbilFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersEntity('char');if(grid&&!grid.dataset.populated){grid.innerHTML=buildAbilBrowseHtml(p);grid.dataset.populated='1'}if(search)search.placeholder=t('list_filter_search_placeholder');syncAbilBrowseCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensureFilterPanelFooter(p+'AbilFilterPanel',{onClear:clearAbilOnly,combineKey:'browseCombCharTrait',combineEntity:'char'})}}
...
function invalidateBrowseFilterDomForEntity(entity){if(entity==='char'){const b=document.getElementById('charLineageFilterBody');if(b)delete b.dataset.populated;const s=document.getElementById('charSeriesFilterBody');if(s)delete s.dataset.populated;const gr=document.getElementById('charSkillGrid');if(gr)delete gr.dataset.populated;const ag=document.getElementById('charAbilGrid');if(ag)delete ag.dataset.populated;const ap=document.getElementById('charAbilFilterPanel');if(ap)delete ap.dataset.populated;const pan=document.getElementById('charSkillFilterPanel');if(pan)delete pan.dataset.populated}else if(entity==='unit'){const b=document.getElementById('unitLineageFilterBody');if(b)delete b.dataset.populated;const s=document.getElementById('unitSeriesFilterBody');if(s)delete s.dataset.populated;const gr=document.getElementById('unitSkillGrid');if(gr)delete gr.dataset.populated;const pan=document.getElementById('unitSkillFilterPanel');if(pan)delete pan.dataset.populated}else if(entity==='supp'){const b=document.getElementById('suppLineageFilterBody');if(b)delete b.dataset.populated}}
async function ensureBrowseFiltersEntity(entity){if(entity!=='char'&&entity!=='unit'&&entity!=='supp')return;const sigField=entity==='char'?'_poolSigChar':entity==='unit'?'_poolSigUnit':'_poolSigSupp';const sig=buildBrowsePoolSig(entity);const cur=S._browseFiltersByEntity;if(cur&&cur._lang===S.lang&&cur[sigField]===sig)return;const infl='_bfInfl'+entity;if(S[infl])return S[infl];S[infl]=(async()=>{const q=buildBrowsePoolQuery(entity);const apiEnt=entity==='char'?'characters':entity==='unit'?'units':'supporters';const langAtStart=S.lang;try{const r=await fetch(`/api/browse_filters?lang=${S.lang}&entity=${apiEnt}&filter_mode=current&${q}`);const data=await r.json();if(S.lang!==langAtStart)return;if(!S._browseFiltersByEntity||S._browseFiltersByEntity._lang!==S.lang){S._browseFiltersByEntity={_lang:S.lang,_poolSigChar:null,_poolSigUnit:null,_poolSigSupp:null,char:{lineages:[],series:[],skills:[],abilities:[]},unit:{lineages:[],series:[],abilities:[]},supp:{lineages:[]}}}const b=S._browseFiltersByEntity;b[sigField]=sig;if(entity==='char'){b.char={lineages:data.lineages||[],series:data.series||[],skills:data.skills||[],abilities:data.abilities||[]}}else if(entity==='unit'){b.unit={lineages:data.lineages||[],series:data.series||[],abilities:data.abilities||[]}}else{b.supp={lineages:data.lineages||[]}}invalidateBrowseFilterDomForEntity(entity)}catch(e){if(S.lang!==langAtStart)return;if(!S._browseFiltersByEntity||S._browseFiltersByEntity._lang!==S.lang){S._browseFiltersByEntity={_lang:S.lang,_poolSigChar:null,_poolSigUnit:null,_poolSigSupp:null,char:{lineages:[],series:[],skills:[],abilities:[]},unit:{lineages:[],series:[],abilities:[]},supp:{lineages:[]}}}const b=S._browseFiltersByEntity;b[sigField]=sig;if(entity==='char'){b.char={lineages:[],series:[],skills:[],abilities:[]}}else if(entity==='unit'){b.unit={lineages:[],series:[],abilities:[]}}else{b.supp={lineages:[]}}invalidateBrowseFilterDomForEntity(entity)}finally{S[infl]=null}})();return S[infl]}
...
function onRoleFilterChange(which){updateRoleFilterButtonLabel(which);if(which==='char')scheduleBrowseListReload('characters');else scheduleBrowseListReload('units')}
...
function onRarityFilterChange(which){updateRarityFilterButtonLabel(which);if(which==='char'){const se=document.getElementById('charSkillFilterSearch'),grid=document.getElementById('charSkillGrid');if(se&&grid&&grid.dataset.populated)filterSkillDropdown('char',se.value||'');scheduleBrowseListReload('characters')}else if(which==='unit'){const se=document.getElementById('unitSkillFilterSearch'),grid=document.getElementById('unitSkillGrid');if(se&&grid&&grid.dataset.populated)filterSkillDropdown('unit',se.value||'');scheduleBrowseListReload('units')}else scheduleBrowseListReload('supporters')}
...
function setStageBrowseSource(mode){if(mode!=='eternal'&&mode!=='score_attack')return;if((S.stages.source||'eternal')===mode)return;S.stages.source=mode;syncStageSourceToolbar();scheduleBrowseListReload('stages')}
