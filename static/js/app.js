...
const STAGE_BROWSE_CP_ICON='/static/images/UI/UI_Common_Icon_Cp.webp';
function stageBrowseThumbTitleHtml(r,locked){if(locked)return`<span class="stage-browse-thumb__title">${esc(t('er_stage_redacted_row'))}</span>`;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';if(isSs||isTs)return`<span class="stage-browse-thumb__title">${esc(r.name)}</span>`;if(isSa)return`<span class="stage-browse-thumb__title"><b>#${fmtN(r.stage_number)}</b> ${esc(r.name)}</span>`;return`<span class="stage-browse-thumb__title"><b>${esc(t('col_stage_no'))} ${fmtN(r.stage_number)}</b> ${esc(r.name)}</span>`}
function renderStageBrowseThumb(r,opts){opts=opts||{};const variant=opts.variant||'table';const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const showDiff=!locked&&!(isSa||isSs||isTs);const diffCode=esc(r.difficulty_code||'unknown');const diffHtml=showDiff?`<span class="stage-browse-thumb__diff stage-browse-thumb__diff--${diffCode}">${esc(r.difficulty_name||'-')}</span>`:'';const portrait=locked?'':(r.portrait?imgTag(r.portrait,{cls:'stage-browse-thumb__portrait',webp:true,alt:'',onerror:"this.style.display='none'"}) :'');const phFallback=!locked&&!r.portrait?'<span class="stage-browse-thumb__ph" aria-hidden="true">🗺️</span>':'';const lockOv=locked?'<span class="stage-browse-thumb__lock" aria-hidden="true">🔒</span>':'';const cpVal=locked?'—':fmtN(r.recommended_cp);const terVal=locked?'—':esc(r.terrain);const cpIcon=imgTag(STAGE_BROWSE_CP_ICON,{cls:'stage-browse-thumb__cp-icon',webp:true,alt:'',onerror:"this.style.display='none'"});const vCls=variant==='grid'?' stage-browse-thumb--grid':variant==='compact'?' stage-browse-thumb--compact':'';const lockCls=locked?' stage-browse-thumb--locked':'';return`<div class="stage-browse-thumb${vCls}${lockCls}" role="presentation"><div class="stage-browse-thumb__bg" aria-hidden="true"></div><div class="stage-browse-thumb__stripes" aria-hidden="true"></div>${portrait}${phFallback}<div class="stage-browse-thumb__shade" aria-hidden="true"></div>${lockOv}<div class="stage-browse-thumb__ui">${diffHtml?`<div class="stage-browse-thumb__head">${diffHtml}${stageBrowseThumbTitleHtml(r,locked)}</div>`:`<div class="stage-browse-thumb__head stage-browse-thumb__head--no-diff">${stageBrowseThumbTitleHtml(r,locked)}</div>`}<div class="stage-browse-thumb__band" aria-hidden="true"></div><div class="stage-browse-thumb__cp"><span class="stage-browse-thumb__cp-left">${cpIcon}<span class="stage-browse-thumb__cp-lbl">${esc(t('recommended_cp'))}</span></span><span class="stage-browse-thumb__cp-val">${cpVal}</span></div><div class="stage-browse-thumb__terrain"><span class="stage-browse-thumb__terrain-l">${esc(t('terrain'))}</span><span class="stage-browse-thumb__terrain-v">${terVal}</span></div></div></div>`}
function renderStageTable(data){document.getElementById('stageBody').innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const diffCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSa||isSs||isTs)?'<span class="stat-value"></span>':`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span>`;const numCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'<span class="stat-value"></span>':`<span class="stat-value">${fmtN(r.stage_number)}</span>`;const nameCell=locked?`<span class="name-text er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</span>`:`<span class="name-text">${esc(r.name)}</span>`;const cpDisp=locked?'—':fmtN(r.recommended_cp);const terDisp=locked?'—':esc(r.terrain);return`<tr data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><td class="col-thum"><div class="stage-thum-wrap">${renderStageBrowseThumb(r,{variant:'table'})}</div></td><td class="col-diff">${diffCell}</td><td class="col-number">${numCell}</td><td class="col-name"><div class="name-cell">${nameCell}</div></td><td class="col-cp"><span class="stat-value">${cpDisp}</span></td><td class="col-terrain"><span class="stat-value">${terDisp}</span></td></tr>`}).join('')}
function renderStageGrid(data){const grid=document.getElementById('stageGrid');grid.innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const metaLine=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'':isSa?`<span class="stat-value">#${fmtN(r.stage_number)}</span>`:`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span> <span class="stat-value">#${fmtN(r.stage_number)}</span>`;const metaRow=!locked&&(isSs||isTs)?'':`<div class="list-grid-card-meta">${metaLine}</div>`;const nameLine=locked?`<div class="list-grid-card-name er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</div>`:`<div class="list-grid-card-name">${esc(r.name)}</div>`;const cpPart=locked?'—':fmtN(r.recommended_cp);const terPart=locked?'—':esc(r.terrain);return`<div class="list-grid-card list-grid-card--stage${locked?' list-grid-card--locked':''}" data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><div class="list-grid-card-thumb">${renderStageBrowseThumb(r,{variant:'grid'})}</div><div class="list-grid-card-body">${metaRow}${nameLine}<div class="list-grid-card-stats">${t('col_stage_cp')} ${cpPart} · ${t('terrain')} ${terPart}</div></div></div>`}).join('')}
