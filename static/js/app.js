...
function renderTowerRewardSection(d){const rows=Array.isArray(d.tower_rewards)?d.tower_rewards:[];if(!rows.length)return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div class="stage-condition-item">${esc(t('none'))}</div></div>`;return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px;">${rows.map(r=>{const clickable=!!(r.detail_type&&r.detail_id);const clickAttrs=clickable?` role="button" tabindex="0" onclick="openDetail('${escJs(String(r.detail_type))}','${escJs(String(r.detail_id))}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openDetail('${escJs(String(r.detail_type))}','${escJs(String(r.detail_id))}')}"`:'';
const thumbHtml=(r.thumb_type==='char'&&r.thumb)?renderListThumb({thum:r.thumb,rarity:r.rarity||'N',role_icon:r.role_icon||'',acquisition_icon:r.acquisition_icon||''},'char',54):(r.thumb_type==='option_part'&&r.thumb)?renderListThumb({thum:r.thumb,rarity:r.rarity||'N'},'unit',54,{pickerThumb:true}):`<div style="width:54px;height:54px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);border-radius:8px;flex-shrink:0;overflow:hidden;">${r.icon?`<img src="${imgUrl(r.icon)}" alt="${escAttr(r.name||'')}" loading="lazy" decoding="async" style="width:42px;height:42px;object-fit:contain;display:block;" onerror="this.style.display='none'">`:'<span style="font-size:12px;color:var(--text-muted)">ITEM</span>'}</div>`;
