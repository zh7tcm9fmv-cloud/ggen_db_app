...
function btFeaturedCell(items){if(!items||!items.length)return'<span class="bt-date-cell">—</span>';let h='';items.forEach(it=>{const row={thum:it.thum,rarity:it.rarity||'N',role_icon:it.role_icon||'',acquisition_icon:it.acquisition_icon||'',special_icons:Array.isArray(it.special_icons)?it.special_icons:[],is_ultimate:!!it.is_ultimate};const dt=it.type==='character'?'character':'unit';const thumbKind=it.type==='character'?'char':'unit';h+='<div class="lr-card" style="width:140px;max-width:min(140px,40vw)" role="button" tabindex="0" data-detail-type="'+escAttr(dt)+'" data-detail-id="'+escAttr(it.id)+'" onclick="openDetail(\''+dt+'\',\''+escJs(it.id)+'\')"><div class="lr-card-thumb">'+renderListThumb(row,thumbKind,56)+'</div><div class="lr-card-name">'+esc(it.name||'')+'</div></div>'});return'<div class="bt-strip">'+h+'</div>'}
function renderBannerTimelineTable(rows){const th='<thead><tr><th>'+esc(t('bt_col_banner'))+'</th><th>'+esc(t('bt_col_units'))+'</th><th>'+esc(t('bt_col_chars'))+'</th><th>'+esc(t('bt_col_start'))+'</th><th>'+esc(t('bt_col_end'))+'</th><th>'+esc(t('bt_col_duration'))+'</th></tr></thead>';let body='<tbody>';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';body+='<tr><td class="bt-banner-cell">'+img+'<div class="bt-banner-title">'+esc(b.name||'')+'</div></td><td>'+btFeaturedCell(b.featured_units)+'</td><td>'+btFeaturedCell(b.featured_chars)+'</td><td class="bt-date-cell">'+esc(b.start_label!=null?String(b.start_label):'—')+'</td><td class="bt-date-cell">'+esc(b.end_label!=null?String(b.end_label):'—')+'</td><td class="bt-date-cell">'+esc(b.duration_label!=null?String(b.duration_label):'—')+'</td></tr>'});body+='</tbody>';return'<div class="bt-table-wrap"><table class="data-table">'+th+body+'</table></div>'}
function renderBannerTimelineRail(rows){let h='<div class="bt-timeline-shell"><div class="bt-timeline-rail">';rows.forEach(b=>{const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';h+='<article class="bt-tl-card"><div class="bt-banner-title">'+esc(b.name||'')+'</div>'+img+'<div class="bt-tl-dates"><span>'+esc(t('bt_col_start'))+': '+esc(String(b.start_label||'—'))+'</span> · <span>'+esc(t('bt_col_end'))+': '+esc(String(b.end_label||'—'))+'</span></div><div class="bt-tl-dur">'+esc(String(b.duration_label||'—'))+'</div><div class="bt-tl-feats"><div class="bt-tl-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(b.featured_units)+'<div class="bt-tl-sub bt-tl-sub--pad">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(b.featured_chars)+'</div></article>'});h+='</div></div>';return h}
function renderBannerTimeline(d){const root=document.getElementById('bannerTimelineRoot');if(!root)return;disposeBtTimelineObs();const rows=d.banners||[];if(!rows.length){root.innerHTML='<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';return}root.innerHTML=S.bannerTimelineView==='table'?renderBannerTimelineTable(rows):renderBannerTimelineRail(rows);if(S.bannerTimelineView==='timeline')requestAnimationFrame(()=>setTimeout(primeBtTimelineObserve,50));syncBtViewToggleButtons()}
async function loadBannerTimeline(){const root=document.getElementById('bannerTimelineRoot');const load=document.getElementById('bannerTimelineLoading');if(!root)return;const cacheKey=S.lang;if(S.btCacheKey===cacheKey&&S.btCacheData){renderBannerTimeline(S.btCacheData);return}if(load)load.style.display='flex';disposeBtTimelineObs();try{const r=await fetch('/api/banner_timeline?lang='+encodeURIComponent(S.lang),{credentials:'same-origin'});if(!r.ok)throw new Error('HTTP '+r.status);const data=await r.json();S.btCacheKey=cacheKey;S.btCacheData=data;renderBannerTimeline(data)}catch(e){S.btCacheKey=null;S.btCacheData=null;root.innerHTML='<div class="empty-state"><div class="empty-state-text">'+esc(String(e))+'</div></div>'}finally{if(load)load.style.display='none'}}
