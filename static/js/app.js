...
const thumbHtml=(r.thumb_type==='char'&&r.thumb)?renderListThumb({thum:r.thumb,rarity:r.rarity||'N',role_icon:r.role_icon||'',acquisition_icon:r.acquisition_icon||''},'char',54):`<div style="width:54px;height:54px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);border-radius:8px;flex-shrink:0;overflow:hidden;">${r.icon?`<img src="${imgUrl(r.icon)}" alt="${escAttr(r.name||'')}" loading="lazy" decoding="async" style="width:42px;height:42px;object-fit:contain;display:block;" onerror="this.style.display='none'">`:'<span style="font-size:12px;color:var(--text-muted)">ITEM</span>'}</div>`;
