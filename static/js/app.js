...
const meta=detailRankingMetaFor(type,d.id,s.name);const loading=!meta||!meta.rank||!meta.total;const w=loading?62:detailRankingBarWidth(meta);const splitPos=!loading&&S.lang!=='EN';let posHtml='...';if(!loading){if(splitPos){const rk=Number(meta.rank)||0,tt=Number(meta.total)||0;posHtml=`<span class="stat-rank-pos-main">${fmtN(rk)}位</span><span class="stat-rank-pos-total">/${fmtN(tt)}</span>`}else posHtml=esc(detailRankingPosText(meta))}
return`<div class="stat-card stat-card--ranking ${hcb?'has-cond-bonus':''} ${loading?'is-loading':''}"><div class="stat-rank-head"><div class="stat-card-label stat-rank-label">${esc(tStat(s.name,type))}</div><div class="stat-rank-bar"><span class="stat-rank-fill ${loading?'is-loading':''}" style="width:${w.toFixed(2)}%"></span></div><div class="stat-rank-pos ${loading?'is-loading':''} ${splitPos?'is-split':''}">${posHtml}</div></div><div class="stat-card-value stat-rank-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`
}).join('');
