...
function btVoteResultsHtml(gid,b){const totals=btVoteTotalsFor(gid);const opts=btVoteOptionsForBanner(b);let sum=0;opts.forEach(o=>{sum+=Number(totals[o.key]||0)});if(!sum)return'';let rows='';opts.forEach(o=>{const n=Number(totals[o.key]||0);if(n<=0)return;const pct=Math.round(n*1000/sum)/10;rows+='<div class="bt-vote-result-row"><div class="bt-vote-result-meta"><span class="bt-vote-result-label">'+esc(o.label)+'</span><span class="bt-vote-result-pct">'+pct+'%</span></div><div class="bt-vote-result-count">'+fmtN(n)+' '+esc(t('bt_vote_count'))+'</div><div class="bt-vote-result-bar"><i style="width:'+pct+'%"></i></div></div>'});if(!rows)return'';return'<div class="bt-vote-results" data-gasha-id="'+escAttr(gid)+'"><div class="bt-vote-results-total">'+esc(t('bt_vote_total'))+' '+fmtN(sum)+'</div>'+rows+'</div>'}
