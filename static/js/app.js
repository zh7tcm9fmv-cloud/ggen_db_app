...
function btFeaturedCell(items,tpx){let px=tpx!=null&&tpx!==undefined?Number(tpx):52;if(Number.isNaN(px)||px<32)px=52;if(!items||!items.length)return'<span class="bt-strip-empty">—</span>';let h='';items.forEach(it=>{const row={thum:it.thum,rarity:it.rarity||'N',role_icon:it.role_icon||'',acquisition_icon:it.acquisition_icon||'',special_icons:Array.isArray(it.special_icons)?it.special_icons:[],is_ultimate:!!it.is_ultimate};const typ=it.type==='character'?'character':it.type==='supporter'?'supporter':'unit';const thumbKind=typ==='character'?'char':typ==='supporter'?'supp':'unit';h+='<div class="lr-card bt-lr-tile" role="button" tabindex="0" data-detail-type="'+escAttr(typ)+'" data-detail-id="'+escAttr(it.id)+'" onclick="openDetail(\''+typ+'\',\''+escJs(it.id)+'\')"><div class="lr-card-thumb">'+renderListThumb(row,thumbKind,Math.round(px))+'</div><div class="lr-card-name">'+esc(it.name||'')+'</div></div>'});return'<div class="bt-strip">'+h+'</div>'}
