...
function btVoteHeaderToolsMarkup(captionId,btnId,inHeader){const cls='bt-th-combo-vote-tools'+(inHeader?' bt-th-combo-vote-tools--in-header':'');return'<div class="'+cls+'">'+btVoteModeCaptionMarkup(captionId)+btVoteModeBtnMarkup(btnId)+'</div>'}
function btSyncMobileVoteBar(){const bar=document.getElementById('btMobileVoteBar');if(!bar)return;const hasTable=!!document.querySelector('#bannerTimelineRoot .bt-table');if(!hasTable){bar.hidden=true;bar.innerHTML='';return}bar.hidden=false;bar.innerHTML='<div class="bt-mobile-vote-bar-inner"><span class="bt-mobile-vote-bar-label">'+esc(t('bt_col_featured'))+'</span>'+btVoteHeaderToolsMarkup('btVoteModeCaptionMobile','btVoteModeBtnMobile',false)+'</div>'}
