...
async function fetchBtVoteTotals(){try{const cid=btVoteClientId();const r=await fetch('/api/banner_timeline/votes?client_id='+encodeURIComponent(cid),{credentials:'same-origin',cache:'no-store'});if(!r.ok)return;const d=await r.json();S.btVoteTotals=d.totals||{};S.btVoteMine=d.mine||{}}catch(_){}}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={}}
