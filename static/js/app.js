...
const net=await fetchDetailPayload(type,id,baseOpts);
...
const st=S.currentDetailData;
if(!st||S.currentDetailType!=='stage'||st.content_locked){openDetail(kind,id);return}
const row=(st.npc_details||[]).find(r=>String(r.npc_id)===String(npcId));
if(!row){openDetail(kind,id);return}
const emb=kind==='character'?row.character:row.unit;
if(!emb){openDetail(kind,id);return}
openDetail(kind,id,{stageNpcEmbed:emb,stageNpcKey:String(npcId),stageNpcContext:{npc_id:String(npcId),unit:_cloneDetailJson(row.unit||null),character:_cloneDetailJson(row.character||null)},returnStageSnapshot:_cloneDetailJson(st)});
}
...
function getNpcCharacterChanceSupportState(d){const parts=_npcDetailEntityAbilityDetailStrings(d);const tx=parts.join('\n');let defSum=0,atkSum=0,chanceForce=0,m;const reDef=/Support\s+Defense(?:\s*\([^)]*\))?[\s\S]{0,200}?[+\uFF0B]\s*(\d+)/gi;while((m=reDef.exec(tx)))defSum+=parseInt(m[1],10);const reAtkCC=/Support\s+Attack\s*\/\s*Counter[\s\S]{0,200}?[+\uFF0B]\s*(\d+)/gi;while((m=reAtkCC.exec(tx)))atkSum+=parseInt(m[1],10);const reAtkLoose=/Support\s+Attack(?!\s*\/\s*Counter)[\s\S]{0,160}?[+\uFF0B]\s*(\d+)/gi;while((m=reAtkLoose.exec(tx)))atkSum+=parseInt(m[1],10);const reForce=/Force\s+activate\s+Chance\s+Step\s+(\d+)/gi;while((m=reForce.exec(tx)))chanceForce+=parseInt(m[1],10);const chanceCount=Math.max(1,1+chanceForce);return{chanceCount,supportDefCount:defSum,supportAtkCount:atkSum}}
function getNpcPilotAccuracyPctBonusFromContext(unitData){const ch=unitData&&unitData.detail_npc_context&&unitData.detail_npc_context.character;if(!ch)return 0;const tx=_npcDetailEntityAbilityDetailStrings(ch).join('\n');let sum=0,m;const re=/Increases\s+Accuracy\s+by\s+(\d+)\s*%/gi;while((m=re.exec(tx)))sum+=parseInt(m[1],10);return sum}
