...
const RANK_SORT_KEYS_UNIT=['HP','EN','ATK','DEF','MOB','MOV'];
...
function renderRankingStatPills(){const host=document.getElementById('rankingStatPills');if(!host)return;const mode=S.ranking.mode;const sort=mode==='characters'?S.ranking.sortChar:S.ranking.sortUnit;const keys=rankingSortKeys();host.innerHTML=keys.map(k=>`<button type="button" class="ranking-stat-pill${k===sort?' active':''}" data-rank-stat="${escAttr(k)}" onclick="setRankingSortKey('${escJs(k)}')">${esc(tStat(k,mode==='characters'?'character':'unit'))}</button>`).join('');const db=document.getElementById('rankingDirBtn');if(db){const d=mode==='characters'?S.ranking.dirChar:S.ranking.dirUnit;db.textContent=d==='desc'?'▼':'▲';db.title=d==='desc'?t('sort_desc'):t('sort_asc');db.setAttribute('aria-label',d==='desc'?t('sort_desc'):t('sort_asc'))}}
...
function renderRankingList(d){const rows=d.rows||[];const bounds=d.stat_bounds;const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';const isChar=S.ranking.mode==='characters';let h='';rows.forEach((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const val=row[sortKey];const valStr=fmtN(val);const thumb=renderListThumb(row,isChar?'char':'unit',56);const name=esc(row.name||'');const typ=isChar?'character':'unit';const ro=rankingDetailOptsFromContext(typ);const oj=encodeURIComponent(JSON.stringify(ro));h+=`<button type="button" class="ranking-row" data-detail-type="${typ}" data-detail-id="${escAttr(String(row.id))}" data-detail-opts="${oj}" onclick="openDetailFromRanking(this)"><span class="ranking-rank-num">#${rank}</span><span class="ranking-row-thumb">${thumb}</span><span class="ranking-row-name">${name}</span><span class="ranking-row-stat"><span class="ranking-stat-val">${valStr}</span>${renderRankingRowBar(val,bounds)}</span></button>`});host.innerHTML=h;renderPag('rank',d);const pp=document.getElementById('rankPerPage');if(pp){const v=String(d.per_page||50);if([...pp.options].some(o=>o.value===v))pp.value=v}const tc=document.getElementById('rankCharToolbarCount'),tu=document.getElementById('rankUnitToolbarCount');const cnt=`<span class="result-count-num">${d.total}</span> ${isChar?t('count_char'):t('count_unit')}`;if(tc)tc.innerHTML=isChar?cnt:'';if(tu)tu.innerHTML=isChar?'':cnt}
...
function initToolbarFilterIcons(){ensureUnitUltRarityRow();['char','unit','supp','rankChar','rankUnit'].forEach(p=>fillRarityPanelIcons(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillSourcePanel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar();['char','rankChar'].forEach(p=>updateAbilBrowseFilterLabel(p))}
