...
function detailRankingPosText(meta){if(!meta||!meta.rank||!meta.total)return'-';const rk=Number(meta.rank)||0,tt=Number(meta.total)||0;if(S.lang==='EN')return`${rk}th/${tt}`;return`${fmtN(rk)}位/${fmtN(tt)}`}
...
function syncUnitListSspForWeaponEffectFilters(){const sk=unitWeaponDebuffStateKey();const sel=Array.isArray(S[sk])?S[sk]:[];if(!sel.includes('preemptive')&&!sel.includes('range_6'))return;const sspKey=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units'?'listRankUnitSsp':'listUnitSsp';const spKey=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units'?'listRankUnitSp':'listUnitSp';if(S[sspKey]&&!S[spKey])return;S[sspKey]=true;S[spKey]=false;if(sspKey==='listUnitSsp'){syncListUnitStatToggles();persistListStatToggles()}else syncRankListStatToggleUi()}
...
const meta=detailRankingMetaFor(type,d.id,s.name);const w=detailRankingBarWidth(meta);const pos=detailRankingPosText(meta);
return`<div class="stat-card stat-card--ranking ${hcb?'has-cond-bonus':''}"><div class="stat-rank-head"><div class="stat-card-label stat-rank-label">${esc(tStat(s.name,type))}</div><div class="stat-rank-bar"><span class="stat-rank-fill" style="width:${w.toFixed(2)}%"></span></div><div class="stat-rank-pos">${esc(pos)}</div></div><div class="stat-card-value stat-rank-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`
}).join('');
...
function renderRankingList(d){const rows=d.rows||[];const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';const disp=rows.slice(0,20);if(vm==='podium'){host.classList.add('ranking-list-inner--podium');host.innerHTML=disp.length?`<div class="ranking-podium-board ranking-podium-board--dashboard">${disp.map((row,idx)=>{const rank=idx+1;const val=Number(row&&row[sortKey]||0);const id=escAttr(String(row&&row.id||''));const nm=esc(row&&row.name||'-');const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));const img=renderListThumb(row,thumbKind,54);const tier=rank===1?'tier-1':rank===2?'tier-2':rank===3?'tier-3':rank<=10?'tier-4':'tier-5';return`<button type="button" class="ranking-dash-card ${tier}" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-dash-badge">${rank}</span><span class="ranking-dash-thumb">${img}</span><span class="ranking-dash-name">${nm}</span><span class="ranking-dash-val">${fmtN(val)}</span></button>`}).join('')}</div>`:`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';const vals=disp.map(r=>Number(r&&r[sortKey]||0));const mx=Math.max(1,...vals);host.innerHTML=disp.map((row,idx)=>{const rank=idx+1;const statVal=Number(row&&row[sortKey]||0);const norm=Math.max(0,Math.min(1,statVal/mx));const w=Math.max(12,norm*100);const name=esc(row&&row.name||'-');const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,48);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-matrix-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-matrix-thumb">${img}</span><span class="ranking-matrix-name">${name}</span><span class="ranking-matrix-track"><span class="ranking-matrix-fill" style="width:${w.toFixed(2)}%"></span><span class="ranking-matrix-dot">${rank}</span></span><span class="ranking-matrix-val">${fmtN(statVal)}</span></button>`}).join('');syncRankingViewModeUi();const pg=document.querySelector('#panel-ranking .pagination-bar-wrap');if(pg)pg.style.display='none'}
