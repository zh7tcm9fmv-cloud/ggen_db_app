...
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const showBtn=show&&!S.btVoteMode;const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);document.querySelectorAll('.bt-vote-mode-btn').forEach(btn=>{const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',showBtn)});syncNavTabsOverflowHints()}
...
async function bootstrapGameNewsNotices(){ensureNavGameNewsNoticeFlare();const lang=normalizeGameNewsLangKey(S.lang||'EN');const seen=readGameNewsSeen(lang);try{const r=await fetch('/api/game_news/status?lang='+encodeURIComponent(lang)+'&last_seen_at='+encodeURIComponent(String(seen.at||0))+'&last_seen_fp='+encodeURIComponent(seen.fp||''),{credentials:'same-origin',cache:'no-store'});if(!r.ok){updateGameNewsNavNotice(false);return}const d=await r.json();const hasNew=!!d.has_new;S._gameNewsStatus=d;updateGameNewsNavNotice(hasNew)}catch(_){updateGameNewsNavNotice(false)}}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};btBindVoteDelegation();ensureNavBannerTimelineNoticeFlare();void bootstrapBtVoteNotices();ensureNavGameNewsNoticeFlare();void bootstrapGameNewsNotices();initGameNewsNoticeRefresh();initNavTabsOverflowHints()}
