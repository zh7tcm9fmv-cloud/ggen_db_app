...
dc_def_npc_map_bonuses:'NPC分隊バフ',dc_def_npc_map_bonuses_tip:'オフにすると、マップ上の「他の敵NPC」が付与する分隊／味方全体系のMS%加算だけを外します。この機体・搭乗パイロット自身のパッシブ（この機体側の「分隊」表現を含む）は残ります。HP・EN・攻撃力・防御力・機動力・移動力の表示とダメージ／命中に反映。パイロット欄は従来どおり当該パイロットのパッシブのみ。',tb_stats_hint_squad:' タグが一致する分隊被動（攻撃力の積み上げまたは攻撃力＋防御力）は編成から自動反映されます。',dc_wpn_trait_effects:'武装威力（効果による加算）',dc_wpn_trait_dist:'距離による加算',dc_wpn_trait_custom_core:'カスタムコア（SSP）',dc_wpn_trait_hp:'残りHPによる加算',dc_wpn_trait_mp:'MP／戦闘開始時による加算',dc_wpn_trait_max_applied:'上限まで自動反映（ダメージ計算）',dc_wpn_trait_ssp_hint:'カスタムコアの武装効果を反映するには、上の機体モードをSSPにしてください。',dc_wpn_trait_lines:'特性：',dc_wpn_trait_override_note:'最終武装威力を上書き中です。以下は自動計算の特効内訳（ダメージには反映されません）。',unit_filter_weapon_range_nm_ssp_ex_only:'SSP/EXのみ',unit_filter_weapon_range_nm_ssp_ex_all:'SSP/EXのみ 全射程',unit_filter_map_weapon_range:'マップ武装タイプ',unit_filter_map_weapon_range_all:'マップタイプすべて',unit_filter_map_weapon_range_multi:'{n} 種類のマップタイプ',unit_filter_mwrt_none:'すべて',unit_filter_mwrt_around_myself:'自身の周囲',unit_filter_mwrt_impact_range:'着弾範囲',unit_filter_mwrt_specify_direction:'方向指定',unit_filter_mwrt_moving_attack:'移動攻撃',unit_filter_mwrt_whole_map:'マップ全体',unit_filter_mwrt_fixed_position:'固定位置'});
Object.assign(T.TW,{npc_unit_cond_targets:'條件被動 — 加成對象機體',npc_unit_cond_target_short:'加成機體'});
Object.assign(T.HK,{npc_unit_cond_targets:'條件被動 — 加成對象機體',npc_unit_cond_target_short:'加成機體'});
...
Object.assign(T.EN,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'Rewards',stage_first_clear_rewards:'First-Clear Reward',detail_back_stage:'Back to stage'});
Object.assign(T.TW,{stage_source_tower:'世代之塔',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'獎勵',stage_first_clear_rewards:'首次通關報酬',detail_back_stage:'返回關卡'});
Object.assign(T.HK,{stage_source_tower:'世代之塔',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'獎勵',stage_first_clear_rewards:'首次通關報酬',detail_back_stage:'返回關卡'});
Object.assign(T.JA,{stage_source_tower:'ジェネレーションタワー',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'報酬',stage_first_clear_rewards:'初回クリア報酬',detail_back_stage:'ステージに戻る'});
Object.assign(T.JP,{stage_source_tower:'ジェネレーションタワー',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'報酬',stage_first_clear_rewards:'初回クリア報酬',detail_back_stage:'ステージに戻る'});
...
function initToolbarFilterIcons(){ensureUnitUltRarityRow('unit');ensureUnitUltRarityRow('rankUnit');ensureUnitWeaponRangeNonMapFilterDom();ensureRankUnitWeaponRangeNonMapFilterDom();pruneUnitMapWeaponRangeSelectionForUi();syncMapWeaponRangeFilterVisibility();['char','unit','supp','rankChar','rankUnit'].forEach(p=>fillRarityPanelIcons(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillSourcePanel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar();['char','rankChar'].forEach(p=>updateAbilBrowseFilterLabel(p));updateUnitWeaponRangeNonMapFilterLabel('unit');updateUnitWeaponRangeNonMapFilterLabel('rankUnit')}
...
function closeAllUnitWeaponRangePanels(){['unit','rankUnit'].forEach(pfx=>{['WeaponRange','WeaponRangeNonMap','MapWeaponRange'].forEach(mid=>{const panel=document.getElementById(pfx+mid+'FilterPanel');const btn=document.getElementById(pfx+mid+'FilterBtn');if(panel)panel.hidden=true;if(btn)btn.setAttribute('aria-expanded','false')})})}
...
function syncUnitListSspForWeaponRangeFilters(){const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';const wrSk=rk?'listRankUnitWeaponRange':'listUnitWeaponRange';const nmSk=rk?'listRankUnitWeaponRangeNonMap':'listUnitWeaponRangeNonMap';const selWr=Array.isArray(S[wrSk])?S[wrSk]:[];const selNm=Array.isArray(S[nmSk])?S[nmSk]:[];const needsSsp=selWr.includes('5')||selWr.includes('6')||selNm.includes('5')||selNm.includes('6');const armedKey=rk?'_rankUnitWeaponRangeSspAutoArmed':'_unitWeaponRangeSspAutoArmed';if(!needsSsp){S[armedKey]=false;return}if(S[armedKey])return;const sspKey=rk?'listRankUnitSsp':'listUnitSsp';const spKey=rk?'listRankUnitSp':'listUnitSp';S[sspKey]=true;S[spKey]=false;S[armedKey]=true;if(sspKey==='listUnitSsp'){syncListUnitStatToggles();persistListStatToggles()}else syncRankListStatToggleUi()}
...
function clearUnitWeaponRangeOnly(){const sk=unitWeaponRangeStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];S[rk?'_rankUnitWeaponRangeSspAutoArmed':'_unitWeaponRangeSspAutoArmed']=false;const pfx=unitWeaponRangeDomPrefix();const body=document.getElementById(pfx+'WeaponRangeFilterBody');if(body)body.innerHTML=buildUnitWeaponRangeFilterHtml();updateUnitWeaponRangeFilterLabel();syncUnitListSspForWeaponRangeFilters();if(rk)void loadRankingList(1);else loadUnits(1)}
function buildUnitWeaponRangeNonMapFilterHtml(skOpt){const sk=skOpt||unitWeaponRangeNonMapStateKey();const selArr=Array.isArray(S[sk])?S[sk]:[];const sel=selArr.length?String(selArr[0]):'';const pfx=weaponRangeLabelPrefix();let h='<div class="unit-debuff-grid"><div class="unit-debuff-grid-row">';UNIT_WEAPON_RANGE_VALUES.forEach((v,idx)=>{const active=sel===v?' is-active':'';h+=`<button type="button" class="unit-debuff-item${active}" onclick="onUnitWeaponRangeNonMapRowClick('${v}',event)" onkeydown="onUnitWeaponRangeNonMapRowKey(event,'${v}')"><span>${esc(pfx+' '+v)}</span></button>`;if(idx%2===1&&idx<UNIT_WEAPON_RANGE_VALUES.length-1)h+='</div><div class="unit-debuff-grid-row">' });if(UNIT_WEAPON_RANGE_VALUES.length%2===1)h+='<span class="unit-debuff-fill" aria-hidden="true"></span>';h+='</div></div>';return h}
...
function onUnitWeaponRangeNonMapChange(){syncUnitListSspForWeaponRangeFilters();updateUnitWeaponRangeNonMapFilterLabel();if(S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units')scheduleRankingListReload();else scheduleBrowseListReload('units')}
function onUnitWeaponRangeNonMapRowClick(v,ev){if(ev){ev.preventDefault();ev.stopPropagation()}const sk=unitWeaponRangeNonMapStateKey();const cur=Array.isArray(S[sk])&&S[sk].length?String(S[sk][0]):'';S[sk]=(cur===String(v)?[]:[String(v)]);syncUnitWeaponRangeNonMapCheckboxes();onUnitWeaponRangeNonMapChange()}
...
function clearUnitWeaponRangeNonMapOnly(){const sk=unitWeaponRangeNonMapStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];const pfx=unitWeaponRangeDomPrefix();const sxk=pfx==='rankUnit'?'listRankUnitWeaponRangeNonMapSspExOnly':'listUnitWeaponRangeNonMapSspExOnly';S[sxk]=false;const body=document.getElementById(pfx+'WeaponRangeNonMapFilterBody');if(body)body.innerHTML=buildUnitWeaponRangeNonMapFilterHtml();syncWeaponRangeNonMapSspExFooterBtn(pfx);updateUnitWeaponRangeNonMapFilterLabel();syncUnitListSspForWeaponRangeFilters();if(rk)void loadRankingList(1);else loadUnits(1)}
function ensureRankUnitWeaponRangeNonMapFilterDom(){const host=document.getElementById('rankingUnitToolbarHost');if(!host)return;const rw=document.getElementById('rankUnitWeaponRangeWrap');if(rw)rw.remove();const debWrap=document.getElementById('rankUnitWeaponDebuffWrap');if(!debWrap)return;if(document.getElementById('rankUnitWeaponRangeNonMapWrap'))return;const clone=debWrap.cloneNode(true);clone.id='rankUnitWeaponRangeNonMapWrap';const btn=clone.querySelector('#rankUnitWeaponDebuffFilterBtn');if(btn){btn.id='rankUnitWeaponRangeNonMapFilterBtn';btn.setAttribute('onclick','toggleUnitWeaponRangeNonMapPanel(event)');btn.setAttribute('title',weaponRangeNonMapFilterName());btn.setAttribute('aria-expanded','false')}const lbl=clone.querySelector('#rankUnitWeaponDebuffFilterLabel');if(lbl){lbl.id='rankUnitWeaponRangeNonMapFilterLabel';lbl.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeNonMapAllText())}</span>`}const panel=clone.querySelector('#rankUnitWeaponDebuffFilterPanel');if(panel){panel.id='rankUnitWeaponRangeNonMapFilterPanel';panel.classList.add('weapon-range-non-map-panel');panel.hidden=true}const body=clone.querySelector('#rankUnitWeaponDebuffFilterBody');if(body){body.id='rankUnitWeaponRangeNonMapFilterBody';body.innerHTML=''}debWrap.insertAdjacentElement('afterend',clone);updateUnitWeaponRangeNonMapFilterLabel('rankUnit')}
function ensureUnitWeaponRangeNonMapFilterDom(){const host=document.getElementById('panel-units');if(!host)return;const uw=document.getElementById('unitWeaponRangeWrap');if(uw)uw.remove();const debWrap=document.getElementById('unitWeaponDebuffWrap');if(!debWrap)return;if(document.getElementById('unitWeaponRangeNonMapWrap'))return;const clone=debWrap.cloneNode(true);clone.id='unitWeaponRangeNonMapWrap';const btn=clone.querySelector('#unitWeaponDebuffFilterBtn');if(btn){btn.id='unitWeaponRangeNonMapFilterBtn';btn.setAttribute('onclick','toggleUnitWeaponRangeNonMapPanel(event)');btn.setAttribute('title',weaponRangeNonMapFilterName());btn.setAttribute('aria-expanded','false')}const lbl=clone.querySelector('#unitWeaponDebuffFilterLabel');if(lbl){lbl.id='unitWeaponRangeNonMapFilterLabel';lbl.innerHTML=`<span class="source-filter-btn-plain">${esc(weaponRangeNonMapAllText())}</span>`}const panel=clone.querySelector('#unitWeaponDebuffFilterPanel');if(panel){panel.id='unitWeaponRangeNonMapFilterPanel';panel.classList.add('weapon-range-non-map-panel');panel.hidden=true}const body=clone.querySelector('#unitWeaponDebuffFilterBody');if(body){body.id='unitWeaponRangeNonMapFilterBody';body.innerHTML=''}debWrap.insertAdjacentElement('afterend',clone);updateUnitWeaponRangeNonMapFilterLabel('unit')}
function buildUnitMapWeaponRangeFilterHtml(skOpt){const sk=skOpt||unitMapWeaponRangeStateKey();const selArr=Array.isArray(S[sk])?S[sk]:[];const selSet=new Set(selArr.map(v=>String(v)));const defs=getMapWeaponRangeTypeDefsForUi();let h='<div class="unit-debuff-grid">';for(let i=0;i<defs.length;i+=2){const a=defs[i],b=defs[i+1];const mk=(d)=>{if(!d)return'<span class="unit-debuff-fill" aria-hidden="true"></span>';const active=selSet.has(String(d.value))?' is-active':'';return`<button type="button" class="unit-debuff-item${active}" onclick="onUnitMapWeaponRangeRowClick('${d.value}',event)" onkeydown="onUnitMapWeaponRangeRowKey(event,'${d.value}')"><span>${esc(t(d.labelKey))}</span></button>`};h+=`<div class="unit-debuff-grid-row">${mk(a)}${mk(b)}</div>`}h+='</div>';return h}
...
function updateUnitMapWeaponRangeFilterLabel(forcePfx){const pfx=forcePfx||unitWeaponRangeDomPrefix();const sk=pfx==='rankUnit'?'listRankUnitMapWeaponRange':'listUnitMapWeaponRange';const label=document.getElementById(pfx+'MapWeaponRangeFilterLabel');const btn=document.getElementById(pfx+'MapWeaponRangeFilterBtn');if(!label||!btn)return;btn.title=t('unit_filter_map_weapon_range');let sel=S[sk];if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_map_weapon_range_all'))}</span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){const def=getMapWeaponRangeTypeDefsForUi().find(d=>String(d.value)===String(sel[0]))||MAP_WEAPON_RANGE_TYPE_DEFS.find(d=>String(d.value)===String(sel[0]));const txt=def?t(def.labelKey):String(sel[0]);label.innerHTML=`<span class="source-filter-btn-plain" style="font-size:13px;font-weight:600">${esc(txt)}</span>`;return}label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_map_weapon_range_multi').replace('{n}',String(sel.length)))}</span>`}
...
function getWeaponRangeOpSuffix(){return browseCombineOpSuffix('weapon_range_op',S.listUnitWeaponRange,S.browseCombWr)}
...
function onLineageItemChange(p){const domP=p==='char'?'char':(p==='rankChar'?'rankChar':(p==='supp'?'supp':(p==='rankUnit'?'rankUnit':'unit')));delete S['_linLblRf'+domP];const key=listLineageKey(p);const checkedSet=new Set(Array.from(document.querySelectorAll(`input[name="${p}LineageSel"].lineage-filter-item:checked`)).map(i=>i.value));const prev=Array.isArray(S[key])?S[key]:[];const next=[];prev.forEach(id=>{if(checkedSet.has(id))next.push(id)});checkedSet.forEach(id=>{if(!next.includes(id))next.push(id)});S[key]=next.length?next:[];syncLineageCombinePriorityMark(p);syncBrowseLineageNameHints(p);updateLineageFilterButtonLabel(p);if(p==='char')scheduleBrowseListReload('characters');else if(p==='supp')scheduleBrowseListReload('supporters');else if(p==='unit')scheduleBrowseListReload('units');else if(p==='rankChar'||p==='rankUnit')scheduleRankingListReload()}
...
async function fetchJsonWithWarmupRetry(url,{maxRetries=40}={}){let waitMs=300;for(let i=0;i<maxRetries;i++){const r=await fetch(url);let d;try{d=await r.json()}catch(_){d={}}if(r.status===503&&d&&d.error==='warming_up'){await new Promise(res=>setTimeout(res,waitMs));waitMs=Math.min(Math.round(waitMs*1.35),1200);continue}return{ok:r.ok,status:r.status,data:d}}return{ok:false,status:503,data:{error:'warming_up'}}}
...
async function bootstrapBtVoteNotices(){try{const reqs=[fetch('/api/banner_timeline/votes',{credentials:'same-origin',cache:'no-store'})];if(!S.btCacheData)reqs.push(fetch('/api/banner_timeline?lang='+encodeURIComponent(S.lang||'EN'),{credentials:'same-origin'}));const rs=await Promise.all(reqs);if(rs[0]&&rs[0].ok){const d=await rs[0].json();S.btVoteTotals=d.totals||{};S.btVoteMine=d.mine||{}}if(!S.btCacheData&&rs[1]&&rs[1].ok)S.btCacheData=await rs[1].json();S._btVoteStatusLoaded=true}catch(_){}updateBtVoteNotices();if(window.GgenContentNotices){GgenContentNotices.setBtState({banners:S.btCacheData&&S.btCacheData.banners,mine:S.btVoteMine,totals:S.btVoteTotals,loaded:S._btVoteStatusLoaded});GgenContentNotices.updateBtVoteNotices()}}
...
function initBannerTimelineExtras(){initScrollTopFab();if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};btBindVoteDelegation();initNavTabsOverflowHints();bootstrapContentNoticesAll()}
...
async function loadStages(p=1){const s=S.stages;s.page=p;s.q=document.getElementById('stageFilter').value.trim();const pp=document.getElementById('stagePerPage').value;const url=buildStagesListUrl(p,pp);const cached=browseListJsonCacheGet(url);if(cached){try{renderStageT(cached);renderPag('stage',cached);setBrowseToolbarCount('stage',`<span class="result-count-num">${cached.total}</span> ${t('count_stage')}`)}catch(e){document.getElementById('stageBody').innerHTML='';const _stg=document.getElementById('stageGrid');if(_stg)_stg.innerHTML='';document.getElementById('stageEmpty').style.display='block';setBrowseToolbarCount('stage','')}afterListLoadIfSpotlightOpen();return}showLoad('stage',true);setBrowseToolbarCount('stage','<span style="color:var(--text-muted)">\u2026</span>');try{const r=await fetch(url);const d=await r.json();browseListJsonCacheSet(url,d);renderStageT(d);renderPag('stage',d);setBrowseToolbarCount('stage',`<span class="result-count-num">${d.total}</span> ${t('count_stage')}`)}catch(e){document.getElementById('stageBody').innerHTML='';const _stg=document.getElementById('stageGrid');if(_stg)_stg.innerHTML='';document.getElementById('stageEmpty').style.display='block';setBrowseToolbarCount('stage','')}finally{showLoad('stage',false);afterListLoadIfSpotlightOpen()}}
...
function renderCharTable(data){const thSz=tableListThumbSize();document.getElementById('charBody').innerHTML=data.rows.map(r=>`<tr data-detail-type="character" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('character','${r.id}')"><td class="col-thum">${renderListThumb(r,'char',thSz)}</td><td class="col-name"><div class="name-cell"><span class="name-text">${esc(r.name)}</span></div></td><td class="col-stat">${charListStatCell(r,'Ranged')}</td><td class="col-stat">${charListStatCell(r,'Melee')}</td><td class="col-stat">${charListStatCell(r,'Awaken')}</td><td class="col-stat">${charListStatCell(r,'Defense')}</td><td class="col-stat">${charListStatCell(r,'Reaction')}</td></tr>`).join('')}
...
function renderCharGridSkills(data){const grid=document.getElementById('charGrid'),th=gridListThumbSize(),cmpLbl=escAttr(t('cmp_compare'));grid.innerHTML=data.rows.map(r=>{const strip=renderCharGridSkillStrip(r.grid_skills||[],3);return`<div class="list-grid-card list-grid-card--cmp list-grid-card--skills list-grid-card--char-skills-v2" data-detail-type="character" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('character','${r.id}')"><div class="list-grid-card-main-row"><div class="list-grid-card-char-col"><div class="list-grid-card-thumb">${renderListThumb(r,'char',th)}</div><div class="list-grid-card-body"><div class="list-grid-card-name">${esc(r.name)}</div></div><div class="list-grid-card-skills-hr">${strip}</div></div></div><div class="list-grid-cmp-wrap" onclick="event.stopPropagation()" title="${cmpLbl}"><span class="list-grid-cmp-inner"><input type="checkbox" class="cmp-cb cmp-cb-grid" data-cmp-id="${escAttr(String(r.id))}" aria-label="${cmpLbl}"></span></div></div>`}).join('');wireGridCompareCheckboxes('charGrid','character')}
...
function renderUnitTable(data){const thSz=tableListThumbSize();document.getElementById('unitBody').innerHTML=data.rows.map(r=>`<tr data-detail-type="unit" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('unit','${r.id}')"><td class="col-thum">${renderListThumb(r,'unit',thSz)}</td><td class="col-name"><div class="name-cell"><span class="name-text">${esc(r.name)}</span><span class="name-special-icons">${(r.special_icons||[]).filter(ic=>ic&&!String(ic).includes('Source')).map(ic=>pictureRasterHtml(ic,{cls:'name-special-icon',loading:'lazy',alt:'',onerror:"this.style.display='none'",lazy:false})).join('')}</span></div></td><td class="col-stat"><span class="stat-value">${fmtN(r.HP)}</span></td><td class="col-stat"><span class="stat-value">${fmtN(r.EN)}</span></td><td class="col-stat"><span class="stat-value">${fmtN(r.ATK)}</span></td><td class="col-stat"><span class="stat-value">${fmtN(r.DEF)}</span></td><td class="col-stat"><span class="stat-value">${fmtN(r.MOB)}</span></td><td class="col-stat"><span class="stat-value">${fmtN(r.MOV)}</span></td></tr>`).join('')}
...
function renderUnitGridStats(data){const grid=document.getElementById('unitGrid'),th=gridListThumbSize(),cmpLbl=escAttr(t('cmp_compare'));grid.innerHTML=data.rows.map(r=>`<div class="list-grid-card list-grid-card--cmp" data-detail-type="unit" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('unit','${r.id}')"><div class="list-grid-card-thumb">${renderListThumb(r,'unit',th)}</div><div class="list-grid-card-body"><div class="list-grid-card-name">${esc(r.name)}</div><div class="list-grid-card-meta">${(r.special_icons||[]).filter(ic=>ic&&!String(ic).includes('Source')).map(ic=>pictureRasterHtml(ic,{cls:'name-special-icon',loading:'lazy',alt:'',onerror:"this.style.display='none'",lazy:false})).join('')}</div><div class="list-grid-card-stats">${t('col_hp')} ${fmtN(r.HP)} · ${t('col_en')} ${fmtN(r.EN)} · ${t('col_atk')} ${fmtN(r.ATK)} · ${t('col_def')} ${fmtN(r.DEF)} · ${t('col_mob')} ${fmtN(r.MOB)} · ${t('col_mov')} ${fmtN(r.MOV)}</div></div><div class="list-grid-cmp-wrap" onclick="event.stopPropagation()" title="${cmpLbl}"><span class="list-grid-cmp-inner"><input type="checkbox" class="cmp-cb cmp-cb-grid" data-cmp-id="${escAttr(String(r.id))}" aria-label="${cmpLbl}"></span></div></div>`).join('');wireGridCompareCheckboxes('unitGrid','unit')}
function renderUnitGridSkills(data){const grid=document.getElementById('unitGrid'),th=gridListThumbSize(),cmpLbl=escAttr(t('cmp_compare'));grid.innerHTML=data.rows.map(r=>{const strip=renderCharGridSkillStrip(r.grid_abilities||[]);return`<div class="list-grid-card list-grid-card--cmp list-grid-card--skills list-grid-card--unit-skills-v2" data-detail-type="unit" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('unit','${r.id}')"><div class="list-grid-card-main-row"><div class="list-grid-card-char-col"><div class="list-grid-card-thumb">${renderListThumb(r,'unit',th)}</div><div class="list-grid-card-body"><div class="list-grid-card-name">${esc(r.name)}</div></div><div class="list-grid-card-skills-hr">${strip}</div></div></div><div class="list-grid-cmp-wrap" onclick="event.stopPropagation()" title="${cmpLbl}"><span class="list-grid-cmp-inner"><input type="checkbox" class="cmp-cb cmp-cb-grid" data-cmp-id="${escAttr(String(r.id))}" aria-label="${cmpLbl}"></span></div></div>`}).join('');wireGridCompareCheckboxes('unitGrid','unit')}
...
function renderSuppTable(data){const thSz=tableListThumbSize();document.getElementById('suppBody').innerHTML=data.rows.map(r=>`<tr data-detail-type="supporter" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('supporter','${r.id}')"><td class="col-thum">${renderListThumb(r,'supp',thSz)}</td><td class="col-name"><div class="name-cell"><span class="name-text">${esc(r.name)}</span></div></td><td class="col-tag"><div class="detail-tags-row" style="margin-top:0;align-items:center;">${renderSkillTags(r.skill_tag_data)}</div></td><td class="col-boost"><div class="skill-summary" style="-webkit-line-clamp:3;white-space:pre-wrap;">${esc(r.boost)}</div></td><td class="col-skill">${r.active_icon?pictureRasterHtml(r.active_icon,{loading:'lazy',alt:'',extra:'style="width:44px;height:44px;border:none;background:transparent;object-fit:contain;margin:0 auto;display:block;"',onerror:"this.style.display='none'",lazy:false}):'<div style="text-align:center;color:var(--text-muted)">-</div>'}</td></tr>`).join('')}
function renderSuppGrid(data){const grid=document.getElementById('suppGrid'),th=gridListThumbSize();grid.innerHTML=data.rows.map(r=>`<div class="list-grid-card" data-detail-type="supporter" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('supporter','${r.id}')"><div class="list-grid-card-thumb">${renderListThumb(r,'supp',th)}</div><div class="list-grid-card-body"><div class="list-grid-card-name">${esc(r.name)}</div><div class="detail-tags-row" style="margin-top:0;align-items:center;justify-content:center;">${renderSkillTags(r.skill_tag_data)}</div><div class="list-grid-card-stats" style="font-size:12px;margin-top:4px;">${esc(r.boost||'')}</div>${r.active_icon?`<div style="margin-top:6px;">${pictureRasterHtml(r.active_icon,{loading:'lazy',alt:'',extra:'style="width:44px;height:44px;object-fit:contain;margin:0 auto;display:block;"',onerror:"this.style.display='none'",lazy:false})}</div>`:''}</div></div>`).join('')}
...
function stageBrowseThumbTitleHtml(r,locked){if(locked)return`<span class="stage-browse-thumb__title">${esc(t('er_stage_redacted_row'))}</span>`;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';if(isSs)return`<span class="stage-browse-thumb__title">${esc(r.name)}</span><span class="stage-browse-thumb__subtitle">${esc(t('stage_source_special'))}</span>`;if(isTs||isSa)return`<span class="stage-browse-thumb__title">${esc(r.name)}</span>`;return`<span class="stage-browse-thumb__title"><b>${esc(t('col_stage_no'))} ${fmtN(r.stage_number)}</b> ${esc(r.name)}</span>`}
function renderStageBrowseThumb(r,opts){opts=opts||{};const variant=opts.variant||'table';const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const posterPortrait=isSs||isTs||isSa;const noShade=posterPortrait;const showDiff=!locked&&!(isSa||isSs||isTs);const diffCode=esc(r.difficulty_code||'unknown');const diffHtml=showDiff?`<span class="stage-diff-badge stage-diff-${diffCode} stage-browse-thumb__diff">${esc(r.difficulty_name||'-')}</span>`:'';const portCls=(isSs||isSa)?' stage-browse-thumb__portrait--banner':isTs?' stage-browse-thumb__portrait--poster':' stage-browse-thumb__portrait--unit';
const phPoster=(isSs||isSa)?' stage-browse-thumb__ph--banner':isTs?' stage-browse-thumb__ph--poster':' stage-browse-thumb__ph--unit';const portrait=locked?'':(r.portrait?imgTag(r.portrait,{cls:'stage-browse-thumb__portrait'+portCls,webp:true,alt:'',onerror:"this.style.display='none'"}) :'');const phFallback=!locked&&!r.portrait?'<span class="stage-browse-thumb__ph'+phPoster+'" aria-hidden="true">🗺️</span>':'';const lockOv=locked?'<span class="stage-browse-thumb__lock" aria-hidden="true">🔒</span>':'';const vCls=variant==='grid'?' stage-browse-thumb--grid':variant==='compact'?' stage-browse-thumb--compact':'';const themeCls=isSs?' stage-browse-thumb--special':(isTs||isSa)?' stage-browse-thumb--tower':'';const lockCls=locked?' stage-browse-thumb--locked':'';const shadeHtml=noShade?'':'<div class="stage-browse-thumb__shade" aria-hidden="true"></div>';return`<div class="stage-browse-thumb${vCls}${themeCls}${lockCls}" role="presentation"><div class="stage-browse-thumb__bg" aria-hidden="true"></div><div class="stage-browse-thumb__stripes" aria-hidden="true"></div>${portrait}${phFallback}${shadeHtml}${lockOv}<div class="stage-browse-thumb__ui">${diffHtml?`<div class="stage-browse-thumb__head">${diffHtml}${stageBrowseThumbTitleHtml(r,locked)}</div>`:`<div class="stage-browse-thumb__head stage-browse-thumb__head--no-diff">${stageBrowseThumbTitleHtml(r,locked)}</div>`}</div></div>`}
function renderStageTable(data){document.getElementById('stageBody').innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const diffCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSa||isSs||isTs)?'<span class="stat-value"></span>':`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span>`;const numCell=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'<span class="stat-value"></span>':`<span class="stat-value">${fmtN(r.stage_number)}</span>`;const nameCell=locked?`<span class="name-text er-stage-redacted-cell">${esc(t('er_stage_redacted_row'))}</span>`:`<span class="name-text">${esc(r.name)}</span>`;const cpDisp=locked?'—':fmtN(r.recommended_cp);const terDisp=locked?'—':esc(r.terrain);return`<tr data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><td class="col-thum"><div class="stage-thum-wrap">${renderStageBrowseThumb(r,{variant:'compact'})}</div></td><td class="col-diff">${diffCell}</td><td class="col-number">${numCell}</td><td class="col-name"><div class="name-cell">${nameCell}</div></td><td class="col-cp"><span class="stat-value">${cpDisp}</span></td><td class="col-terrain"><span class="stat-value">${terDisp}</span></td></tr>`}).join('')}
function renderStageGrid(data){const grid=document.getElementById('stageGrid');grid.innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const cpPart=locked?'—':fmtN(r.recommended_cp);const terPart=stageBrowseTerrainLabel(r.terrain,locked);return`<div class="list-grid-card list-grid-card--stage${locked?' list-grid-card--locked':''}" data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><div class="list-grid-card-thumb">${renderStageBrowseThumb(r,{variant:'grid'})}</div><div class="list-grid-card-body"><div class="list-grid-card-stats">${t('col_stage_cp')} ${cpPart} · ${terPart}</div></div></div>`}).join('')}
...
function ensureStageTowerFilterDom(){let wrap=document.getElementById('stageTowerWrap');if(wrap)return wrap;const host=document.querySelector('#panel-stages .list-toolbar-left');const diff=document.getElementById('stageDiffWrap');if(!host||!diff)return null;wrap=document.createElement('div');wrap.className='stage-source-toggle-wrap';wrap.id='stageTowerWrap';wrap.setAttribute('role','group');wrap.innerHTML=`<button type="button" class="stage-source-toggle-btn" id="stageTowerEastBtn" onclick="setStageTowerSide('E')" aria-pressed="false"><img src="${imgUrl(_stageTowerLogoBySide('E'))}" alt="" loading="lazy" style="width:24px;height:24px;object-fit:contain;margin-right:8px;vertical-align:middle"><span>${esc(_stageTowerLabelBySide('E'))}</span></button><button type="button" class="stage-source-toggle-btn" id="stageTowerWestBtn" onclick="setStageTowerSide('W')" aria-pressed="false"><img src="${imgUrl(_stageTowerLogoBySide('W'))}" alt="" loading="lazy" style="width:24px;height:24px;object-fit:contain;margin-right:8px;vertical-align:middle"><span>${esc(_stageTowerLabelBySide('W'))}</span></button>`;host.insertBefore(wrap,diff);return wrap}
...
function syncStageSourceToolbar(){const src=S.stages.source||'eternal';const diff=document.getElementById('stageDiffWrap');const tower=ensureStageTowerFilterDom();if(diff)diff.style.display=(src==='eternal')?'':'none';if(tower)tower.style.display=(src==='tower_stage')?'':'none';const bE=document.getElementById('stageSourceEternalBtn');const bT=document.getElementById('stageSourceTowerBtn');const bS=document.getElementById('stageSourceScoreBtn');const bP=document.getElementById('stageSourceSpecialBtn');if(bE)bE.textContent=t('stage_source_eternal');if(bT)bT.textContent=t('stage_source_tower');if(bS)bS.textContent=t('stage_source_score');if(bP)bP.textContent=t('stage_source_special');if(bE){bE.classList.toggle('active',src==='eternal');bE.setAttribute('aria-pressed',src==='eternal'?'true':'false')}if(bT){bT.classList.toggle('active',src==='tower_stage');bT.setAttribute('aria-pressed',src==='tower_stage'?'true':'false')}if(bS){bS.classList.toggle('active',src==='score_attack');bS.setAttribute('aria-pressed',src==='score_attack'?'true':'false')}if(bP){bP.classList.toggle('active',src==='special_stage');bP.setAttribute('aria-pressed',src==='special_stage'?'true':'false')}const side=(S.stages.towerSide==='W'||S.stages.towerSide==='E')?S.stages.towerSide:'E';S.stages.towerSide=side;const be=document.getElementById('stageTowerEastBtn');const bw=document.getElementById('stageTowerWestBtn');if(be){be.classList.toggle('active',side==='E');be.setAttribute('aria-pressed',side==='E'?'true':'false');const s=be.querySelector('span');if(s)s.textContent=_stageTowerLabelBySide('E')}if(bw){bw.classList.toggle('active',side==='W');bw.setAttribute('aria-pressed',side==='W'?'true':'false');const s=bw.querySelector('span');if(s)s.textContent=_stageTowerLabelBySide('W')}}
function setStageBrowseSource(mode){if(mode!=='eternal'&&mode!=='tower_stage'&&mode!=='score_attack'&&mode!=='special_stage')return;if((S.stages.source||'eternal')===mode)return;S.stages.source=mode;if(mode==='tower_stage'&&(S.stages.towerSide!=='E'&&S.stages.towerSide!=='W'))S.stages.towerSide='E';syncStageSourceToolbar();scheduleBrowseListReload('stages')}
...
else{S.stageMapExpanded=false;S.stageMapAutoFit=true;S.stageMapZoom=1;S.stageMapReinforcementOnly=false}
...
function renderMapObjectivesSection(d){
  const rows=Array.isArray(d&&d.map_objectives)?d.map_objectives:[];
  if(!rows.length)return'';
  const L=(k,fb)=>{const v=t(k);return(v&&v!==k)?v:fb};
  return`<div class="detail-section"><div class="section-title">${esc(L('sec_map_objectives','Map objectives'))}</div>${rows.map(x=>`<div class="stage-condition-item">${esc(x)}</div>`).join('')}</div>`;
}
...
function _stageMapShowStepOrderForUnit(u,pool){
  if(!S.stageMapReinforcementOnly||!u)return false;
...
function _stageMapStepOrderLabel(so){
  const L=(k,fb)=>{const v=t(k);return(v&&v!==k)?v:fb};
  return`${L('stage_map_step_order','Step')} ${fmtN(so)}`;
}
function stageMapHasReinfStepOrders(md){
  const pool=(md&&md.units)||[];
  return pool.some(u=>_stageMapShowStepOrderForUnit(u,pool));
}
...
function renderStageMapSection(d){
  ...
  const stepHint=ro&&stageMapHasReinfStepOrders(md)?(()=>{const L=(k,fb)=>{const v=t(k);return(v&&v!==k)?v:fb};return`<p class="stage-map-reinf-step-hint">${esc(L('stage_map_reinf_step_hint','Reinforcement spawn order (Step) is shown on the map and in the enemy list below.'))}</p>`})():'';
  const controls=S.stageMapExpanded?`<div class="stage-map-controls">${togglesRow}${stepHint}
...
function renderStageMapGrid(md){
  ...
  for(let y=win.maxY;y>=win.minY;y--){
    for(let x=win.minX;x<=win.maxX;x++){
      ...
      const showStepOrder=u&&_stageMapShowStepOrderForUnit(u,pool);
      ...
      if(showStepOrder)cellTitle+=` — ${_stageMapStepOrderLabel(_stageNpcStepOrder(u))}`;
...
function setStageMapReinforcementOnly(on){
  ...
  if(S.currentDetailType==='stage'){
    ...
    if(S.stageMapExpanded&&gridWrap&&md&&md.width>0&&md.height>0){
...
function renderStageNpcCompactTile(n,idx){
  ...
  const showStep=S.stageMapReinforcementOnly&&_stageMapShowStepOrderForUnit(pseudoU,pool);
  const badges=[];
  if(showStep)badges.push(`<span class="stage-npc-meta-badge stage-npc-meta-badge--step">${esc(_stageMapStepOrderLabel(_stageNpcStepOrder(n)))}</span>`);
...
function renderNpcDetails(list,stageId,ui){if(!list||!list.length)return`<div class="detail-section"><div class="section-title">${t('sec_npc_details')}</div><div class="ability-item"><div class="ability-info"><div class="ability-detail">${t('none')}</div></div></div></div>`;const sid=String(stageId!=null?stageId:'').replace(/[^a-zA-Z0-9_-]/g,'_')||'stage';const indexed=(list||[]).map((n,i)=>({n,idx:i}));const pool=S.currentDetailData?.map_data?.units||[];const guests=indexed.filter(o=>o.n.side==='guest');const friendly=indexed.filter(o=>o.n.side==='friendly');let enemies=indexed.filter(o=>o.n.side==='enemy');if(S.stageMapReinforcementOnly){
    enemies=enemies.filter(o=>{
      const mapU=_stageMapUnitByNpcId(pool,o.n.npc_id);
      return mapU&&_stageMapEnemyIsReinforcementSpawn(mapU,pool);
    });
    enemies.sort((a,b)=>_stageNpcStepOrder(a.n)-_stageNpcStepOrder(b.n));
  }
  const guestOpen=stageNpcGroupOpenFromUi(ui,sid,'guest',true);const friendlyOpen=stageNpcGroupOpenFromUi(ui,sid,'friendly',!guests.length);const enemyOpen=stageNpcGroupOpenFromUi(ui,sid,'enemy',S.stageMapReinforcementOnly||(!guests.length&&!friendly.length));const body=npcDetailsGroupSection(sid,'guest',t('stage_npc_guest_tab'),guests,guestOpen)+npcDetailsGroupSection(sid,'friendly',t('stage_npc_friendly_forces_tab'),friendly,friendlyOpen)+npcDetailsGroupSection(sid,'enemy',S.stageMapReinforcementOnly?(t('stage_npc_reinf_tab')!=='stage_npc_reinf_tab'?t('stage_npc_reinf_tab'):'Reinforcements'):t('stage_npc_enemy_tab'),enemies,enemyOpen);return`<div class="detail-section"><div class="section-title">${t('sec_npc_details')}</div>${body}</div>`}
...
function renderModifiers(rows){if(!rows||!rows.length)return'';return`<div class="detail-section"><div class="section-title">Modifier</div><div class="ability-list">${rows.map(md=>{const src=md&&typeof md==='object'?md:{};const details=Array.isArray(src.details)?src.details:[];const name=src.name||'Modifier';return`<div class="ability-item"><div class="ability-icon-wrap">${renderAbilIcon(src)}</div><div class="ability-info"><div class="ability-name">${esc(name)}</div>${details.map(d=>renderAbilityDetailChunks(d)).join('')}</div></div>`}).join('')}</div></div>`}
