...
async function toggleAbilPanel(p,ev){if(ev)ev.stopPropagation();const panel=document.getElementById(p+'AbilFilterPanel');const btn=document.getElementById(p+'AbilFilterBtn');const grid=document.getElementById(p+'AbilGrid');const search=document.getElementById(p+'AbilFilterSearch');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){await ensureBrowseFiltersMeta();if(grid&&!grid.dataset.populated){grid.innerHTML=buildAbilBrowseHtml(p);grid.dataset.populated='1'}if(search)search.placeholder=t('list_filter_search_placeholder');syncAbilBrowseCheckboxes(p);panel.hidden=false;btn.setAttribute('aria-expanded','true');ensureFilterPanelFooter(p+'AbilFilterPanel',{onClear:clearAbilOnly,combineKey:'browseCombCharTrait',combineEntity:'char'})}}
...
function getLineageOpSuffix(which){const p=which==='char'?'char':(which==='supp'?'supp':'unit');const key=p==='char'?'listCharLineage':(p==='supp'?'listSuppLineage':'listUnitLineage');let sel=S[key];if(!Array.isArray(sel)||sel.length<2)return '';const ck=p==='char'?'browseCombCharLineage':(p==='supp'?'browseCombSuppLineage':'browseCombUnitLineage');return '&lineage_op='+encodeURIComponent(S[ck]==='or'?'or':'and')}
function getSeriesOpSuffix(which){const p=which==='char'?'char':'unit';let sel=S[p==='char'?'listCharSeries':'listUnitSeries'];if(!Array.isArray(sel)||sel.length<2)return '';const ck=p==='char'?'browseCombCharSeries':'browseCombUnitSeries';return '&series_op='+encodeURIComponent(S[ck]==='and'?'and':'or')}
