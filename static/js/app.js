...
function getCharacterChanceSupportState(d){const blocks=_charAbilityTextBlocks(d);const chanceRe=/chance\s*step\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/ig;const suppDefRe=/support\s*defen[cs]e\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/ig;const suppAtkRe=/support\s*attack\/counter\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/ig;let chanceUn=0,chanceCond=0,defUn=0,defCond=0,atkUn=0,atkCond=0;const mc=(re,tx)=>{const m=String(tx||'').match(re);return m?m.length:0};blocks.forEach(b=>{const tx=String(b&&b.text||'');const c=mc(chanceRe,tx),df=mc(suppDefRe,tx),ak=mc(suppAtkRe,tx);if(b.conditional){chanceCond+=c;defCond+=df;atkCond+=ak}else{chanceUn+=c;defUn+=df;atkUn+=ak}});const condOn=!!S.conditionalPassiveActive;const chanceBonus=chanceUn+(condOn?chanceCond:0);const defBonus=defUn+(condOn?defCond:0);const atkBonus=atkUn+(condOn?atkCond:0);const hasBaseDef=charRoleIsDefense(d&&d.role);const hasBaseAtk=charRoleIsSupport(d&&d.role);const chanceCount=Math.max(1,Math.min(2,1+(chanceBonus>0?1:0)));const supportDefCount=Math.max(0,Math.min(2,Math.max(hasBaseDef?1:0,defBonus)));const supportAtkCount=Math.max(0,Math.min(2,Math.max(hasBaseAtk?1:0,atkBonus)));return{chanceCount,supportDefCount,supportAtkCount}}
function characterHasConditionalChanceOrSupportAbility(d){const blocks=_charAbilityTextBlocks(d);const condRe=/(chance\s*step\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?)|(support\s*defen[cs]e\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?)|(support\s*attack\/counter\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?)/i;return blocks.some(b=>b&&b.conditional&&condRe.test(String(b.text||'')))}
