...
function initToolbarFilterIcons(){ensureUnitUltRarityRow('unit');ensureUnitUltRarityRow('rankUnit');ensureUnitMapWeaponRangeFilterDom();ensureRankUnitMapWeaponRangeFilterDom();ensureUnitWeaponRangeNonMapFilterDom();ensureRankUnitWeaponRangeNonMapFilterDom();['char','unit','supp','rankChar','rankUnit'].forEach(p=>fillRarityPanelIcons(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillSourcePanel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar();['char','rankChar'].forEach(p=>updateAbilBrowseFilterLabel(p));updateUnitWeaponRangeNonMapFilterLabel('unit');updateUnitWeaponRangeNonMapFilterLabel('rankUnit')}
...
function triggerMapFilterAppearAnimation(el){if(!el)return;el.setAttribute('data-appear-state','out');requestAnimationFrame(()=>{requestAnimationFrame(()=>el.setAttribute('data-appear-state','in'))})}
function buildUnitMapWeaponRangeFilterHtml(skOpt){const sk=skOpt||unitMapWeaponRangeStateKey();const selArr=Array.isArray(S[sk])?S[sk]:[];const selSet=new Set(selArr.map(v=>String(v)));let h='<div class="unit-debuff-grid">';for(let i=0;i<MAP_WEAPON_RANGE_TYPE_DEFS.length;i+=2){const a=MAP_WEAPON_RANGE_TYPE_DEFS[i],b=MAP_WEAPON_RANGE_TYPE_DEFS[i+1];const mk=(d)=>{if(!d)return'<span class="unit-debuff-fill" aria-hidden="true"></span>';const active=selSet.has(String(d.value))?' is-active':'';return`<button type="button" class="unit-debuff-item${active}" onclick="onUnitMapWeaponRangeRowClick('${d.value}',event)" onkeydown="onUnitMapWeaponRangeRowKey(event,'${d.value}')"><span>${esc(t(d.labelKey))}</span></button>`};h+=`<div class="unit-debuff-grid-row">${mk(a)}${mk(b)}</div>`}h+='</div>';return h}
function syncUnitMapWeaponRangeCheckboxes(){const pfx=unitWeaponRangeDomPrefix();const body=document.getElementById(pfx+'MapWeaponRangeFilterBody');if(body)body.innerHTML=buildUnitMapWeaponRangeFilterHtml()}
function toggleUnitMapWeaponRangePanel(ev){if(ev)ev.stopPropagation();const pfx=unitWeaponRangeDomPrefix();if(!mapWeaponDebuffActiveForPfx(pfx))return;const panel=document.getElementById(pfx+'MapWeaponRangeFilterPanel');const btn=document.getElementById(pfx+'MapWeaponRangeFilterBtn');const body=document.getElementById(pfx+'MapWeaponRangeFilterBody');if(!panel||!btn)return;const willOpen=panel.hidden;closeAllFilterPanels();if(willOpen){if(body)body.innerHTML=buildUnitMapWeaponRangeFilterHtml();panel.hidden=false;panel.classList.add('map-weapon-range-filter-panel');btn.setAttribute('aria-expanded','true');triggerMapFilterAppearAnimation(panel);const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';ensureFilterPanelFooter(pfx+'MapWeaponRangeFilterPanel',{onClear:clearUnitMapWeaponRangeOnly,combineKey:rk?'browseCombRankMapWr':'browseCombMapWr',combineEntity:rk?'rankUnit':'unit'})}}
...
function updateUnitMapWeaponRangeFilterLabel(forcePfx){const pfx=forcePfx||unitWeaponRangeDomPrefix();const sk=pfx==='rankUnit'?'listRankUnitMapWeaponRange':'listUnitMapWeaponRange';const label=document.getElementById(pfx+'MapWeaponRangeFilterLabel');const btn=document.getElementById(pfx+'MapWeaponRangeFilterBtn');if(!label||!btn)return;btn.title=t('unit_filter_map_weapon_range');let sel=S[sk];if(!Array.isArray(sel))sel=[];if(!sel.length){label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_map_weapon_range_all'))}</span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){const def=MAP_WEAPON_RANGE_TYPE_DEFS.find(d=>String(d.value)===String(sel[0]));const txt=def?t(def.labelKey):String(sel[0]);label.innerHTML=`<span class="source-filter-btn-plain" style="font-size:13px;font-weight:600">${esc(txt)}</span>`;return}label.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_map_weapon_range_multi').replace('{n}',String(sel.length)))}</span>`}
function clearUnitMapWeaponRangeOnly(){const sk=unitMapWeaponRangeStateKey();const rk=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units';if(!rk)cancelBrowseListReloadTimer('units');S[sk]=[];const pfx=unitWeaponRangeDomPrefix();const body=document.getElementById(pfx+'MapWeaponRangeFilterBody');if(body)body.innerHTML=buildUnitMapWeaponRangeFilterHtml();updateUnitMapWeaponRangeFilterLabel();if(rk)void loadRankingList(1);else loadUnits(1)}
function syncMapWeaponRangeFilterVisibility(forcePfx){['unit','rankUnit'].forEach(pfx=>{if(forcePfx&&forcePfx!==pfx)return;const wrap=document.getElementById(pfx+'MapWeaponRangeWrap');if(!wrap)return;const active=mapWeaponDebuffActiveForPfx(pfx);if(!active){wrap.hidden=true;wrap.setAttribute('data-state','out');const sk=pfx==='rankUnit'?'listRankUnitMapWeaponRange':'listUnitMapWeaponRange';if(Array.isArray(S[sk])&&S[sk].length){S[sk]=[];updateUnitMapWeaponRangeFilterLabel(pfx)}return}const wasHidden=wrap.hidden;wrap.hidden=false;if(wasHidden){wrap.setAttribute('data-state','out');requestAnimationFrame(()=>{requestAnimationFrame(()=>wrap.setAttribute('data-state','in'))})}else wrap.setAttribute('data-state','in')})}
function setupMapWeaponRangeFilterDom(pfx,debWrap){if(!debWrap||document.getElementById(pfx+'MapWeaponRangeWrap'))return;const clone=debWrap.cloneNode(true);clone.id=pfx+'MapWeaponRangeWrap';clone.classList.add('map-weapon-range-filter-wrap');clone.hidden=true;clone.setAttribute('data-state','out');const btn=clone.querySelector('#'+pfx+'WeaponDebuffFilterBtn');if(btn){btn.id=pfx+'MapWeaponRangeFilterBtn';btn.setAttribute('onclick','toggleUnitMapWeaponRangePanel(event)');btn.setAttribute('title',t('unit_filter_map_weapon_range'));btn.setAttribute('aria-expanded','false')}const lbl=clone.querySelector('#'+pfx+'WeaponDebuffFilterLabel');if(lbl){lbl.id=pfx+'MapWeaponRangeFilterLabel';lbl.innerHTML=`<span class="source-filter-btn-plain">${esc(t('unit_filter_map_weapon_range_all'))}</span>`}const panel=clone.querySelector('#'+pfx+'WeaponDebuffFilterPanel');if(panel){panel.id=pfx+'MapWeaponRangeFilterPanel';panel.classList.add('map-weapon-range-filter-panel');panel.hidden=true;panel.removeAttribute('data-appear-state')}const body=clone.querySelector('#'+pfx+'WeaponDebuffFilterBody');if(body){body.id=pfx+'MapWeaponRangeFilterBody';body.innerHTML=''}debWrap.insertAdjacentElement('afterend',clone);updateUnitMapWeaponRangeFilterLabel(pfx)}
function ensureRankUnitMapWeaponRangeFilterDom(){const host=document.getElementById('rankingUnitToolbarHost');if(!host)return;const debWrap=document.getElementById('rankUnitWeaponDebuffWrap');if(!debWrap)return;setupMapWeaponRangeFilterDom('rankUnit',debWrap);syncMapWeaponRangeFilterVisibility('rankUnit')}
function ensureUnitMapWeaponRangeFilterDom(){const host=document.getElementById('panel-units');if(!host)return;const debWrap=document.getElementById('unitWeaponDebuffWrap');if(!debWrap)return;setupMapWeaponRangeFilterDom('unit',debWrap);syncMapWeaponRangeFilterVisibility('unit')}
