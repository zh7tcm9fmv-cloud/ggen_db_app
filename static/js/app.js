...
const BT_VOTE_UI={mission:'/static/images/UI/UI_Home_Menu_Icon_Mission.webp',selectFrame:'/static/images/UI/UI_Common_SelectFrame_Tmb_S.webp',check:'/static/images/UI/UI_Common_BtnIcon_Check_Posi_S.webp',voteBtn:'/static/images/UI/UI_StageTop_Button_Lsize_DesignLine_2.webp',poolAll:'/static/images/UI/IMG_StageTop_MaterialStage.webp'};const BT_VOTE_MAX=2;
...
function btVoteSelectOverlay(sel){return'<div class="bt-vote-select" aria-hidden="true"><img class="bt-vote-frame" src="'+escAttr(btVoteUiImg(BT_VOTE_UI.selectFrame))+'" alt="" decoding="async"><img class="bt-vote-check" src="'+escAttr(btVoteUiImg(BT_VOTE_UI.check))+'" alt="" decoding="async"></div>'}
function btVoteExtraCard(gid,key,label,skip){const sel=btVotePickList(gid).includes(key);const cls='bt-vote-extra-thumb bt-vote-tile'+(skip?' bt-vote-extra--skip':'')+(sel?' is-vote-selected':'');return'<div class="'+cls+'" role="button" tabindex="0" data-gasha-id="'+escAttr(gid)+'" data-vote-key="'+escAttr(key)+'" aria-pressed="'+(sel?'true':'false')+'">'+btVoteSelectOverlay(sel)+'<img class="bt-vote-extra-ic" src="'+escAttr(btVoteUiImg(BT_VOTE_UI.poolAll))+'" alt="" loading="lazy" decoding="async"><div class="lr-card-name">'+esc(label)+'</div></div>'}
