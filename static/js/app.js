...
function renderStageRestrictions(d){if(!d.sortie_groups||!d.sortie_groups.length)return`<div class="detail-section"><div class="section-title">${t('sec_sortie_restrictions')}</div><div class="ability-item"><div class="ability-info"><div class="ability-detail">${t('none')}</div></div></div></div>`;return`<div class="detail-section"><div class="section-title">${t('sec_sortie_restrictions')}</div>${d.sortie_groups.map(g=>{const rr=g.restrictions||[];const body=rr.map(r=>{const items=Array.isArray(r.restriction_items)?r.restriction_items.filter(x=>x&&typeof x==='object'):[];const chips=items.length?items.map(renderStageRestrictionItem).join(''):((r.restriction_names||[]).length?(r.restriction_names||[]).map(n=>createTagHtml({id:'',name:n,type:'group'})).join(''):'');return`<div class="stage-restriction-row"><div class="stage-restriction-applies">${esc(r.applies_to||'-')}</div><div class="stage-tags-wrap">${chips||`<span style="color:var(--text-muted)">${t('none')}</span>`}</div></div>`}).join('');return`<div class="stage-restriction-block"><div class="stage-restriction-group-title">${t('sortie_group')} ${g.group_no}</div><div class="stage-restriction-rows${rr.length>1?' stage-restriction-rows--split':''}">${body}</div></div>`}).join('')}</div>`}
