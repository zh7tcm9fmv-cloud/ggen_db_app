...
async function fetchJsonWithWarmupRetry(url,{maxRetries=30}={}){for(let i=0;i<maxRetries;i++){const r=await fetch(url);let d;try{d=await r.json()}catch(_){d={}}if(r.status===503&&d&&d.error==='warming_up'){await new Promise(res=>setTimeout(res,(Number(d.retry_after)||2)*1000));continue}return{ok:r.ok,status:r.status,data:d}}return{ok:false,status:503,data:{error:'warming_up'}}}
