...
function renderStageBrowseThumb(r,opts){opts=opts||{};const variant=opts.variant||'table';const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const showDiff=!locked&&!(isSa||isSs||isTs);const diffCode=esc(r.difficulty_code||'unknown');const diffHtml=showDiff?`<span class="stage-diff-badge stage-diff-${diffCode} stage-browse-thumb__diff">${esc(r.difficulty_name||'-')}</span>`:'';const portrait=locked?'':(r.portrait?imgTag(r.portrait,{cls:'stage-browse-thumb__portrait',webp:true,alt:'',onerror:"this.style.display='none'"}) :'');const phFallback=!locked&&!r.portrait?'<span class="stage-browse-thumb__ph" aria-hidden="true">🗺️</span>':'';const lockOv=locked?'<span class="stage-browse-thumb__lock" aria-hidden="true">🔒</span>':'';const vCls=variant==='grid'?' stage-browse-thumb--grid':variant==='compact'?' stage-browse-thumb--compact':'';const lockCls=locked?' stage-browse-thumb--locked':'';return`<div class="stage-browse-thumb${vCls}${lockCls}" role="presentation"><div class="stage-browse-thumb__bg" aria-hidden="true"></div><div class="stage-browse-thumb__stripes" aria-hidden="true"></div>${portrait}${phFallback}<div class="stage-browse-thumb__shade" aria-hidden="true"></div>${lockOv}<div class="stage-browse-thumb__ui">${diffHtml?`<div class="stage-browse-thumb__head">${diffHtml}${stageBrowseThumbTitleHtml(r,locked)}</div>`:`<div class="stage-browse-thumb__head stage-browse-thumb__head--no-diff">${stageBrowseThumbTitleHtml(r,locked)}</div>`}</div></div>`}
...
function renderStageGrid(data){const grid=document.getElementById('stageGrid');grid.innerHTML=data.rows.map(r=>{const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const metaLine=locked?`<span class="stat-value er-stage-redacted-cell">—</span>`:(isSs||isTs)?'':isSa?`<span class="stat-value">#${fmtN(r.stage_number)}</span>`:`<span class="stage-diff-badge stage-diff-${esc(r.difficulty_code||'unknown')}">${esc(r.difficulty_name||'-')}</span> <span class="stat-value">#${fmtN(r.stage_number)}</span>`;const metaRow=!locked&&(isSs||isTs)?'':`<div class="list-grid-card-meta">${metaLine}</div>`;const cpPart=locked?'—':fmtN(r.recommended_cp);const terPart=locked?'—':esc(r.terrain);return`<div class="list-grid-card list-grid-card--stage${locked?' list-grid-card--locked':''}" data-detail-type="stage" data-detail-id="${escAttr(String(r.id))}" onclick="openDetail('stage','${r.id}')"><div class="list-grid-card-thumb">${renderStageBrowseThumb(r,{variant:'grid'})}</div><div class="list-grid-card-body">${metaRow}<div class="list-grid-card-stats">${t('col_stage_cp')} ${cpPart} · ${t('terrain')} ${terPart}</div></div></div>`}).join('')}
