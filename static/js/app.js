...
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const showBtn=show&&!S.btVoteMode;const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);['btVoteModeBtn','btVoteModeBtnMobile'].forEach(id=>{const btn=document.getElementById(id);if(!btn)return;const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',showBtn)})}
...
function updateGameNewsNavNotice(show){const tab=document.getElementById('navGameNewsTab');if(!tab)return;tab.classList.toggle('has-ui-notice',!!show)}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};ensureNavBannerTimelineNoticeFlare();void bootstrapBtVoteNotices();ensureNavGameNewsNoticeFlare();void bootstrapGameNewsNotices()}
