...
function rankingListUrlFor(mode,sortKey,page,perPage,dir){const p=Math.max(1,Number(page)||1),pp=Math.max(1,Number(perPage)||5),dv=(dir==='asc'?'asc':'desc');const base=mode==='characters'?buildRankingCharactersListUrl(p,pp):buildRankingUnitsListUrl(p,pp);let u=base.replace(/([?&])sort=[^&]*/,'$1sort='+encodeURIComponent(sortKey));u=u.replace(/([?&])dir=[^&]*/,'$1dir='+dv);u=u.replace(/([?&])per_page=[^&]*/,'$1per_page='+pp);u=u.replace(/([?&])page=[^&]*/,'$1page='+p);return u}
...
async function fetchRankingPodiumData(mode){const key=rankingPodiumCacheKey();if(_rankingPodiumCache.has(key))return _rankingPodiumCache.get(key);const stats=(mode==='characters'?RANK_SORT_KEYS_CHAR:RANK_SORT_KEYS_UNIT).slice();const out={};await Promise.all(stats.map(async sk=>{try{const r=await fetch(rankingListUrlFor(mode,sk,1,5,'desc'));if(!r.ok)return;const d=await r.json();out[sk]={rows:Array.isArray(d&&d.rows)?d.rows.slice(0,5):[],total:Number(d&&d.total||0),sort:sk,page:1,per_page:5,stat_bounds:d&&d.stat_bounds||null}}catch(_){}}));_rankingPodiumCache.set(key,out);while(_rankingPodiumCache.size>24){const k=_rankingPodiumCache.keys().next().value;_rankingPodiumCache.delete(k)}return out}
...
function detailRankingSortKeyForStat(type,statName){if(type==='character'){const m={Ranged:'Ranged',Melee:'Melee',Awaken:'Awaken',Defense:'Defense',Reaction:'Reaction'};return m[statName]||''}const m={HP:'HP',EN:'EN',Attack:'ATK',Defense:'DEF',Mobility:'MOB'};return m[statName]||''}
function detailRankingCacheKey(type,id){return`${type}:${String(id)}:${S.lang}:${detailRankingPerspectiveKey(type)}`}
function detailRankingFetchUrlFor(type,sortKey){if(type==='character'){const stQ=listStatQChar(!!S.spActive,!!S.conditionalPassiveActive);return`/api/characters?lang=${S.lang}&page=1&per_page=5000&sort=${encodeURIComponent(sortKey)}&dir=desc&stat_bounds=1&q=${encodeURIComponent('')}${stQ}`}const stQ=listStatQUnit(!!S.spActive,!!S.sspActive,!!S.conditionalPassiveActive);return`/api/units?lang=${S.lang}&page=1&per_page=5000&sort=${encodeURIComponent(sortKey)}&dir=desc&stat_bounds=1&q=${encodeURIComponent('')}${stQ}`}
...
function renderRankingList(d){const rows=d.rows||[];const bounds=d.stat_bounds;const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';if(vm==='podium'){const grouped=(d&&d.podiumByStat)||{};const keys=rankingSortKeys();const sections=keys.map(k=>{const block=grouped[k]||{rows:[]};const topRows=(Array.isArray(block.rows)?block.rows:[]).slice(0,5);const vals=topRows.map(r=>Number(r&&r[k]||0));const mx=Math.max(1,...vals);return`<section class="ranking-podium-section"><div class="ranking-podium-title">${esc(tStat(k,typ))}</div><div class="ranking-podium-row">${topRows.map((row,idx)=>{const rank=idx+1;const val=Number(row&&row[k]||0);const h=Math.max(4,Math.round((val/mx)*100));const n=esc(row&&row.name||'-');const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,42);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-podium-col" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)" title="${n}"><span class="ranking-podium-rank">#${rank}</span><span class="ranking-podium-rail"><span class="ranking-podium-fill" style="height:${h}%"></span></span><span class="ranking-podium-thumb">${img}</span><span class="ranking-podium-val">${fmtN(val)}</span></button>`}).join('')||`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`}</div></section>`}).join('');host.classList.add('ranking-list-inner--podium');host.innerHTML=`<div class="ranking-podium-board">${sections}</div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';host.innerHTML=rows.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const statVal=row[sortKey];const name=esc(row.name||'-');const id=escAttr(String(row.id||''));const img=renderListThumb(row,thumbKind,52);const bar=renderRankingRowBar(statVal,bounds);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-rank-num">#${rank}</span><span class="ranking-row-thumb">${img}</span><span class="ranking-row-name">${name}</span><span class="ranking-row-stat"><span class="ranking-stat-val">${fmtN(statVal)}</span>${bar}</span></button>`}).join('');syncRankingViewModeUi();renderPagination(d.page,d.per_page,d.total,pp=>loadRankingList(pp),'rankPagination')}
async function loadRankingList(p=1){if(S.currentTab!=='ranking')return;p=Number(p)||1;const mode=S.ranking.mode;const vm=(S.ranking&&S.ranking.viewMode)||'list';if(vm==='podium')p=1;const ppEl=document.getElementById('rankPerPage');const pp=ppEl?parseInt(ppEl.value,10)||50:50;if(mode==='characters')S.ranking.pageChar=p;else S.ranking.pageUnit=p;const url=mode==='characters'?buildRankingCharactersListUrl(p,pp):buildRankingUnitsListUrl(p,pp);const load=document.getElementById('rankLoading');const host=document.getElementById('rankListInner');if(load)load.style.display='flex';if(host)host.innerHTML='';try{const r=await fetch(url);const d=await r.json();if(vm==='podium')d.podiumByStat=await fetchRankingPodiumData(mode);_rankingLastPayloadByMode[mode==='characters'?'characters':'units']=d;if(mode==='units'){applyWeaponDebuffPresentFromApi(d);applyMechanismPresentFromApi(d)}renderRankingList(d)}catch(e){if(load)load.style.display='none';if(host)host.innerHTML=`<div class="empty-state"><div class="empty-state-text">${esc(String(e))}</div></div>`}}
