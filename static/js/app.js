...
Object.assign(T.EN,{stage_sortie_unit_restrictions:'Unit Sortie Restrictions',stage_sortie_char_restrictions:'Character Sortie Restrictions',stage_sortie_no_limit:'No Limit'});
Object.assign(T.TW,{stage_sortie_unit_restrictions:'機體出擊限制',stage_sortie_char_restrictions:'角色出擊限制',stage_sortie_no_limit:'無限制'});
Object.assign(T.HK,{stage_sortie_unit_restrictions:'機體出擊限制',stage_sortie_char_restrictions:'角色出擊限制',stage_sortie_no_limit:'無限制'});
Object.assign(T.JA,{stage_sortie_unit_restrictions:'ユニット出撃制限',stage_sortie_char_restrictions:'キャラクター出撃制限',stage_sortie_no_limit:'制限なし'});
Object.assign(T.JP,{stage_sortie_unit_restrictions:'ユニット出撃制限',stage_sortie_char_restrictions:'キャラクター出撃制限',stage_sortie_no_limit:'制限なし'});
...
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const showBtn=show&&!S.btVoteMode;const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);const btn=document.getElementById('btVoteModeBtn');if(btn){const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',showBtn)}syncNavTabsOverflowHints()}
...
function btVoteThumbBlock(thumbHtml,voteMode){if(!voteMode)return'<div class="lr-card-thumb">'+thumbHtml+'</div>';return'<div class="bt-vote-thumb-wrap"><div class="lr-card-thumb"><div class="bt-vote-thumb-target">'+thumbHtml+btVoteSelectOverlay()+'</div></div></div>'}
...
function btFeaturedCell(items,tpx,gid,voteMode){let px=tpx!=null&&tpx!==undefined?Number(tpx):52;if(Number.isNaN(px)||px<32)px=52;const picks=voteMode&&gid?btVoteMineList(gid):[];let h='';if(!items||!items.length){if(!h)return'<span class="bt-strip-empty">—</span>'}else items.forEach(it=>{const row={thum:it.thum,rarity:it.rarity||'N',role_icon:it.role_icon||'',acquisition_icon:it.acquisition_icon||'',special_icons:Array.isArray(it.special_icons)?it.special_icons:[],is_ultimate:!!it.is_ultimate};const typ=it.type==='character'?'character':it.type==='supporter'?'supporter':'unit';const thumbKind=typ==='character'?'char':typ==='supporter'?'supp':'unit';const lim=!!it.is_limited_time;const limCls=lim?(' bt-lr-tile--limited bt-lr-tile--lt-'+typ):'';const lbl=t('limited_label');const nm=String(it.name||'');const aria=lim?escAttr(nm+' — '+lbl):escAttr(nm);const bar=lim?'<div class="bt-limited-topbar" aria-hidden="true"><span class="bt-limited-topbar-inner">'+esc(lbl)+'</span></div>':'';const fx=lim?'<span class="bt-limited-shimmer" aria-hidden="true"></span>':'';const vkey=btVoteOptionKey(typ,it.id);const vsel=picks.includes(vkey);const voteCls=voteMode?' bt-vote-tile'+(vsel?' is-vote-selected':''):'';const click=voteMode?'':' onclick="openDetail(\''+typ+'\',\''+escJs(it.id)+'\')"';const voteAttr=voteMode&&gid?' data-gasha-id="'+escAttr(gid)+'" data-vote-key="'+escAttr(vkey)+'" aria-pressed="'+(vsel?'true':'false')+'"':'';const thumb=btVoteThumbBlock(renderListThumb(row,thumbKind,Math.round(px)),voteMode);h+='<div class="lr-card bt-lr-tile'+limCls+voteCls+'" role="button" tabindex="0" data-detail-type="'+escAttr(typ)+'" data-detail-id="'+escAttr(it.id)+'"'+voteAttr+' aria-label="'+aria+'"'+click+'>'+fx+bar+thumb+'<div class="lr-card-name">'+esc(nm)+'</div></div>'});return'<div class="bt-strip">'+h+'</div>'}
...
function btFeaturedMergedCell(units,chars,supporters,gid,b,voteModeOverride){const voteOk=btVoteEnabledForBanner(b);const voteMode=(voteModeOverride!=null?!!voteModeOverride:!!S.btVoteMode)&&voteOk;const sups=supporters&&supporters.length?supporters:null;let inner='';if(voteMode&&gid)inner+='<div class="bt-feat-block bt-feat-block--vote-extras">'+btVoteExtrasStrip(gid)+'</div>';inner+='<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(units,74,gid,voteMode)+'</div>'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(chars,74,gid,voteMode)+'</div>';if(sups)inner+='<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_supporters'))+'</div>'+btFeaturedCell(sups,74,gid,voteMode)+'</div>';const rowCls=voteOk?'':' bt-vote-row--disabled';return'<td class="bt-feat-stack-cell'+rowCls+'" data-gasha-id="'+escAttr(gid||'')+'">'+'<div class="bt-feat-stack">'+inner+'</div></td>'}
function btBuildBannerTbodyHtml(rows,voteModeOverride){const sorted=btSortBannerRows(rows||[]);let body='';sorted.forEach(b=>{const gid=String(b.gasha_id||'');const img=b.banner_url?'<div class="bt-banner-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const voteRowCls=btVoteEnabledForBanner(b)?'':' bt-vote-row--disabled';body+='<tr class="'+voteRowCls.trim()+'" data-gasha-id="'+escAttr(gid)+'"><td class="bt-banner-cell">'+img+'<div class="bt-banner-title">'+esc(b.name||'')+'</div>'+btBannerScheduleBlock(b)+'</td>'+btFeaturedMergedCell(b.featured_units,b.featured_chars,b.featured_supporters,gid,b,voteModeOverride)+'</tr>'});return body}
function btInvalidateVoteTbodyCache(){S._btTbodyOff=null;S._btTbodyOn=null}
function btRebuildVoteTbodyCache(rows){const rs=rows||[];S._btTbodyOff=btBuildBannerTbodyHtml(rs,false);S._btTbodyOn=btBuildBannerTbodyHtml(rs,true)}
...
function btSwapVoteTbody(){const root=document.getElementById('bannerTimelineRoot');const tbody=root&&root.querySelector('.bt-table tbody');if(!tbody||!S.btCacheData)return false;if(S._btTbodyOff==null||S._btTbodyOn==null)btRebuildVoteTbodyCache(S.btCacheData.banners||[]);const tpl=document.createElement('template');tpl.innerHTML='<tbody>'+(S.btVoteMode?S._btTbodyOn:S._btTbodyOff)+'</tbody>';const neu=tpl.content.querySelector('tbody');if(!neu)return false;tbody.replaceWith(neu);btSyncVoteModeChrome();return true}
function btScheduleVoteTbodyCache(rows){btInvalidateVoteTbodyCache();const run=()=>{if(S.btCacheData)btRebuildVoteTbodyCache(rows)};if(typeof requestIdleCallback==='function')requestIdleCallback(run,{timeout:2500});else setTimeout(run,120)}
function renderBannerTimelineTable(rows){const sorted=btSortBannerRows(rows||[]);const dir=S.btBannerSortDir||'desc';const arrow=dir==='desc'?'▼':'▲';const voteOn=!!S.btVoteMode;const voteBtnHead='<div class="bt-th-combo-vote-tools">'+btVoteModeCaptionMarkup('btVoteModeCaption')+btVoteModeBtnMarkup('btVoteModeBtn')+'</div>';const thead='<thead><tr><th class="bt-th-banner bt-th-sortable" scope="col" role="button" tabindex="0" onclick="toggleBtBannerTimelineSort()" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();toggleBtBannerTimelineSort()}" title="'+escAttr(t('bt_sort_start_hint'))+'" aria-label="'+escAttr(t('bt_sort_start_hint'))+'">'+esc(t('bt_col_banner'))+' <span class="bt-sort-ind" aria-hidden="true">'+arrow+'</span></th><th class="bt-th-combo" scope="col"><div class="bt-th-combo-inner"><span class="bt-th-combo-label">'+esc(t('bt_col_featured'))+'</span>'+voteBtnHead+'</div></th></tr></thead>';const body='<tbody>'+btBuildBannerTbodyHtml(sorted,voteOn)+'</tbody>';return'<div class="bt-table-wrap"><table class="data-table bt-table'+(voteOn?' bt-vote-mode-on':'')+'">'+thead+body+'</table></div>'}
...
function toggleBtVoteMode(){if(!S.btCacheData)return;const root=document.getElementById('bannerTimelineRoot');if(!root||!root.querySelector('.bt-table'))return;document.querySelectorAll('.bt-vote-mode-btn').forEach(b=>{b.disabled=true});const turningOn=!S.btVoteMode;S.btVoteMode=!S.btVoteMode;if(turningOn)btMarkVoteNoticeDismissed();const finish=()=>{document.querySelectorAll('.bt-vote-mode-btn').forEach(b=>{b.disabled=false});updateBtVoteNotices();requestAnimationFrame(()=>{armBannerTimelineScrollFabBaseline();updateBannerTimelineScrollFabVisibility()})};const apply=()=>{if(!btSwapVoteTbody())renderBannerTimeline(S.btCacheData);finish()};requestAnimationFrame(apply)}
...
function btBindVoteDelegation(){const root=document.getElementById('bannerTimelineRoot');if(!root||root._btVoteBound)return;root._btVoteBound=1;root.addEventListener('click',ev=>{if(!S.btVoteMode)return;const tile=ev.target.closest('.bt-vote-tile[data-vote-key]');if(!tile)return;ev.preventDefault();ev.stopPropagation();const gid=tile.getAttribute('data-gasha-id');const key=tile.getAttribute('data-vote-key');if(!gid||!key||!btVoteEnabledForGid(gid))return;btToggleVote(gid,key)})}
...
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});if(typeof S.btVoteMode!=='boolean')S.btVoteMode=false;if(!S.btVoteMine)S.btVoteMine={};ensureNavBannerTimelineNoticeFlare();void bootstrapBtVoteNotices();ensureNavGameNewsNoticeFlare();void bootstrapGameNewsNotices();initGameNewsNoticeRefresh();initNavTabsOverflowHints()}
...
function renderStageRestrictions(d){if(!d.sortie_groups||!d.sortie_groups.length)return`<div class="detail-section"><div class="section-title">${t('sec_sortie_restrictions')}</div><div class="ability-item"><div class="ability-info"><div class="ability-detail">${t('none')}</div></div></div></div>`;return`<div class="detail-section"><div class="section-title">${t('sec_sortie_restrictions')}</div>${d.sortie_groups.map(g=>`<div class="stage-restriction-block"><div class="stage-restriction-group-title"><span class="stage-restriction-group-title-text">${t('sortie_group')} ${g.group_no}</span></div>${renderSortieGroupRestrictions(g)}</div>`).join('')}</div>`}
