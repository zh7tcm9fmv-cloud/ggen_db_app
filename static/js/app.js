...
function btUserHasAnyVote(){return btVoteEnabledBanners().some(b=>btVoteMineList(String(b.gasha_id||'')).length>0)}
function btUserNeedsVoteNotice(){if(btVoteNoticeDismissed()||btUserHasAnyVote())return false;if(!S._btVoteStatusLoaded)return false;return btVoteEnabledBanners().length>0}
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);['btVoteModeBtn','btVoteModeBtnMobile'].forEach(id=>{const btn=document.getElementById(id);if(!btn)return;const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',show)})}
async function bootstrapBtVoteNotices(){const cid=btVoteClientId();try{const reqs=[fetch('/api/banner_timeline/votes?client_id='+encodeURIComponent(cid),{credentials:'same-origin',cache:'no-store'})];if(!S.btCacheData)reqs.push(fetch('/api/banner_timeline?lang='+encodeURIComponent(S.lang||'EN'),{credentials:'same-origin'}));const rs=await Promise.all(reqs);if(rs[0]&&rs[0].ok){const d=await rs[0].json();S.btVoteTotals=d.totals||{};S.btVoteMine=d.mine||{}}if(!S.btCacheData&&rs[1]&&rs[1].ok)S.btCacheData=await rs[1].json();S._btVoteStatusLoaded=true}catch(_){}updateBtVoteNotices()}
...
function btVoteClientId(){if(S.btVoteClientId)return S.btVoteClientId;let id='';try{id=localStorage.getItem('ggen_bt_vote_client')||''}catch(_){}if(!id){id='v_'+Math.random().toString(36).slice(2)+Date.now().toString(36);try{localStorage.setItem('ggen_bt_vote_client',id)}catch(_){}}S.btVoteClientId=id;return id}
...
function btFeaturedMergedCell(units,chars,supporters,gid,b,voteModeOverride){const voteOk=btVoteEnabledForBanner(b);const voteMode=(voteModeOverride!=null?!!voteModeOverride:!!S.btVoteMode)&&voteOk;const sups=supporters&&supporters.length?supporters:null;let inner='';if(voteMode&&gid)inner+='<div class="bt-feat-block bt-feat-block--vote-extras">'+btVoteExtrasStrip(gid)+'</div>';inner+='<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_units'))+'</div>'+btFeaturedCell(units,62,gid,voteMode)+'</div>'+'<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_chars'))+'</div>'+btFeaturedCell(chars,62,gid,voteMode)+'</div>';if(sups)inner+='<div class="bt-feat-block"><div class="bt-feat-sub">'+esc(t('bt_col_supporters'))+'</div>'+btFeaturedCell(sups,62,gid,voteMode)+'</div>';const rowCls=voteOk?'':' bt-vote-row--disabled';return'<td class="bt-feat-stack-cell'+rowCls+'" data-gasha-id="'+escAttr(gid||'')+'">'+'<div class="bt-feat-stack">'+inner+'</div></td>'}
...
async function fetchBtVoteTotals(){try{const cid=btVoteClientId();const r=await fetch('/api/banner_timeline/votes?client_id='+encodeURIComponent(cid),{credentials:'same-origin',cache:'no-store'});if(!r.ok)return;const d=await r.json();S.btVoteTotals=d.totals||{};S.btVoteMine=d.mine||{};S._btVoteStatusLoaded=true;updateBtVoteNotices()}catch(_){}}
