...
function getCharacterChanceSupportState(d){const blocks=_charAbilityTextBlocks(d);const chanceRe=/chance\s*step\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/i;const suppDefRe=/support\s*defen[cs]e\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/i;const suppAtkRe=/support\s*attack\/counter\s*\+\s*1(?!\d)(?:\s*time(?:\(s\))?)?/i;let chanceUn=false,chanceCond=false,defUn=false,defCond=false,atkUn=false,atkCond=false;blocks.forEach(b=>{const tx=String(b&&b.text||'');if(chanceRe.test(tx)){if(b.conditional)chanceCond=true;else chanceUn=true}if(suppDefRe.test(tx)){if(b.conditional)defCond=true;else defUn=true}if(suppAtkRe.test(tx)){if(b.conditional)atkCond=true;else atkUn=true}});const condOn=!!S.conditionalPassiveActive;const chanceExtra=chanceUn||(condOn&&chanceCond);const hasBaseDef=charRoleIsDefense(d&&d.role);const hasBaseAtk=charRoleIsSupport(d&&d.role);const defExtra=defUn||(condOn&&defCond);const atkExtra=atkUn||(condOn&&atkCond);const chanceCount=Math.max(1,Math.min(2,1+(chanceExtra?1:0)));const supportDefCount=Math.max(0,Math.min(2,(hasBaseDef?1:0)+(defExtra?1:0)));const supportAtkCount=Math.max(0,Math.min(2,(hasBaseAtk?1:0)+(atkExtra?1:0)));return{chanceCount,supportDefCount,supportAtkCount}}
function renderDetailIconCountRow(label,iconPath,count){const c=Math.max(0,Number(count)||0);const icons=Array.from({length:c}).map(()=>`<img class="char-extra-info-icon" src="${imgUrl('/static/images/UI/'+iconPath+'.webp')}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'">`).join('');return`<div class="char-extra-info-row"><div class="char-extra-info-label">${esc(label)}</div><div class="char-extra-info-icons">${icons||'<span class="char-extra-info-none">-</span>'}</div></div>`}
function renderCharacterExtraInfo(d){if(!d)return'';const s=getCharacterChanceSupportState(d);return`<div class="char-extra-info-wrap">${renderDetailIconCountRow('Chance Step','UI_Common_Icon_ChanceStep',s.chanceCount)}${renderDetailIconCountRow('Support Defense','UI_Common_BattleIcon_AssistDeffence_S',s.supportDefCount)}${renderDetailIconCountRow('Support Attack','UI_Common_BattleIcon_AssistAtack_S',s.supportAtkCount)}</div>`}
