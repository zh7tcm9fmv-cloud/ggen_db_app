...
function renderStageBrowseThumb(r,opts){opts=opts||{};const variant=opts.variant||'table';const locked=!!r.content_locked;const isSa=r.stage_category==='score_attack';const isSs=r.stage_category==='special_stage';const isTs=r.stage_category==='tower_stage';const posterPortrait=isSs||isTs||isSa;const noShade=posterPortrait;const showDiff=!locked&&!(isSa||isSs||isTs);const diffCode=esc(r.difficulty_code||'unknown');const diffHtml=showDiff?`<span class="stage-diff-badge stage-diff-${diffCode} stage-browse-thumb__diff">${esc(r.difficulty_name||'-')}</span>`:'';const portCls=posterPortrait?' stage-browse-thumb__portrait--poster':' stage-browse-thumb__portrait--unit';const portrait=locked?'':(r.portrait?imgTag(r.portrait,{cls:'stage-browse-thumb__portrait'+portCls,webp:true,alt:'',onerror:"this.style.display='none'"}) :'');const phFallback=!locked&&!r.portrait?'<span class="stage-browse-thumb__ph'+(posterPortrait?' stage-browse-thumb__ph--poster':' stage-browse-thumb__ph--unit')+'" aria-hidden="true">🗺️</span>':'';const lockOv=locked?'<span class="stage-browse-thumb__lock" aria-hidden="true">🔒</span>':'';const vCls=variant==='grid'?' stage-browse-thumb--grid':variant==='compact'?' stage-browse-thumb--compact':'';const themeCls=isSs?' stage-browse-thumb--special':(isTs||isSa)?' stage-browse-thumb--tower':'';const lockCls=locked?' stage-browse-thumb--locked':'';const shadeHtml=noShade?'':'<div class="stage-browse-thumb__shade" aria-hidden="true"></div>';return`<div class="stage-browse-thumb${vCls}${themeCls}${lockCls}" role="presentation"><div class="stage-browse-thumb__bg" aria-hidden="true"></div><div class="stage-browse-thumb__stripes" aria-hidden="true"></div>${portrait}${phFallback}${shadeHtml}${lockOv}<div class="stage-browse-thumb__ui">${diffHtml?`<div class="stage-browse-thumb__head">${diffHtml}${stageBrowseThumbTitleHtml(r,locked)}</div>`:`<div class="stage-browse-thumb__head stage-browse-thumb__head--no-diff">${stageBrowseThumbTitleHtml(r,locked)}</div>`}</div></div>`}
