...
function detailRankingFetchUrlFor(type,sortKey,page=1,per=1000){if(type==='character'){const stQ=listStatQChar(!!S.spActive,!!S.conditionalPassiveActive);const qEl=document.getElementById('rankCharFilter');const q=qEl?String(qEl.value||'').trim():'';return`/api/characters?lang=${S.lang}&page=${page}&per_page=${per}&sort=${encodeURIComponent(sortKey)}&dir=desc&stat_bounds=1&q=${encodeURIComponent(q)}${stQ}`}const stQ=listStatQUnit(!!S.spActive,!!S.sspActive,!!S.conditionalPassiveActive);const qEl=document.getElementById('rankUnitFilter');const qRaw=qEl?String(qEl.value||'').trim():'';const qApi=expandUnitSearchQuery(qRaw);return`/api/units?lang=${S.lang}&page=${page}&per_page=${per}&sort=${encodeURIComponent(sortKey)}&dir=desc&stat_bounds=1&q=${encodeURIComponent(qApi)}${stQ}`}
async function fetchDetailRankingMetaForSort(type,sortKey,id){const iid=String(id);let page=1;const reqPer=5000;let total=0,maxPages=1,effPer=reqPer;while(page<=maxPages){const r=await fetch(detailRankingFetchUrlFor(type,sortKey,page,reqPer));if(!r.ok)return null;const d=await r.json();const rows=Array.isArray(d&&d.rows)?d.rows:[];if(page===1){total=Number(d&&d.total||0);effPer=Number(d&&d.per_page||rows.length||reqPer)||reqPer;maxPages=Math.max(1,Math.ceil((total||0)/effPer)||1)}const idx=rows.findIndex(x=>String(x&&x.id||'')===iid);if(idx>=0)return{rank:(page-1)*effPer+idx+1,total:total||Number(d&&d.total||0),value:Number(rows[idx]&&rows[idx][sortKey]||0),bounds:d&&d.stat_bounds||null};if(!rows.length)break;page++}return null}
...
async function ensureDetailRankingStats(type,id){if((type!=='character'&&type!=='unit')||!id)return null;const ck=detailRankingCacheKey(type,id);const iid=String(id);if(_detailRankingStatsCache.has(ck))return _detailRankingStatsCache.get(ck);seedDetailRankingFromLastPayload(type,iid,ck);if(_detailRankingStatsInflight.has(ck))return await _detailRankingStatsInflight.get(ck);const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB'];const p=(async()=>{const byStat=_detailRankingStatsCache.get(ck)||{};await Promise.all(sortKeys.map(async sk=>{try{const meta=await fetchDetailRankingMetaForSort(type,sk,iid);if(!meta)return;byStat[sk]=meta;_detailRankingStatsCache.set(ck,byStat);refreshDetailRankingIfActive(type,iid)}catch(_){}}));_detailRankingStatsCache.set(ck,byStat);return byStat})().finally(()=>_detailRankingStatsInflight.delete(ck));_detailRankingStatsInflight.set(ck,p);return await p}
...
function openDetailFromRecommend(type,id,viewRanking){if(viewRanking){const o=detailRecommendOptsForType(type);if(type==='character'){S.spActive=!!o.listCharSpPerspective;S.conditionalPassiveActive=!!o.listCharCondPerspective;S.sspActive=false}else if(type==='unit'){S.sspActive=!!o.listUnitSspPerspective;S.spActive=!!o.listUnitSpPerspective&&!S.sspActive;S.conditionalPassiveActive=!!o.listUnitCondPerspective}openDetail(type,String(id),o);return}openDetail(type,String(id))}
function buildRankingCharactersListUrl(p,pp){const sort=S.ranking.sortChar,dir=S.ranking.dirChar;const q=document.getElementById('rankCharFilter').value.trim();const roleQ=getRoleQuerySuffix('rankChar');const rq=getRarityQuerySuffix('rankChar');const stQ=listStatQChar(!!S.listRankCharSp,!!S.listRankCharCond);const srcQ=(cur=>{if(!cur||cur==='ALL')return '';return '&source='+encodeURIComponent(cur)})(S.listRankCharSource||'ALL');const linQ=(()=>{let sel=S.listRankCharLineage;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&lineage_id='+encodeURIComponent(sel.join(','))})();const linOp=(()=>{let sel=S.listRankCharLineage;if(!Array.isArray(sel)||sel.length<2)return '';return '&lineage_op='+encodeURIComponent(S.browseCombRankCharLineage==='or'?'or':'and')})();const serQ=(()=>{let sel=S.listRankCharSeries;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel)){if(String(sel).trim())sel=[String(sel)];else sel=[]}if(!sel.length)return '';return '&series_id='+encodeURIComponent(sel.join(','))})();const serOp=(()=>{let sel=S.listRankCharSeries;if(!Array.isArray(sel)||sel.length<2)return '';return '&series_op='+encodeURIComponent(S.browseCombRankCharSeries==='and'?'and':'or')})();const skillQ=(()=>{let sel=S.listRankCharSkills;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&skill_id='+encodeURIComponent(sel.join(','))})();const skOp=(()=>{let sel=S.listRankCharSkills;if(!Array.isArray(sel)||sel.length<2)return '';return '&skill_op='+encodeURIComponent(S.browseCombRankCharSkill==='or'?'or':'and')})();const abilQ=(()=>{let sel=S.listRankCharAbilities;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&ability_id='+encodeURIComponent(sel.join(','))})();const traitOp=(()=>{let sel=S.listRankCharAbilities;if(!Array.isArray(sel)||sel.length<2)return '';return '&ability_op='+encodeURIComponent(S.browseCombRankCharTrait==='or'?'or':'and')})();return`/api/characters?lang=${S.lang}&page=${p}&per_page=${pp}&sort=${encodeURIComponent(sort)}&dir=${dir}&stat_bounds=1&q=${encodeURIComponent(q)}${roleQ}${rq}${stQ}${srcQ}${linQ}${linOp}${serQ}${serOp}${skillQ}${skOp}${abilQ}${traitOp}`}
function buildRankingUnitsListUrl(p,pp){const sort=S.ranking.sortUnit,dir=S.ranking.dirUnit;const qRaw=document.getElementById('rankUnitFilter').value.trim();const qApi=expandUnitSearchQuery(qRaw);const roleQ=getRoleQuerySuffix('rankUnit');const rq=getRarityQuerySuffix('rankUnit');const stQ=listStatQUnit(!!S.listRankUnitSp,!!S.listRankUnitSsp,!!S.listRankUnitCond);const srcQ=(cur=>{if(!cur||cur==='ALL')return '';return '&source='+encodeURIComponent(cur)})(S.listRankUnitSource||'ALL');const terrQ=(()=>{let sel=S.listRankUnitTerrain;if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&terrain='+encodeURIComponent(sel.join(','))})();const terrOp=(()=>{let sel=S.listRankUnitTerrain;if(!Array.isArray(sel)||sel.length<2)return '';return '&terrain_op='+encodeURIComponent(S.browseCombRankTerrain==='or'?'or':'and')})();const debQ=(()=>{let sel=S.listRankUnitWeaponDebuff;if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&weapon_debuff='+encodeURIComponent(sel.join(','))})();const debOp=(()=>{let sel=S.listRankUnitWeaponDebuff;if(!Array.isArray(sel)||sel.length<2)return '';return '&weapon_debuff_op='+encodeURIComponent(S.browseCombRankWb==='or'?'or':'and')})();const mechQ=(()=>{let sel=S.listRankUnitMechanism;if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&mechanism='+encodeURIComponent(sel.join(','))})();const mechOp=(()=>{let sel=S.listRankUnitMechanism;if(!Array.isArray(sel)||sel.length<2)return '';return '&mechanism_op='+encodeURIComponent(S.browseCombRankMech==='or'?'or':'and')})();const linQ=(()=>{let sel=S.listRankUnitLineage;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&lineage_id='+encodeURIComponent(sel.join(','))})();const linOp=(()=>{let sel=S.listRankUnitLineage;if(!Array.isArray(sel)||sel.length<2)return '';return '&lineage_op='+encodeURIComponent(S.browseCombRankUnitLineage==='or'?'or':'and')})();const serQ=(()=>{let sel=S.listRankUnitSeries;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel)){if(String(sel).trim())sel=[String(sel)];else sel=[]}if(!sel.length)return '';return '&series_id='+encodeURIComponent(sel.join(','))})();const serOp=(()=>{let sel=S.listRankUnitSeries;if(!Array.isArray(sel)||sel.length<2)return '';return '&series_op='+encodeURIComponent(S.browseCombRankUnitSeries==='and'?'and':'or')})();const abQ=(()=>{let sel=S.listRankUnitAbilities;if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];if(!sel.length)return '';return '&ability_id='+encodeURIComponent(sel.join(','))})();const abOp=(()=>{let sel=S.listRankUnitAbilities;if(!Array.isArray(sel)||sel.length<2)return '';return '&ability_op='+encodeURIComponent(S.browseCombRankUnitAbil==='or'?'or':'and')})();return`/api/units?lang=${S.lang}&page=${p}&per_page=${pp}&sort=${encodeURIComponent(sort)}&dir=${dir}&stat_bounds=1&q=${encodeURIComponent(qApi)}${roleQ}${rq}${stQ}${srcQ}${terrQ}${terrOp}${debQ}${debOp}${mechQ}${mechOp}${linQ}${linOp}${serQ}${serOp}${abQ}${abOp}`}
...
function initToolbarFilterIcons(){ensureUnitUltRarityRow('unit');ensureUnitUltRarityRow('rankUnit');['char','unit','supp','rankChar','rankUnit'].forEach(p=>fillRarityPanelIcons(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillRolePanelIcons(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateRarityFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateRoleFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>fillSourcePanel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSourceFilterButtonLabel(p));['char','unit','supp','rankChar','rankUnit'].forEach(p=>updateLineageFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSeriesFilterButtonLabel(p));['char','unit','rankChar','rankUnit'].forEach(p=>updateSkillBrowseFilterLabel(p));fillStageDiffPanel();updateStageDifficultyFilterButtons();syncStageSourceToolbar();['char','rankChar'].forEach(p=>updateAbilBrowseFilterLabel(p))}
...
function updateAbilBrowseFilterLabel(p){const key=p==='rankChar'?'listRankCharAbilities':'listCharAbilities';let sel=S[key];if(sel==='ALL'||!sel)sel=[];if(!Array.isArray(sel))sel=[];const label=document.getElementById(p+'AbilFilterLabel');const btn=document.getElementById(p+'AbilFilterBtn');if(!label||!btn)return;btn.title=t('list_filter_ability_char');const pack=p==='rankChar'?getRankBrowsePack('rankChar'):getBrowsePack(p);const rows=groupBrowseRowsNoLevel((pack&&pack.abilities)||[]);const allLbl=t('list_filter_all_abilities_char');const toolbarLead=`<img class="skill-filter-toolbar-ic" src="${imgUrl(CHAR_ABILITY_BROWSE_ALL_ICON)}" alt="" role="presentation">`;if(sel.length===0){label.innerHTML=`<span class="skill-filter-toolbar-label">${toolbarLead}<span class="source-filter-btn-plain">${esc(allLbl)}</span></span>`;btn.classList.remove('active');return}btn.classList.add('active');if(sel.length===1){let name=sel[0];const found=rows.find(x=>String(x.id)===String(sel[0]));if(found)name=found.name;const ic=found&&found.icon?`<img class="skill-filter-toolbar-ic" src="${imgUrl(found.icon)}" alt="" role="presentation">`:'';label.innerHTML=`<span class="skill-filter-toolbar-label">${ic}<span class="source-filter-btn-plain">${esc(name)}</span></span>`;return}label.innerHTML=`<span class="skill-filter-toolbar-label"><span class="source-filter-btn-plain">${esc(t('list_filter_lineage_multi').replace('{n}',String(sel.length)))}</span></span>`}
...
if(S.spActive){sr=cp?d.sp_stats_with_ex:d.sp_stats;bs=d.sp_stats}
else{sr=cp?d.stats_with_ex:d.stats;bs=d.stats}
}else{sr=cp?d.stats_with_ex:d.stats;bs=d.stats}
}
let th='';
if(hcf){const cplab=type==='unit'?_dcUnitConditionalToggleLabel(d):t('conditional_passive');th=`<div class="conditional-toggle" style="justify-content:flex-end;margin-bottom:8px;"><div class="toggle-clickable ${S.conditionalPassiveActive?'active':''}" role="button" tabindex="0" onclick="toggleConditionalPassive(!S.conditionalPassiveActive)" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();toggleConditionalPassive(!S.conditionalPassiveActive)}"><span class="toggle-switch"></span><span class="toggle-label">${esc(cplab)}</span></div></div>`}
let exRow='';
...
const b=bs[i];const hcb=cp&&s.total>b.total;let eb='';
if(s.bonus>0){const showSsp=(type==='unit'&&S.sspActive&&s.name==='Move');eb=`<div class="stat-card-bonus">${showSsp?`<img src="${imgUrl('/static/images/UI/UI_Common_Icon_Ssp.webp')}" alt="SSP" style="height:14px;vertical-align:-2px;margin-right:4px;" onerror="this.style.display='none'">`:''}+${fmtN(s.bonus)}</div>`}
if(!isRankingView||(type==='unit'&&s.name==='Move'))return`<div class="stat-card ${hcb?'has-cond-bonus':''}"><div class="stat-card-label">${esc(tStat(s.name,type))}</div><div class="stat-card-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`;
const meta=detailRankingMetaFor(type,d.id,s.name);const loading=!meta||!meta.rank||!meta.total;const w=loading?62:detailRankingBarWidth(meta);const pos=loading?'...':detailRankingPosText(meta);
return`<div class="stat-card stat-card--ranking ${hcb?'has-cond-bonus':''} ${loading?'is-loading':''}"><div class="stat-rank-head"><div class="stat-card-label stat-rank-label">${esc(tStat(s.name,type))}</div><div class="stat-rank-bar"><span class="stat-rank-fill ${loading?'is-loading':''}" style="width:${w.toFixed(2)}%"></span></div><div class="stat-rank-pos ${loading?'is-loading':''}">${esc(pos)}</div></div><div class="stat-card-value stat-rank-value ${hcb?'has-bonus-val':''}">${fmtN(s.total)}</div>${eb}</div>`
}).join('');
...
function ensureRankingTopControlEnhancements(){const row=document.querySelector('#panel-ranking .ranking-sort-row');if(!row)return;const dirBtn=document.getElementById('rankingDirBtn');const listSvg=`<svg class="ranking-view-svg" viewBox="0 0 18 18" aria-hidden="true"><rect x="2" y="3.5" width="14" height="2.5" rx="1"/><rect x="2" y="8" width="14" height="2.5" rx="1"/><rect x="2" y="12.5" width="14" height="2.5" rx="1"/></svg>`;const html=`<button type="button" class="ranking-view-btn" data-ranking-view="list" onclick="setRankingViewMode('list')" title="List">${listSvg}<span class="ranking-view-txt">List</span></button><button type="button" class="ranking-view-btn" data-ranking-view="podium" onclick="setRankingViewMode('podium')" title="Dashboard"><img class="ranking-view-icon" src="${imgUrl('/static/images/UI/UI_Home_Campaign_Image_01.webp')}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'"><span class="ranking-view-txt">Dashboard</span></button>`;let tail=row.querySelector('.ranking-tail-controls');if(!tail){tail=document.createElement('div');tail.className='ranking-tail-controls';row.appendChild(tail)}let toggle=row.querySelector('.ranking-view-toggle');if(!toggle){toggle=document.createElement('div');toggle.className='ranking-view-toggle';toggle.setAttribute('role','group');toggle.setAttribute('aria-label','Ranking view mode')}toggle.innerHTML=html;if(toggle.parentElement!==tail)tail.appendChild(toggle);if(dirBtn&&dirBtn.parentElement!==tail)tail.appendChild(dirBtn)}
...
async function warmDetailRankingIndex(type){const key=detailRankingIndexKey(type);if(_detailRankingIndexCache.has(key))return _detailRankingIndexCache.get(key);if(_detailRankingIndexInflight.has(key))return _detailRankingIndexInflight.get(key);const mode=type==='character'?'characters':'units';const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB'];const p=(async()=>{const out={};await Promise.all(sortKeys.map(async sk=>{try{const reqPer=5000;let page=1,total=0,maxPages=1,effPer=reqPer;const byId=new Map();let bounds=null;while(page<=maxPages){const r=await fetch(rankingListUrlFor(mode,sk,page,reqPer,'desc'));if(!r.ok)break;const d=await r.json();const rows=Array.isArray(d&&d.rows)?d.rows:[];if(page===1){total=Number(d&&d.total||0);effPer=Number(d&&d.per_page||rows.length||reqPer)||reqPer;maxPages=Math.max(1,Math.ceil((total||0)/effPer)||1);bounds=d&&d.stat_bounds||null}rows.forEach((x,i)=>{byId.set(String(x&&x.id||''),{rank:(page-1)*effPer+i+1,value:Number(x&&x[sk]||0)})});if(!rows.length)break;page++}if(byId.size)out[sk]={total:total||byId.size,bounds,byId}}catch(_){}}));if(Object.keys(out).length)_detailRankingIndexCache.set(key,out);while(_detailRankingIndexCache.size>20){const k=_detailRankingIndexCache.keys().next().value;_detailRankingIndexCache.delete(k)}return out})().finally(()=>_detailRankingIndexInflight.delete(key));_detailRankingIndexInflight.set(key,p);return p}
...
async function warmDetailRankingIndex(type){const key=detailRankingIndexKey(type);if(_detailRankingIndexCache.has(key))return _detailRankingIndexCache.get(key);if(_detailRankingIndexInflight.has(key))return _detailRankingIndexInflight.get(key);const mode=type==='character'?'characters':'units';const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB'];const p=(async()=>{const reqPer=5000;let page=1,total=0,maxPages=1,effPer=reqPer;const all=[];while(page<=maxPages){const r=await fetch(rankingListUrlFor(mode,sortKeys[0],page,reqPer,'desc'));if(!r.ok)break;const d=await r.json();const rows=Array.isArray(d&&d.rows)?d.rows:[];if(page===1){total=Number(d&&d.total||0);effPer=Number(d&&d.per_page||rows.length||reqPer)||reqPer;maxPages=Math.max(1,Math.ceil((total||0)/effPer)||1)}if(rows.length)all.push(...rows);if(!rows.length)break;page++}const out={};if(all.length){sortKeys.forEach(sk=>{const arr=all.slice().sort((a,b)=>{const av=detailRankingStatValueFromRow(a,sk),bv=detailRankingStatValueFromRow(b,sk);if(bv!==av)return bv-av;const an=String(a&&a.name||''),bn=String(b&&b.name||'');if(an!==bn)return an.localeCompare(bn);return String(a&&a.id||'').localeCompare(String(b&&b.id||''))});const byId=new Map();let mn=Infinity,mx=-Infinity;arr.forEach((x,i)=>{const val=detailRankingStatValueFromRow(x,sk);byId.set(String(x&&x.id||''),{rank:i+1,value:val});if(val<mn)mn=val;if(val>mx)mx=val});if(byId.size)out[sk]={total:total||arr.length,bounds:Number.isFinite(mn)&&Number.isFinite(mx)?{min:mn,max:mx}:null,byId}})}if(Object.keys(out).length)_detailRankingIndexCache.set(key,out);while(_detailRankingIndexCache.size>20){const k=_detailRankingIndexCache.keys().next().value;_detailRankingIndexCache.delete(k)}return out})().finally(()=>_detailRankingIndexInflight.delete(key));_detailRankingIndexInflight.set(key,p);return p}
async function ensureDetailRankingStats(type,id){if((type!=='character'&&type!=='unit')||!id)return null;const ck=detailRankingCacheKey(type,id);const iid=String(id);if(_detailRankingStatsCache.has(ck))return _detailRankingStatsCache.get(ck);if(_detailRankingStatsInflight.has(ck))return await _detailRankingStatsInflight.get(ck);seedDetailRankingFromLastPayload(type,iid,ck);let byStat=primeDetailRankingMetaFromIndex(type,iid,_detailRankingStatsCache.get(ck)||{});if(Object.keys(byStat).length){_detailRankingStatsCache.set(ck,byStat);refreshDetailRankingIfActive(type,iid)}const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB'];const missing=sortKeys.filter(sk=>!byStat[sk]);if(!missing.length)return byStat;void warmDetailRankingIndex(type).then(()=>{const merged=primeDetailRankingMetaFromIndex(type,iid,_detailRankingStatsCache.get(ck)||{});_detailRankingStatsCache.set(ck,merged);refreshDetailRankingIfActive(type,iid)}).catch(()=>{});const p=(async()=>{const cur=_detailRankingStatsCache.get(ck)||{};await Promise.all(missing.map(async sk=>{try{const meta=await fetchDetailRankingMetaForSort(type,sk,iid);if(meta)cur[sk]=meta}catch(_){}}));_detailRankingStatsCache.set(ck,cur);return cur})().finally(()=>_detailRankingStatsInflight.delete(ck));_detailRankingStatsInflight.set(ck,p);return await p}
function renderRankingList(d){const rows=d.rows||[];const bounds=d.stat_bounds;const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';if(vm==='podium'){const topRows=rows.slice(0,15);const vals=topRows.map(r=>Number(r&&r[sortKey]||0));const mx=Math.max(1,...vals);const pow=.68;const items=topRows.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const val=Number(row&&row[sortKey]||0);const norm=mx>0?val/mx:0;const h=Math.max(6,Math.min(100,Math.pow(norm,pow)*100));const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,52);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));const slot=(rank===1)?0:((Math.floor((rank-2)/2)+1)*(rank%2===0?-1:1));return{slot,html:`<button type="button" class="ranking-podium-col" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-podium-val">${fmtN(val)}</span><span class="ranking-podium-rail"><span class="ranking-podium-fill" style="height:${h.toFixed(2)}%"></span></span><span class="ranking-podium-thumb">${img}</span><span class="ranking-podium-rank">#${rank}</span></button>`}}).sort((a,b)=>a.slot-b.slot);host.classList.add('ranking-list-inner--podium');host.innerHTML=items.length?`<div class="ranking-podium-board ranking-podium-board--bars">${items.map(x=>x.html).join('')}</div>`:`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';host.innerHTML=rows.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const statVal=row[sortKey];const name=esc(row.name||'-');const id=escAttr(String(row.id||''));const img=renderListThumb(row,thumbKind,52);const bar=renderRankingRowBar(statVal,bounds);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-rank-num">#${rank}</span><span class="ranking-row-thumb">${img}</span><span class="ranking-row-name">${name}</span><span class="ranking-row-stat"><span class="ranking-stat-val">${fmtN(statVal)}</span>${bar}</span></button>`}).join('');syncRankingViewModeUi();renderPag('rank',d)}
async function loadRankingList(p=1){if(S.currentTab!=='ranking')return;p=Number(p)||1;const mode=S.ranking.mode;const vm=(S.ranking&&S.ranking.viewMode)||'list';if(vm==='podium')p=1;const ppEl=document.getElementById('rankPerPage');const pp=ppEl?parseInt(ppEl.value,10)||50:50;if(mode==='characters')S.ranking.pageChar=p;else S.ranking.pageUnit=p;const url=mode==='characters'?buildRankingCharactersListUrl(p,pp):buildRankingUnitsListUrl(p,pp);const load=document.getElementById('rankLoading');const host=document.getElementById('rankListInner');if(load)load.style.display='flex';if(host)host.innerHTML='';try{const r=await fetch(url);const d=await r.json();_rankingLastPayloadByMode[mode==='characters'?'characters':'units']=d;if(mode==='units'){applyWeaponDebuffPresentFromApi(d);applyMechanismPresentFromApi(d)}renderRankingList(d)}catch(e){if(load)load.style.display='none';if(host)host.innerHTML=`<div class="empty-state"><div class="empty-state-text">${esc(String(e))}</div></div>`}finally{const t=mode==='characters'?'character':'unit';void warmDetailRankingIndex(t)}}

function renderRankingList(d){const rows=d.rows||[];const bounds=d.stat_bounds;const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';const disp=rows.slice(0,20);if(vm==='podium'){host.classList.add('ranking-list-inner--podium');host.innerHTML=disp.length?`<div class="ranking-podium-board ranking-podium-board--dashboard">${disp.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const val=Number(row&&row[sortKey]||0);const id=escAttr(String(row&&row.id||''));const nm=esc(row&&row.name||'-');const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));const img=renderListThumb(row,thumbKind,54);const tier=rank===1?'tier-1':rank===2?'tier-2':rank===3?'tier-3':rank<=10?'tier-4':'tier-5';return`<button type="button" class="ranking-dash-card ${tier}" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-dash-badge">${rank}</span><img class="ranking-dash-corner-icon" src="${imgUrl('/static/images/UI/UI_Home_Campaign_Image_01.webp')}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'"><span class="ranking-dash-thumb">${img}</span><span class="ranking-dash-name">${nm}</span><span class="ranking-dash-val">${fmtN(val)}</span></button>`}).join('')}</div>`:`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';const vals=disp.map(r=>Number(r&&r[sortKey]||0));const mx=Math.max(1,...vals);host.innerHTML=disp.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const statVal=Number(row&&row[sortKey]||0);const norm=Math.max(0,Math.min(1,statVal/mx));const w=Math.max(6,norm*100);const name=esc(row&&row.name||'-');const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,48);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-matrix-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-matrix-thumb">${img}</span><span class="ranking-matrix-name">${name}</span><span class="ranking-matrix-track"><span class="ranking-matrix-fill" style="width:${w.toFixed(2)}%"></span><span class="ranking-matrix-dot">${rank}</span></span><span class="ranking-matrix-val">${fmtN(statVal)}</span></button>`}).join('');syncRankingViewModeUi();renderPag('rank',d)}
function renderRankingList(d){const rows=d.rows||[];const bounds=d.stat_bounds;const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';if(vm==='podium'){const topRows=rows.slice(0,20);const vals=topRows.map(r=>Number(r&&r[sortKey]||0));const mx=Math.max(1,...vals);const pow=.68;const mk=(row,rank)=>{const val=Number(row&&row[sortKey]||0);const norm=mx>0?val/mx:0;const h=Math.max(6,Math.min(100,Math.pow(norm,pow)*100));const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,52);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-podium-col" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-podium-rank">#${rank}</span><span class="ranking-podium-rail"><span class="ranking-podium-fill" style="height:${h.toFixed(2)}%"></span></span><span class="ranking-podium-thumb">${img}</span><span class="ranking-podium-val">${fmtN(val)}</span></button>`};const top3=`<div class="ranking-podium-top3">${topRows[1]?mk(topRows[1],2):''}${topRows[0]?mk(topRows[0],1):''}${topRows[2]?mk(topRows[2],3):''}</div>`;const rest=topRows.slice(3).map((row,i)=>{const rank=i+4;const id=escAttr(String(row&&row.id||''));const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));const val=Number(row&&row[sortKey]||0);return`<button type="button" class="ranking-podium-rest-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-podium-rest-rank">#${rank}</span><span class="ranking-podium-rest-name">${esc(row&&row.name||'-')}</span><span class="ranking-podium-rest-val">${fmtN(val)}</span></button>`}).join('');host.classList.add('ranking-list-inner--podium');host.innerHTML=topRows.length?`<div class="ranking-podium-board ranking-podium-board--split">${top3}<div class="ranking-podium-rest">${rest}</div></div>`:`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';host.innerHTML=rows.map((row,idx)=>{const rank=(d.page-1)*(d.per_page||50)+idx+1;const statVal=row[sortKey];const name=esc(row.name||'-');const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,52);const bar=renderRankingRowBar(statVal,bounds);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-rank-num">#${rank}</span><span class="ranking-row-thumb">${img}</span><span class="ranking-row-name">${name}</span><span class="ranking-row-stat"><span class="ranking-stat-val">${fmtN(statVal)}</span>${bar}</span></button>`}).join('');syncRankingViewModeUi();renderPag('rank',d)}

async function ensureDetailRankingStats(type,id){if((type!=='character'&&type!=='unit')||!id)return null;const ck=detailRankingCacheKey(type,id);const iid=String(id);if(_detailRankingStatsCache.has(ck))return _detailRankingStatsCache.get(ck);if(_detailRankingStatsInflight.has(ck))return await _detailRankingStatsInflight.get(ck);seedDetailRankingFromLastPayload(type,iid,ck);let byStat=primeDetailRankingMetaFromIndex(type,iid,_detailRankingStatsCache.get(ck)||{});if(Object.keys(byStat).length){_detailRankingStatsCache.set(ck,byStat);refreshDetailRankingIfActive(type,iid)}const sortKeys=type==='character'?['Ranged','Melee','Awaken','Defense','Reaction']:['HP','EN','ATK','DEF','MOB'];if(sortKeys.every(sk=>byStat[sk]))return byStat;const p=(async()=>{try{await warmDetailRankingIndex(type)}catch(_){}const merged=primeDetailRankingMetaFromIndex(type,iid,_detailRankingStatsCache.get(ck)||{});_detailRankingStatsCache.set(ck,merged);refreshDetailRankingIfActive(type,iid);return merged})().finally(()=>_detailRankingStatsInflight.delete(ck));_detailRankingStatsInflight.set(ck,p);return await p}

async function loadRankingList(p=1){if(S.currentTab!=='ranking')return;p=1;const mode=S.ranking.mode;const vm=(S.ranking&&S.ranking.viewMode)||'list';const pp=(vm==='list'||vm==='podium')?20:((el=>el?parseInt(el.value,10)||50:50)(document.getElementById('rankPerPage')));if(mode==='characters')S.ranking.pageChar=p;else S.ranking.pageUnit=p;const t=mode==='characters'?'character':'unit';void warmDetailRankingIndex(t);const url=mode==='characters'?buildRankingCharactersListUrl(p,pp):buildRankingUnitsListUrl(p,pp);const load=document.getElementById('rankLoading');const host=document.getElementById('rankListInner');if(load)load.style.display='flex';if(host)host.innerHTML='';try{const r=await fetch(url);const d=await r.json();_rankingLastPayloadByMode[mode==='characters'?'characters':'units']=d;if(mode==='units'){applyWeaponDebuffPresentFromApi(d);applyMechanismPresentFromApi(d)}renderRankingList(d)}catch(e){if(load)load.style.display='none';if(host)host.innerHTML=`<div class="empty-state"><div class="empty-state-text">${esc(String(e))}</div></div>`}finally{void warmDetailRankingIndex(t)}}
...
function renderRankingList(d){const rows=d.rows||[];const sortKey=(d.sort||'');const host=document.getElementById('rankListInner');const empty=document.getElementById('rankEmpty');const load=document.getElementById('rankLoading');if(load)load.style.display='none';if(!host)return;const isChar=S.ranking.mode==='characters';const typ=isChar?'character':'unit';const thumbKind=isChar?'char':'unit';const vm=(S.ranking&&S.ranking.viewMode)||'list';const disp=rows.slice(0,20);if(vm==='podium'){host.classList.add('ranking-list-inner--podium');host.innerHTML=disp.length?`<div class="ranking-podium-board ranking-podium-board--dashboard">${disp.map((row,idx)=>{const rank=idx+1;const badge=rankingOrdinalLabel(rank);const val=Number(row&&row[sortKey]||0);const id=escAttr(String(row&&row.id||''));const nm=esc(row&&row.name||'-');const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));const img=renderListThumb(row,thumbKind,81);const tier=rank===1?'tier-1':rank===2?'tier-2':rank===3?'tier-3':rank<=10?'tier-4':'tier-5';return`<button type="button" class="ranking-dash-card ${tier}" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-dash-badge">${badge}</span><span class="ranking-dash-thumb">${img}</span><span class="ranking-dash-name">${nm}</span><span class="ranking-dash-val">${fmtN(val)}</span></button>`}).join('')}</div>`:`<div class="empty-state"><div class="empty-state-text">${esc(t('empty'))}</div></div>`;if(empty)empty.style.display='none';syncRankingViewModeUi();return}host.classList.remove('ranking-list-inner--podium');if(!rows.length){host.innerHTML='';if(empty){empty.style.display='block';empty.querySelector('.empty-state-text').textContent=S.ranking.mode==='characters'?t('empty_char'):t('empty_unit')}return}if(empty)empty.style.display='none';const vals=disp.map(r=>Number(r&&r[sortKey]||0));const mx=Math.max(1,...vals);host.innerHTML=disp.map((row,idx)=>{const rank=idx+1;const statVal=Number(row&&row[sortKey]||0);const norm=Math.max(0,Math.min(1,statVal/mx));const w=Math.max(12,norm*100);const name=esc(row&&row.name||'-');const id=escAttr(String(row&&row.id||''));const img=renderListThumb(row,thumbKind,48);const o=encodeURIComponent(JSON.stringify(detailRecommendOptsForType(typ)));return`<button type="button" class="ranking-matrix-row" data-detail-type="${typ}" data-detail-id="${id}" data-detail-opts="${o}" onclick="openDetailFromRanking(this)"><span class="ranking-matrix-thumb">${img}</span><span class="ranking-matrix-name">${name}</span><span class="ranking-matrix-track"><span class="ranking-matrix-fill" style="width:${w.toFixed(2)}%"></span><span class="ranking-matrix-dot">${rank}</span></span><span class="ranking-matrix-val">${fmtN(statVal)}</span></button>`}).join('');syncRankingViewModeUi();const pg=document.querySelector('#panel-ranking .pagination-bar-wrap');if(pg)pg.style.display='none'}
