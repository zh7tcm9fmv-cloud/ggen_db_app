...
function renderBannerTimelineRailHz(rows){
  const sorted=btSortBannerRows(rows||[]);
  const items=sorted.slice().sort((a,b)=>(+(a.start_ms||0))-(+(b.start_ms||0)));
  if(!items.length)return'<div class="bt-empty"><div class="empty-state-icon">📭</div><div class="empty-state-text">'+esc(t('bt_empty'))+'</div></div>';
  let h='<div class="bt-timeline-shell bt-ig-shell bt-ig-shell--horizontal bt-hz-classic"><div class="bt-hz-classic-inner"><div class="bt-hz-spine" aria-hidden="true"></div><div class="bt-hz-steps" role="list">';
  items.forEach((b,ii)=>{const pal=ii%7;const dn=b&&b.start_ms>0?btJstStartDayNum(b.start_ms):null;const num=dn!=null?String(dn):'—';const tipRaw=b&&b.start_ms>0?btFmtBannerNodeTooltip(b.start_ms):(b&&(b.name||''))||'';const sub=b&&b.start_ms>0?btFmtTooltipMonth(b.start_ms):'';h+='<div class="bt-hz-step" role="listitem">';h+='<div class="bt-hz-node-wrap">';h+='<span class="bt-hz-marker bt-ig-node bt-ig-node--pal-'+pal+'" title="'+escAttr(tipRaw)+'"><span class="bt-ig-num">'+esc(num)+'</span></span>';if(sub)h+='<span class="bt-hz-sublabel">'+esc(sub)+'</span>';h+='</div>';h+='<div class="bt-hz-card-slot">'+btBannerTimelineCardBodyHtml(b)+'</div></div>'});
  h+='</div></div></div>';
  return h;
}
...
function updateBannerTimelineScrollFabVisibility(){const b=document.getElementById('bannerTimelineScrollTopFab');if(!b)return;const st=Math.max(window.scrollY||0,document.documentElement.scrollTop||0,document.body.scrollTop||0);const show=S.currentTab==='banner_timeline'&&st>180;b.hidden=!show}
function initBannerTimelineExtras(){if(S._bannerTimelineExtras)return;S._bannerTimelineExtras=1;window.addEventListener('scroll',updateBannerTimelineScrollFabVisibility,{passive:true});const mql=window.matchMedia('(min-width: 1180px)');function onMqlChange(){if(S.currentTab!=='banner_timeline'||!S.btCacheData||S.bannerTimelineView!=='timeline')return;renderBannerTimeline(S.btCacheData)}if(typeof mql.addEventListener==='function')mql.addEventListener('change',onMqlChange);else if(typeof mql.addListener==='function')mql.addListener(onMqlChange)}
