...
return`<button type="button" class="ranking-matrix-row" data-detail-type="${typ}" data-detail-id="${id}" data-rank-value="${statVal}" data-detail-opts="${o}" onclick="onRankingMatrixRowClick(this,event)"><span class="ranking-matrix-start"><span class="ranking-matrix-thumb">${img}</span><span class="ranking-matrix-name">${name}</span></span><span class="ranking-matrix-track"><span class="ranking-matrix-fill" style="width:${w.toFixed(2)}%"></span></span><span class="ranking-matrix-end"><span class="ranking-matrix-dot">${rankLabel}</span>${pctHtml}<span class="ranking-matrix-val">${fmtN(statVal)}</span></span></button>`
}).join('');
syncRankingViewModeUi();
