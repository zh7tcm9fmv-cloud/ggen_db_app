...
function btBannerTimelineCardBodyHtml(b){const thPx=56;const img=b.banner_url?'<div class="bt-banner-thumb bt-vtl-thumb"><img src="'+escAttr(imgUrl(b.banner_url))+'" alt="" loading="lazy" decoding="async"></div>':'';const dsl=btFmtScheduleSlot(b.start_label,b);const del=btFmtScheduleSlot(b.end_label,b);const dur=btFmtDuration(b);const meta='<div class="bt-vtl-meta-compact">'+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_start'))+'</span> '+esc(dsl)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_end'))+'</span> '+esc(del)+'</span>'+' <span class="bt-vtl-msep">·</span> '+'<span class="bt-vtl-meta-bit"><span class="bt-vtl-mk">'+esc(t('bt_col_duration'))+'</span> <span class="bt-vtl-dur-inline">'+esc(dur)+'</span></span></div>';const head='<div class="bt-vtl-head"><h3 class="bt-vtl-title">'+esc(b.name||'')+'</h3>'+img+meta+'</div>';const blocks='<div class="bt-vtl-blocks"><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_units'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_units,thPx)+'</div></section><section class="bt-vtl-block"><div class="bt-vtl-sub">'+esc(t('bt_col_chars'))+'</div><div class="bt-vtl-strip-wrap">'+btFeaturedCell(b.featured_chars,thPx)+'</div></section></div>';return'<div class="bt-vtl-body">'+head+blocks+'</div>'}
function renderBannerTimelineRail(rows){const sorted=btSortBannerRows(rows||[]);let h='<div class="bt-timeline-shell bt-ig-shell"><div class="bt-ig-spine-line" aria-hidden="true"></div><div class="bt-ig-list">';let lastYm='';sorted.forEach((b,idx)=>{const ym=btYmKey(b&&b.start_ms);if(ym!==lastYm&&ym&&ym!=='__'){lastYm=ym;h+='<div class="bt-ig-monthlabel">'+esc(btMonthHeadingFromKey(ym))+'</div>'}const isRight=idx%2===0;const tipA=escAttr(btFmtTooltipMonth(b.start_ms)),num=String(idx+1).padStart(2,'0'),pal=idx%7;const card='<div class="bt-ig-card-skin">'+btBannerTimelineCardBodyHtml(b)+'</div>';const emp='<div class="bt-ig-zone bt-ig-zone--blank" aria-hidden="true"></div>';const left=isRight?emp:'<div class="bt-ig-zone bt-ig-zone--card">'+card+'</div>';const right=isRight?'<div class="bt-ig-zone bt-ig-zone--card">'+card+'</div>':emp;const node='<div class="bt-ig-rail-slot"><div class="bt-ig-node bt-ig-node--pal-'+pal+'" title="'+tipA+'" aria-label="'+tipA+'"><span class="bt-ig-num">'+esc(num)+'</span></div></div>';h+='<div class="bt-ig-step '+(isRight?'bt-ig-step--right':'bt-ig-step--left')+' bt-ig-step--pal-'+pal+' bt-vtl-card">'+left+node+right+'</div>'});h+='</div></div>';return h}
