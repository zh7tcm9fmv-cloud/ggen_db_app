...
function pruneUnitWeaponDebuffSelectionForLocale(){if(!Array.isArray(S.listUnitWeaponDebuff))return false;const drop=new Set(['range_all','mp_2','mp_3']);const next=S.listUnitWeaponDebuff.filter(k=>!drop.has(String(k)));if(next.length===S.listUnitWeaponDebuff.length)return false;S.listUnitWeaponDebuff=next;const body=document.getElementById('unitWeaponDebuffFilterBody');if(body)body.innerHTML=buildUnitWeaponDebuffFilterHtml();return true}
