...
Object.assign(T.EN,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'Rewards'});
Object.assign(T.TW,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'獎勵'});
Object.assign(T.HK,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'獎勵'});
Object.assign(T.JA,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'報酬'});
Object.assign(T.JP,{stage_source_tower:'GTower',stage_tower_e:'GTower E',stage_tower_w:'GTower W',stage_rewards:'報酬'});
...
function renderTowerRewardSection(d){const rows=Array.isArray(d.stage_rewards)?d.stage_rewards:(Array.isArray(d.tower_rewards)?d.tower_rewards:[]);if(!rows.length)return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div class="stage-condition-item">${esc(t('none'))}</div></div>`;return`<div class="detail-section"><div class="section-title">${esc(t('stage_rewards'))}</div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px;">${rows.map(r=>{const clickable=!!(r.detail_type&&r.detail_id);const clickAttrs=clickable?` role="button" tabindex="0" onclick="openDetail('${escJs(String(r.detail_type))}','${escJs(String(r.detail_id))}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openDetail('${escJs(String(r.detail_type))}','${escJs(String(r.detail_id))}')}"`:'';
const rt=String(r.reward_type_index||'');
const thumbHtml=(r.thumb_type==='char'&&r.thumb)?renderListThumb({thum:r.thumb,rarity:r.rarity||'N',role_icon:r.role_icon||'',acquisition_icon:r.acquisition_icon||''},'char',54):(r.thumb_type==='option_part'&&r.thumb)?renderListThumb({thum:r.thumb,rarity:r.rarity||'N'},'unit',54,{pickerThumb:true}):(rt==='30'&&r.icon)?`<div style="width:132px;height:54px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);border-radius:8px;flex-shrink:0;overflow:hidden;"><img src="${imgUrl(r.icon)}" alt="${escAttr(r.name||'')}" loading="lazy" decoding="async" style="width:126px;height:42px;object-fit:contain;display:block;" onerror="this.style.display='none'"></div>`:`<div style="width:54px;height:54px;display:flex;align-items:center;justify-content:center;background:rgba(255,255,255,0.04);border:1px solid var(--border-color);border-radius:8px;flex-shrink:0;overflow:hidden;">${r.icon?`<img src="${imgUrl(r.icon)}" alt="${escAttr(r.name||'')}" loading="lazy" decoding="async" style="width:42px;height:42px;object-fit:contain;display:block;" onerror="this.style.display='none'">`:'<span style="font-size:12px;color:var(--text-muted)">ITEM</span>'}</div>`;
return`<div class="ability-item${clickable?' detail-clickable-item':''}" style="display:flex;align-items:center;gap:10px;margin:0;${clickable?'cursor:pointer;':''}"${clickAttrs}>${thumbHtml}<div style="min-width:0;display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;"><div style="min-width:0;"><div style="font-size:13px;line-height:1.3;word-break:break-word;">${esc(r.name||'')}</div>${r.description?`<div style="font-size:11px;opacity:.75;line-height:1.2;margin-top:2px;">${esc(r.description)}</div>`:''}</div><span class="stage-meta-badge" style="white-space:nowrap;">x${fmtN(r.count||0)}</span></div></div>`}).join('')}</div></div>`}
