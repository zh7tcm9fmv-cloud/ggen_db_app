...
function ensureStageTowerFilterDom(){let wrap=document.getElementById('stageTowerWrap');if(wrap)return wrap;const host=document.querySelector('#panel-stages .list-toolbar-left');const diff=document.getElementById('stageDiffWrap');if(!host||!diff)return null;wrap=document.createElement('div');wrap.className='stage-source-toggle-wrap';wrap.id='stageTowerWrap';wrap.setAttribute('role','group');wrap.innerHTML=`<button type="button" class="stage-source-toggle-btn" id="stageTowerEastBtn" onclick="setStageTowerSide('E')" aria-pressed="false"><img src="${imgUrl(_stageTowerLogoBySide('E'))}" alt="" loading="lazy" style="width:18px;height:18px;object-fit:contain;margin-right:6px;vertical-align:middle"><span>${esc(_stageTowerLabelBySide('E'))}</span></button><button type="button" class="stage-source-toggle-btn" id="stageTowerWestBtn" onclick="setStageTowerSide('W')" aria-pressed="false"><img src="${imgUrl(_stageTowerLogoBySide('W'))}" alt="" loading="lazy" style="width:18px;height:18px;object-fit:contain;margin-right:6px;vertical-align:middle"><span>${esc(_stageTowerLabelBySide('W'))}</span></button>`;host.insertBefore(wrap,diff);return wrap}
