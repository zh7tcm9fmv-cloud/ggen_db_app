...
function btUserNeedsVoteNotice(){if(!S._btVoteStatusLoaded)return false;const enabled=btVoteEnabledBanners();if(!enabled.length)return false;return enabled.some(b=>!btVoteMineList(String(b.gasha_id||'')).length)}
function updateBtVoteNotices(){const show=btUserNeedsVoteNotice();const tab=document.getElementById('navBannerTimelineTab');if(tab)tab.classList.toggle('has-ui-notice',show);['btVoteModeBtn','btVoteModeBtnMobile'].forEach(id=>{const btn=document.getElementById(id);if(!btn)return;const anchor=btn.closest('.ui-notice-anchor')||btn;anchor.classList.toggle('has-ui-notice',show&&!S.btVoteMode)})}
...
function toggleBtVoteMode(){if(!S.btCacheData)return;const root=document.getElementById('bannerTimelineRoot');if(!root||!root.querySelector('.bt-table'))return;document.querySelectorAll('.bt-vote-mode-btn').forEach(b=>{b.disabled=true});S.btVoteMode=!S.btVoteMode;const finish=()=>{document.querySelectorAll('.bt-vote-mode-btn').forEach(b=>{b.disabled=false});updateBtVoteNotices();requestAnimationFrame(()=>{armBannerTimelineScrollFabBaseline();updateBannerTimelineScrollFabVisibility()})};const apply=()=>{if(!btSwapVoteTbody())renderBannerTimeline(S.btCacheData);finish()};requestAnimationFrame(apply)}
