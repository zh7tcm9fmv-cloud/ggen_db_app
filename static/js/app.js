...
function bindSearchRecallObserver(){const fab=document.getElementById('searchRecallFab');if(S._searchRecallObs){S._searchRecallObs.disconnect();S._searchRecallObs=null}if(!fab)return;if(S.currentTab==='latest_release'||S.currentTab==='banner_timeline'||S.currentTab==='calculator'||S.currentTab==='team_builder'){fab.classList.remove('visible');return}const panel=document.getElementById('panel-'+S.currentTab);if(!panel)return;let anchor=panel.querySelector('.browse-toolbar-top-row');if(!anchor)anchor=panel.querySelector('.list-toolbar-left>.browse-toolbar-leading');if(!anchor)anchor=panel.querySelector('.list-toolbar');if(!anchor)return;function applyRecallFabFromIO(ents){const e=ents&&ents[0];fab.classList.toggle('visible',!!e&&!e.isIntersecting)}S._searchRecallObs=new IntersectionObserver(applyRecallFabFromIO,{root:null,threshold:0,rootMargin:'-72px 0px 0px 0px'});S._searchRecallObs.observe(anchor);requestAnimationFrame(()=>{try{applyRecallFabFromIO(S._searchRecallObs.takeRecords())}catch(_){}});requestAnimationFrame(()=>{try{applyRecallFabFromIO(S._searchRecallObs.takeRecords())}catch(_){}})}
