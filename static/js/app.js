...
function getMapWeaponRangeOpSuffix(){const debKey=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units'?'listRankUnitWeaponDebuff':'listUnitWeaponDebuff';let deb=S[debKey];if(!Array.isArray(deb)||!deb.includes('map_weapon'))return '';let sel=S[unitMapWeaponRangeStateKey()];if(!Array.isArray(sel)||sel.length<2)return '';const combKey=S.currentTab==='ranking'&&S.ranking&&S.ranking.mode==='units'?'browseCombRankMapWr':'browseCombMapWr';return '&map_weapon_range_op='+encodeURIComponent(S[combKey]==='or'?'or':'and')}
...
function getWeaponRangeNonMapOpSuffix(){let sel=S.listUnitWeaponRangeNonMap;if(!Array.isArray(sel)||sel.length<2)return '';return '&weapon_range_non_map_op='+encodeURIComponent(S.browseCombWrNm==='or'?'or':'and')}
