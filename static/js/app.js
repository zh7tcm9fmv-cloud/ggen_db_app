...
function _dcDefNpcDispStatPair(total,bonus){
const t=Math.max(0,Number(total)||0),b=Math.max(0,Number(bonus)||0);
if(_dcDefNpcMapBonusesEnabled())return{val:t,bonusLine:b};
return{val:Math.max(0,t-b),bonusLine:0};
}
...
if(lb){lb.textContent=t('dc_def_npc_map_bonuses');lb.title=t('dc_def_npc_map_bonuses_tip')}
}
...
const defPair=_dcDefNpcDispStatPair(defTotal,defBon);
...
const pr=_dcDefNpcDispStatPair(us[s]||0,ub[s]||0);
...
const pr=_dcDefNpcDispStatPair(cs[s]||0,cb[s]||0);
...
/** stats_raw is total on map; bonus_amounts is the green (+n) increment — base = total − bonus when map bonuses are off. */
const us=defUnit&&defUnit.stats_raw||{},ub=defUnit&&defUnit.bonus_amounts||{};
const cs=defChar&&defChar.stats_raw||{},cb=defChar&&defChar.bonus_amounts||{};
const inclMap=_dcDefNpcMapBonusesEnabled();
const effU=(k)=>{const t=us[k]||0,b=ub[k]||0;return inclMap?t:Math.max(0,t-b)};
const effC=(k)=>{const t=cs[k]||0,b=cb[k]||0;return inclMap?t:Math.max(0,t-b)};
let unitDef=effU('Defense');
let charDef=effC('Defense');
