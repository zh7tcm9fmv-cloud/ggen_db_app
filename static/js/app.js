...
function btScheduleVoteTbodyCache(rows){if(S.btCacheData)btRebuildVoteTbodyCache(rows||S.btCacheData.banners||[])}
...
function toggleBtVoteMode(){if(!S.btCacheData)return;const root=document.getElementById('bannerTimelineRoot');if(!root||!root.querySelector('.bt-table'))return;const turningOn=!S.btVoteMode;S.btVoteMode=!S.btVoteMode;if(turningOn)btMarkVoteNoticeDismissed();btSyncVoteModeChrome();if(S._btTbodyOff==null||S._btTbodyOn==null)btRebuildVoteTbodyCache(S.btCacheData.banners||[]);if(!btSwapVoteTbody())renderBannerTimeline(S.btCacheData);updateBtVoteNotices();requestAnimationFrame(()=>{armBannerTimelineScrollFabBaseline();updateBannerTimelineScrollFabVisibility()})}
