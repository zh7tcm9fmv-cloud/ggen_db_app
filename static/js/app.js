...
let _rankingStatPillIndicatorRo=null;
function syncRankingDirBtnUi(){const db=document.getElementById('rankingDirBtn');if(!db||!S.ranking)return;const mode=S.ranking.mode;const d=mode==='characters'?S.ranking.dirChar:S.ranking.dirUnit;const isDesc=d==='desc';const ic=isDesc?'/static/images/UI/UI_Common_BtnIcon_Order_Down.webp':'/static/images/UI/UI_Common_BtnIcon_Order_UP.webp';db.innerHTML=`<img class="ranking-dir-icon" src="${imgUrl(ic)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none'">`;db.title=isDesc?t('sort_desc'):t('sort_asc');db.setAttribute('aria-label',isDesc?t('sort_desc'):t('sort_asc'))}
function syncRankingStatPillIndicator(instant){const host=document.getElementById('rankingStatPills');if(!host)return;let indicator=host.querySelector('.ranking-stat-pill-indicator');if(!indicator){indicator=document.createElement('span');indicator.className='ranking-stat-pill-indicator';indicator.setAttribute('aria-hidden','true');host.prepend(indicator);if(!_rankingStatPillIndicatorRo&&typeof ResizeObserver!=='undefined'){_rankingStatPillIndicatorRo=new ResizeObserver(()=>syncRankingStatPillIndicator(true));_rankingStatPillIndicatorRo.observe(host)}}const active=host.querySelector('.ranking-stat-pill.active');if(!active){indicator.classList.remove('is-ready');return}const hr=host.getBoundingClientRect();const ar=active.getBoundingClientRect();indicator.classList.toggle('is-instant',!!instant);indicator.style.left=`${ar.left-hr.left+host.scrollLeft}px`;indicator.style.top=`${ar.top-hr.top+host.scrollTop}px`;indicator.style.width=`${ar.width}px`;indicator.style.height=`${ar.height}px`;indicator.classList.add('is-ready');if(instant)requestAnimationFrame(()=>indicator.classList.remove('is-instant'))}
function renderRankingStatPills(){const host=document.getElementById('rankingStatPills');if(!host)return;const mode=S.ranking.mode;const sort=mode==='characters'?S.ranking.sortChar:S.ranking.sortUnit;const keys=rankingSortKeys();host.innerHTML=`<span class="ranking-stat-pill-indicator" aria-hidden="true"></span>`+keys.map(k=>`<button type="button" class="ranking-stat-pill${k===sort?' active':''}" data-rank-stat="${escAttr(k)}" onclick="setRankingSortKey('${escJs(k)}')">${esc(tStat(k,mode==='characters'?'character':'unit'))}</button>`).join('');syncRankingDirBtnUi();requestAnimationFrame(()=>syncRankingStatPillIndicator(true));syncRankingViewModeUi()}
function setRankingSortKey(k){const keys=rankingSortKeys();if(!keys.includes(k))return;const mode=S.ranking.mode;if(mode==='characters')S.ranking.sortChar=k;else S.ranking.sortUnit=k;const host=document.getElementById('rankingStatPills');if(host){host.querySelectorAll('.ranking-stat-pill').forEach(btn=>btn.classList.toggle('active',btn.dataset.rankStat===k));syncRankingStatPillIndicator(false)}else renderRankingStatPills();scheduleRankingListReload()}
function toggleRankingSortDir(){const mode=S.ranking.mode;if(mode==='characters')S.ranking.dirChar=S.ranking.dirChar==='desc'?'asc':'desc';else S.ranking.dirUnit=S.ranking.dirUnit==='desc'?'asc':'desc';syncRankingDirBtnUi();scheduleRankingListReload()}
