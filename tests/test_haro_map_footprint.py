...
class TestHaroMapFootprint(unittest.TestCase):
    ...
    def test_haro_stage_map_has_no_warship_map_origin(self):
        with self.app.app.test_client() as c:
            d = c.get('/api/stage/11000417?lang=EN').get_json()
        haro = next(
            u for u in (d.get('map_data') or {}).get('units') or []
            if self.app.is_haro_map_unit(u.get('unit_id'))
        )
        self.assertNotIn('map_origin', haro)
