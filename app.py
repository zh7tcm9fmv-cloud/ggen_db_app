...
def resolve_condition_tags(cond_id, trait_condition_raw_map, lineage_lookup, series_name_map, lang_code='EN'):
    if cond_id == '0': return []
    raw = trait_condition_raw_map.get(cond_id, {}); resolved = []
...
def resolve_tags(lin_map, eid, lc):
    ld = get_lang_data(lc); llk = ld.get('lineage_lookup', {}); ll = ld.get('lineage_list', []); tags = []
        ...
        if name:
            ...
            for fid, val in ll:
                if fid.endswith(lid) and len(lid) >= 4:
                    if val not in tags: tags.append(val); break
    return sorted(tags)
...
def list_characters():
    ...
    sq = request.args.get('q', '').strip().lower(); ck = f"cl_{lc}_{page}_{pp}_{sb}_{sd}_{sq}"
...
def list_units():
    lc = validate_lang_code(request.args.get('lang', DEFAULT_LANG)); page = max(1, int(request.args.get('page', 1)))
    pp = min(100, max(10, int(request.args.get('per_page', 50)))); sb = request.args.get('sort', 'rarity'); sd = request.args.get('dir', 'desc')
    sq = request.args.get('q', '').strip().lower(); ck = f"ul_{lc}_{page}_{pp}_{sb}_{sd}_{sq}"
    cached = get_cached_response(ck)
    if cached: return jsonify(cached)
    ld = get_lang_data(lc); rows = []
