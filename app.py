...
def browse_combo_from_unit_args(args):
    ...
    return {
        ...
        'weapon_range_non_map_combine': normalize_filter_combine_op(args.get('weapon_range_non_map_op'), 'and'),
        'map_weapon_range_combine': normalize_filter_combine_op(args.get('map_weapon_range_op'), 'and'),
    }
...
def map_weapon_range_filter_cache_fragment(expr):
    if expr is None:
        return 'mwr0'
    xs = []
    for x in expr:
...
_UNIT_WEAPON_RANGE_MOD_CACHE = {}


def _weapon_range_type_keys_from_phrase(phrase):
...
def _parse_weapon_max_range_increases_from_text(text):
    """Return [(type_keys, inc), …] for max-range-up trait lines (EN / JA / zh-Hant)."""
    if not text:
        return []
    s = str(text).strip()
    sl = s.lower()
    out = []
    seen = set()
    for m in re.finditer(r'increase\s+max\s+range\s+of\s+(.+?)\s+by\s+(\d+)', sl, re.I):
    ...
    for m in re.finditer(
            r'((?:物理|光束|特殊|鐳射)(?:[、及或&\s]+(?:物理|光束|特殊|鐳射))*)武[裝装]的最大射程提升(\d+)', s):
        types = _weapon_range_type_keys_from_phrase(m.group(1))
        inc = int(m.group(2))
        key = (types, inc)
        if key not in seen:
            ...
            out.append((types, inc))
    for m in re.finditer(
            r'((?:物理|ビーム|特殊)(?:[、及或\s]+(?:物理|ビーム|特殊))*)武装の最大射程(?:が)?(\d+)(?:増加|上昇|延び)', s):
        types = _weapon_range_type_keys_from_phrase(m.group(1))
        inc = int(m.group(2))
        key = (types, inc)
        if key not in seen:
...
def _weapon_row_matches_range_type_keys(wm, type_keys):
    if not type_keys:
        return False
    ai = str(wm.get('attribute', '0') or '0')
    wkeys = set(WEAPON_ATTR_SET_TYPE_KEYS.get(ai, []) or [])
    return bool(wkeys & set(type_keys))


def _unit_ability_entries_for_weapon_range(uid, ld, lc, stat_mode):
    """Resolved MS ability rows (base + SSP replace/gain) for weapon-range modifier parsing."""
    uid = normalize_id(uid)
    sm = (stat_mode or 'normal').strip().lower()
    if sm not in ('normal', 'sp', 'ssp'):
        sm = 'normal'
    ...
    for ab in sorted(ua, key=lambda x: int(x.get('sort', 0) or 0)):
        ...
        try:
            bab = build_ability_entry(
                ...
                ld['abil_desc_map'], sort_order=ab['sort'], lang_code=lc,
            )
        except Exception:
            continue
        rows.append(bab)
        ...
        for idx, gain_aid in enumerate(gain_list):
            try:
                bab = build_ability_entry(
                    ...
                    ld['lang_text_map'], ldc['lang_text_map'], trait_condition_raw_map,
                    ld['lineage_lookup'], ld['series_name_map'], ability_resource_map,
                    ld['abil_desc_map'], sort_order=max_so + idx + 1, lang_code=lc,
                )
            except Exception:
                continue
            rows.append(bab)
...
def _unit_ability_range_mod_cp_eligible(ad, part, hc, ie, ability_cond, detail_idx):
    """True when this trait line's max-range-up belongs in the CP bucket (matches stat routing)."""
    ...
    if (not is_cond and ad is not None and detail_idx is not None
        ...
        return False
    return _unit_line_ms_stats_conditional_bucket(
        part, hc, ie, is_cond, ability_cond, ad, detail_idx,
    )


def _collect_unit_cp_weapon_range_modifiers(uid, ld, lc, stat_mode):
    """Per-unit list of {type_keys, inc, cp_eligible} from MS abilities."""
    cache_key = (normalize_id(uid), (stat_mode or 'normal').strip().lower(), lc)
    hit = _UNIT_WEAPON_RANGE_MOD_CACHE.get(cache_key)
    if hit is not None:
        return hit
    mods = []
    for ad in _unit_ability_entries_for_weapon_range(uid, ld, lc, stat_mode):
        ...
        for di, d2 in enumerate(ad.get('details', []) or []):
            ...
            for part in parts:
                ...
                cp_ok = _unit_ability_range_mod_cp_eligible(
...
def _conditional_passive_weapon_range_bonus(uid, wid, wm, ld, lc, stat_mode, cond_active):
    """Extra max-range from CP-on MS abilities (e.g. Vigor-gated beam range up)."""
    if not cond_active:
        return 0
    bonus = 0
    for mod in _collect_unit_cp_weapon_range_modifiers(uid, ld, lc, stat_mode):
        if not mod.get('cp_eligible'):
            continue
...
def _unit_weapon_subset_effective_ranges(uid, ld, lc, stat_mode, subset, cond_active=False):
    ...
    for wp in unit_weapon_map.get(uid, []) or []:
        ...
        bonus += _conditional_passive_weapon_range_bonus(uid, wid, wm, ld, lc, sm, cond_active)
        eff = max(0, rx + bonus)
        if eff > 0:
            ranges.add(int(eff))
    return ranges
...
def _weapon_range_non_map_filter_subset(stat_mode, ssp_ex_only=False):
    """Subset for the non-MAP weapon-range filter."""
    if ssp_ex_only:
        return 'ssp_ex'
    if (stat_mode or 'normal').strip().lower() == 'ssp':
        return 'all'
    return 'non_map'


def unit_weapon_subset_max_range(uid, ld, lc, stat_mode, subset, cond_active=False):
    """Max effective range among non-MAP weapons; subset 'ssp_ex' (EX + SSP ids) or 'non_map' (base grid weapons only)."""
    ranges = _unit_weapon_subset_effective_ranges(uid, ld, lc, stat_mode, subset, cond_active)
    if not ranges:
        return None
    return max(ranges)


def unit_weapon_subset_has_range_tier(uid, ld, lc, stat_mode, subset, tier, cond_active=False):
    ...
    if want < 1 or want > 6:
        return False
    return want in _unit_weapon_subset_effective_ranges(uid, ld, lc, stat_mode, subset, cond_active)
...
def unit_has_non_map_weapon_max_range_ge(uid, ld, lc, stat_mode='normal', need_max=6, cond_active=False):
    """True if any grid (non-MAP) weapon has max_range + SSP / CP range bonus ≥ need_max under stat_mode."""
    sm = (stat_mode or 'normal').strip().lower()
    if sm not in ('normal', 'sp', 'ssp'):
        sm = 'normal'
    uid = normalize_id(uid)
...
def unit_matches_weapon_range_filter(uid, ld, lc, want_filter, stat_mode='normal', combine='and', info=None, cond_active=False):
    if want_filter is None:
        return True
    finfo = info if info is not None else unit_info_map.get(normalize_id(uid), {})
    for fid in _unit_ids_for_terrain_filter(uid, finfo):
        if apply_browse_combine_match(
            want_filter,
            lambda x, _fid=fid: unit_weapon_subset_has_range_tier(
                _fid, ld, lc, stat_mode, 'ssp_ex', x, cond_active),
            combine,
        ):
            return True
    return False
...
def _unit_form_matches_weapon_range_non_map_filter(
        uid, ld, lc, want_filter, stat_mode='normal', combine='and', subset='non_map', cond_active=False):
    if want_filter is None:
        return True
    ss = subset if subset in ('non_map', 'ssp_ex', 'all') else 'non_map'
    return apply_browse_combine_match(
        want_filter,
        lambda x: unit_weapon_subset_has_range_tier(uid, ld, lc, stat_mode, ss, x, cond_active),
        combine,
    )


def unit_matches_weapon_range_non_map_filter(
        uid, ld, lc, want_filter, stat_mode='normal', combine='and', subset='non_map', info=None,
        cond_active=False):
    if want_filter is None:
        return True
    finfo = info if info is not None else unit_info_map.get(normalize_id(uid), {})
    for fid in _unit_ids_for_terrain_filter(uid, finfo):
        if _unit_form_matches_weapon_range_non_map_filter(
                fid, ld, lc, want_filter, stat_mode, combine, subset, cond_active):
            return True
    return False


def collect_unit_weapon_range_debuff_keys(uid, ld, lc, stat_mode='normal', cond_active=False):
    """Tiered max-range filter; range_6 includes SSP Custom Core range (type 4) when listing with stat_mode=ssp."""
    if unit_has_non_map_weapon_max_range_ge(uid, ld, lc, stat_mode=stat_mode, need_max=6, cond_active=cond_active):
...
def create_stage_map(d):
    ...
    for item in extract_data_list(d):
        ...
        if sid == '0': continue
        lookup[sid] = {
            'group1_set_id': normalize_id(item.get('Group1SortieRestrictionSetId') or item.get('group1SortieRestrictionSetId')),
            'group2_set_id': normalize_id(item.get('Group2SortieRestrictionSetId') or item.get('group2SortieRestrictionSetId')),
            'group1_sortie_count': safe_int(item.get('Group1SortieCount') or item.get('group1SortieCount'), 0),
            'group2_sortie_count': safe_int(item.get('Group2SortieCount') or item.get('group2SortieCount'), 0),
            'recommended_cp': safe_int(item.get('RecommendedCombatPower'), 0),
            'terrain_type_index': normalize_id(item.get('StageTerrainTypeIndex') or item.get('stageTerrainTypeIndex')),
            'sortie_terrain_type_index': normalize_id(item.get('SortieTerrainTypeIndex') or item.get('sortieTerrainTypeIndex')),
            'battle_condition_set_id': normalize_id(item.get('StageBattleConditionSetId') or item.get('stageBattleConditionSetId')),
            'schedule_id': normalize_id(item.get('ScheduleId') or item.get('scheduleId'), '0'),
            ...
            'stage_drop_set_id': normalize_id(item.get('StageDropSetId') or item.get('stageDropSetId')),
            'stage_score_evaluation_set_id': normalize_id(item.get('StageScoreEvaluationSetId') or item.get('stageScoreEvaluationSetId')),
            'base_score': safe_int(item.get('BaseScore') or item.get('baseScore'), 0),
        }
    return lookup


def create_stage_drop_lookups(drop_data, content_data):
    content_by_set = {}
    for item in extract_data_list(content_data):
        if not isinstance(item, dict):
            continue
        cset = normalize_id(item.get('StageDropContentSetId') or item.get('stageDropContentSetId'))
        if cset == '0':
            continue
        content_by_set.setdefault(cset, []).append({
            'weight': safe_int(item.get('Weight') or item.get('weight'), 0),
            'reward_set_id': normalize_id(item.get('RewardSetId') or item.get('rewardSetId')),
        })
    for arr in content_by_set.values():
        arr.sort(key=lambda r: (-safe_int(r.get('weight'), 0), r.get('reward_set_id', '')))
    by_drop_set = {}
    for item in extract_data_list(drop_data):
        if not isinstance(item, dict):
            continue
        dsid = normalize_id(item.get('StageDropSetId') or item.get('stageDropSetId'))
        if dsid == '0':
            continue
        by_drop_set.setdefault(dsid, []).append({
            'lottery_count': safe_int(item.get('LotteryCount') or item.get('lotteryCount'), 1),
            'content_set_id': normalize_id(item.get('StageDropContentSetId') or item.get('stageDropContentSetId')),
        })
    return by_drop_set, content_by_set


def create_stage_score_eval_by_set(d):
    by = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('StageScoreEvaluationSetId') or item.get('stageScoreEvaluationSetId'))
        if sid == '0':
            continue
        by.setdefault(sid, []).append({
            'step': safe_int(item.get('Step') or item.get('step'), 0),
            'score': safe_int(item.get('Score') or item.get('score'), 0),
        })
    for arr in by.values():
        arr.sort(key=lambda r: (-safe_int(r.get('step'), 0), -safe_int(r.get('score'), 0)))
    return by


def create_stage_score_eval_percent_list(d):
    rows = []
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ti = safe_int(item.get('StageScoreEvaluationTypeIndex') or item.get('stageScoreEvaluationTypeIndex'), 0)
        if ti <= 0:
            continue
        rows.append({
            'type_index': ti,
            'percent': safe_int(item.get('Percent') or item.get('percent'), 0),
        })
    rows.sort(key=lambda r: safe_int(r.get('type_index'), 0))
    return rows


def create_stage_recommended_sortie_by_terrain(d):
    by = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sti = normalize_id(item.get('StageTerrainTypeIndex') or item.get('stageTerrainTypeIndex'))
        rti = normalize_id(item.get('RecommendedSortieTerrainTypeIndex') or item.get('recommendedSortieTerrainTypeIndex'))
        if sti == '0' or rti == '0':
            continue
        by.setdefault(sti, []).append(rti)
    return by


def create_stage_reinforcement_area_by_stage(d):
    by = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        if sid == '0':
            continue
        by.setdefault(sid, []).append({
            'x': safe_int(item.get('X') or item.get('x'), 0),
            'y': safe_int(item.get('Y') or item.get('y'), 0),
            'w': max(1, safe_int(item.get('Width') or item.get('width'), 1)),
            'h': max(1, safe_int(item.get('Height') or item.get('height'), 1)),
            'reinforcement_limit': safe_int(item.get('ReinforcementLimit') or item.get('reinforcementLimit'), 0),
        })
    return by


def create_stage_event_area_skill_lookup(d):
    lk = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
...
def create_map_stage_event_lookup(d):
    lk = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        lk[eid] = {
            ...
            'event_type_index': safe_int(item.get('EventTypeIndex') or item.get('eventTypeIndex'), 0),
        }
    return lk
...
def create_scenario_stage_map(d):
    ...
    for item in extract_data_list(d):
        ...
        if sid == '0':
            continue
        lookup[sid] = {
            'scenario_stage_series_id': normalize_id(item.get('ScenarioStageSeriesId') or item.get('scenarioStageSeriesId')),
            'thumbnail_resource_id': str(item.get('ThumbnailResourceId') or item.get('thumbnailResourceId') or '').strip(),
            'stage_difficulty_type_index': safe_int(item.get('StageDifficultyTypeIndex') or item.get('stageDifficultyTypeIndex'), 1),
...
def create_main_stage_series_challenge_map(d):
    lookup = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        lookup[cid] = {
            'id': cid,
            'main_stage_series_id': normalize_id(item.get('MainStageSeriesId') or item.get('mainStageSeriesId')),
            'bromide_resource_id': str(item.get('BromideResourceId') or item.get('bromideResourceId') or '').strip(),
            'recommended_combat_power': safe_int(item.get('RecommendedCombatPower') or item.get('recommendedCombatPower'), 0),
        }
    return lookup

def create_main_stage_challenge_map(d):
    lookup = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('Id') or item.get('StageId') or item.get('stageId'))
        if sid == '0':
            continue
        lookup[sid] = {
            'stage_id': sid,
...
def create_main_scenario_stage_challenge_map(d):
    lookup = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('ScenarioStageId') or item.get('scenarioStageId') or item.get('Id') or item.get('id'))
        if sid == '0':
            continue
        lookup[sid] = {
            'scenario_stage_id': sid,
            'main_stage_series_challenge_id': normalize_id(item.get('MainStageSeriesChallengeId') or item.get('mainStageSeriesChallengeId')),
            'stage_number': safe_int(item.get('StageNumber') or item.get('stageNumber'), 0),
...
def create_capturable_units_by_stage(d):
    ...
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('StageId') or item.get('stageId'))
        uid = normalize_id(item.get('MainUnitId') or item.get('mainUnitId'))
        if sid == '0' or uid == '0':
            continue
        lookup.setdefault(sid, []).append({
            'main_unit_id': uid,
            'is_pickup': bool(item.get('IsStageSelectPickupCaptureUnit') or item.get('isStageSelectPickupCaptureUnit')),
            'default_capture_rate_permil': safe_int(item.get('DefaultCaptureRatePermil') or item.get('defaultCaptureRatePermil'), 0),
            'leader_killed_capture_rate_permil': safe_int(item.get('LeaderKilledCaptureRatePermil') or item.get('leaderKilledCaptureRatePermil'), 0),
        })
    return lookup

def create_main_stage_challenge_npc_unit_override_by_stage(d):
    """MainStageChallengeId -> MapNpcId -> [unit rows] (same shape as map_npc_unit_lookup values)."""
    by_stage = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('MainStageChallengeId') or item.get('mainStageChallengeId'))
        ...
        row = {
            ...
            'hp': safe_int(item.get('Hp'), 0),
            'en': safe_int(item.get('En'), 0),
            'attack': safe_int(item.get('Attack'), 0),
            'defense': safe_int(item.get('Defense'), 0),
            'mobility': safe_int(item.get('Mobility'), 0),
            'movement': safe_int(item.get('Movement'), 0),
            'ability_set_id': normalize_id(item.get('MapNpcUnitAbilitySetId') or item.get('mapNpcUnitAbilitySetId')),
            'weapon_set_id': normalize_id(item.get('MapNpcUnitWeaponSetId') or item.get('mapNpcUnitWeaponSetId')),
        }
        by_stage.setdefault(sid, {}).setdefault(nid, []).append(row)
    for stage_rows in by_stage.values():
        for rows in stage_rows.values():
            rows.sort(key=lambda x: x['number'])
    return by_stage

def create_main_stage_challenge_npc_character_override_by_stage(d):
    """MainStageChallengeId -> MapNpcId -> [character rows] (same shape as map_npc_character_lookup values)."""
    by_stage = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        row = {
            'number': safe_int(item.get('Number'), 0),
            'character_id': normalize_id(item.get('CharacterId') or item.get('characterId')),
            'level': safe_int(item.get('Level'), 0),
            'ranged': safe_int(item.get('Ranged'), 0),
            'melee': safe_int(item.get('Melee'), 0),
            'defense': safe_int(item.get('Defense'), 0),
            'reaction': safe_int(item.get('Reaction'), 0),
            'awaken': safe_int(item.get('Awaken'), 0),
            'ability_set_id': normalize_id(item.get('MapNpcCharacterAbilitySetId') or item.get('mapNpcCharacterAbilitySetId')),
            'skill_set_id': normalize_id(item.get('MapNpcCharacterSkillSetId') or item.get('mapNpcCharacterSkillSetId')),
        }
        by_stage.setdefault(sid, {}).setdefault(nid, []).append(row)
    for stage_rows in by_stage.values():
        for rows in stage_rows.values():
...
def create_main_stage_series_challenge_layout_map(d):
    lookup = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        sid = normalize_id(item.get('MainStageSeriesChallengeId') or item.get('mainStageSeriesChallengeId'))
        if sid == '0':
            continue
        lookup[sid] = {
            'logo_offset_x': safe_int(item.get('LogoOffsetX') or item.get('logoOffsetX'), 0),
            'bromide_scale_percent': safe_int(item.get('BromideScalePercent') or item.get('bromideScalePercent'), 100),
        }
    return lookup
...
def create_map_stage_conditions_by_map_stage(d):
    by_msid = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        if msid == '0':
            continue
        by_msid.setdefault(msid, []).append({
            'sort_order': safe_int(item.get('SortOrder') or item.get('sortOrder'), 0),
            'condition_type_index': safe_int(item.get('MapStageConditionTypeIndex') or item.get('mapStageConditionTypeIndex'), 0),
            'text_lang_id': normalize_id(item.get('ShortConditionSentenceLanguageId') or item.get('shortConditionSentenceLanguageId')),
        })
    for arr in by_msid.values():
        arr.sort(key=lambda x: (safe_int(x.get('sort_order'), 0), x.get('text_lang_id', '')))
    return by_msid


def create_map_stage_event_areas_by_map_stage(d):
    by_msid = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        ...
        by_msid.setdefault(msid, []).append({
            ...
            'w': max(1, safe_int(item.get('Width') or item.get('width'), 1)),
            'h': max(1, safe_int(item.get('Height') or item.get('height'), 1)),
            'event_id': normalize_id(item.get('MapStageEventId') or item.get('mapStageEventId')),
            'area_unique_name': str(item.get('AreaUniqueName') or item.get('areaUniqueName') or '').strip(),
        })
    return by_msid


def create_map_npc_terrain_extension_lookup(d):
    by_npc = {}
    for item in extract_data_list(d):
        if not isinstance(item, dict):
            continue
        nid = normalize_id(item.get('MapNpcId') or item.get('mapNpcId'))
        tti = safe_int(item.get('TerrainTypeIndex') or item.get('terrainTypeIndex'), 0)
        tci = safe_int(item.get('TerrainCapabilityTypeIndex') or item.get('terrainCapabilityTypeIndex'), 1)
        tn = MAP_NPC_TERRAIN_TYPE_INDEX.get(tti)
        if nid == '0' or not tn:
            continue
        try:
            tier = int(tci or 0)
        ...
        if tier < 1:
            ...
            tier = 3
        by_npc.setdefault(nid, {})[tn] = tier
    return by_npc


def resolve_map_stage_objectives(map_stage_id, lc):
    msid = normalize_id(map_stage_id)
    rows = (map_stage_conditions_by_map_stage or {}).get(msid, [])
    if not rows:
        return []
    ld = get_lang_data(lc)
    tm = ld.get('map_stage_condition_text_map', {}) or {}
    out = []
    seen = set()
    for row in rows:
        lid = normalize_id(row.get('text_lang_id', '0'))
        if lid == '0':
            continue
...
def create_map_npc_lookup(d):
    ...
    for item in extract_data_list(d):
        ...
        entry = {
            ...
            'step_order': safe_int(item.get('StepOrder') or item.get('stepOrder'), 0),
            'is_story_event_boss': bool(item.get('IsStoryEventBoss') or item.get('isStoryEventBoss')),
...
def apply_map_npc_strategy_hints(npc_id, up, cp, hints):
    ...
    for h in hints:
        ...
        def _assign_first_unit_ability():
            if not up:
                return False
        ...
        def _assign_first_char_ability():
            if not cp:
                return False
        ...
        def _assign_targeted_unit_weapon():
            if not up:
                return False
            for w in (up.get('weapons') or []):
                if isinstance(w, dict) and normalize_id(w.get('id')) == tgt:
                    w['strategy_hint_icon'] = url
                    return True
            return False

        def _assign_targeted_unit_ability():
            if not up:
                return False
            for ab in (up.get('abilities') or []):
        ...
        def _assign_targeted_char_ability():
            if not cp:
                return False
            for ab in (cp.get('abilities') or []):
                if _assign_strategy_hint_to_ability_row(ab, url, tgt):
        ...
        if not matched and not targetless:
            matched = (
                _assign_targeted_unit_ability()
                or _assign_targeted_unit_weapon()
                or _assign_targeted_char_ability()
            )
        ...
        if not matched and targetless:
            _assign_first_unit_ability() or _assign_first_unit_weapon() or _assign_first_char_ability()
        elif not matched:
            if typ == 1:
                _assign_first_unit_weapon() or _assign_first_unit_ability()
            elif typ == 3:
...
def _resolve_map_npc_unit_rows(map_npc_id, challenge_stage_id=None):
    """Base m_map_npc_unit rows, or challenge override rows when present."""
    nid = normalize_id(map_npc_id)
    if challenge_stage_id:
        sid = normalize_id(challenge_stage_id)
        ostage = (main_stage_challenge_npc_unit_override_by_stage or {}).get(sid, {})
        if nid in ostage:
            return ostage[nid]
    return map_npc_unit_lookup.get(nid, []) or map_npc_unit_lookup.get(map_npc_id, [])


def _resolve_map_npc_character_rows(map_npc_id, challenge_stage_id=None):
    """Base m_map_npc_character rows, or challenge override rows when present."""
    nid = normalize_id(map_npc_id)
    if challenge_stage_id:
        sid = normalize_id(challenge_stage_id)
        ostage = (main_stage_challenge_npc_character_override_by_stage or {}).get(sid, {})
        if nid in ostage:
            return ostage[nid]
    return map_npc_character_lookup.get(nid, []) or map_npc_character_lookup.get(map_npc_id, [])
...
scenario_stage_data = load_json(os.path.join(BASE_DIR, "m_scenario_stage.json"))
capturable_unit_data = load_json(os.path.join(BASE_DIR, "m_capturable_unit.json"))
...
stage_score_evaluation_data = load_json(os.path.join(BASE_DIR, "m_stage_score_evaluation.json"))
stage_score_evaluation_percent_data = load_json(os.path.join(BASE_DIR, "m_stage_score_evaluation_percent.json"))
stage_recommended_sortie_terrain_data = load_json(os.path.join(BASE_DIR, "m_stage_recommended_sortie_terrain_type.json"))
stage_reinforcement_area_data = load_json(os.path.join(BASE_DIR, "m_stage_reinforcement_area.json"))
stage_event_area_skill_master_data = load_json(os.path.join(BASE_DIR, "m_stage_event_area_skill.json"))
map_stage_event_data = load_json(os.path.join(BASE_DIR, "m_map_stage_event.json"))
...
map_stage_condition_data = load_json(os.path.join(BASE_DIR, "m_map_stage_condition.json"))
map_stage_event_area_data = load_json(os.path.join(BASE_DIR, "m_map_stage_event_area.json"))
map_npc_unit_terrain_extension_data = load_json(os.path.join(BASE_DIR, "m_map_npc_unit_terrain_capability_extension.json"))
...
stage_score_eval_percent_list = create_stage_score_eval_percent_list(stage_score_evaluation_percent_data) if stage_score_evaluation_percent_data else []
stage_recommended_sortie_by_terrain = create_stage_recommended_sortie_by_terrain(stage_recommended_sortie_terrain_data) if stage_recommended_sortie_terrain_data else {}
stage_reinforcement_area_by_stage = create_stage_reinforcement_area_by_stage(stage_reinforcement_area_data) if stage_reinforcement_area_data else {}
stage_event_area_skill_lookup = create_stage_event_area_skill_lookup(stage_event_area_skill_master_data) if stage_event_area_skill_master_data else {}
map_stage_event_lookup = create_map_stage_event_lookup(map_stage_event_data) if map_stage_event_data else {}
STAGE_SCORE_EVAL_RANK_LABELS = {3: 'S', 2: 'A', 1: 'B'}
...
main_scenario_stage_challenge_map = create_main_scenario_stage_challenge_map(main_scenario_stage_challenge_data) if main_scenario_stage_challenge_data else {}
capturable_units_by_stage = create_capturable_units_by_stage(capturable_unit_data) if capturable_unit_data else {}
...
map_stage_conditions_by_map_stage = create_map_stage_conditions_by_map_stage(map_stage_condition_data) if map_stage_condition_data else {}
...
for lang_code, paths in LANG_PATHS.items():
    ...
    map_stage_condition_lang = load_json(os.path.join(lang_dir, "m_map_stage_condition.json"))
    stage_event_area_skill_lang = load_json(os.path.join(lang_dir, "m_stage_event_area_skill.json"))
    ...
    scenario_stage_lang_text = load_json(os.path.join(lang_dir, "m_scenario_stage.json"))
    ...
    scenario_stage_text_map = create_lang_text_map(scenario_stage_lang_text) if scenario_stage_lang_text else {}
    ...
    map_stage_condition_text_map = {}
    for item in extract_data_list(map_stage_condition_lang):
        if isinstance(item, dict):
            lid = normalize_id(item.get('id') or item.get('Id'))
            val = item.get('value') or item.get('Value') or item.get('text') or item.get('Text')
            if lid != '0' and val:
                map_stage_condition_text_map[lid] = str(val).replace("\\n", "\n")
    stage_event_area_skill_name_map = {}
    stage_event_area_skill_desc_map = {}
    if stage_event_area_skill_lang:
        for item in extract_data_list(stage_event_area_skill_lang):
            if not isinstance(item, dict):
                continue
            lid = normalize_id(item.get('id') or item.get('Id'))
            val = item.get('value') or item.get('Value') or item.get('text') or item.get('Text')
            ...
            if str(lid).startswith('131200'):
                stage_event_area_skill_desc_map[lid] = text
            else:
                stage_event_area_skill_name_map[lid] = text
...
def _resolve_lang_text_from_maps(ld, lang_id, map_keys):
    ...
    for key in map_keys:
        tm = (ld or {}).get(key, {}) or {}
        if lid in tm:
            return str(tm.get(lid) or '').strip()
        for k, v in tm.items():
            if str(k).endswith(lid):
...
def resolve_stage_drop_pools(stage_master_id, lc):
    sm = (stage_map or {}).get(normalize_id(stage_master_id), {})
    dsid = normalize_id(sm.get('stage_drop_set_id', '0'))
    if dsid == '0':
        return []
    pools = []
    for drop_row in (stage_drop_by_set or {}).get(dsid, []):
        cset = normalize_id(drop_row.get('content_set_id', '0'))
        entries = []
        for crow in (stage_drop_content_by_set or {}).get(cset, []):
            rsid = normalize_id(crow.get('reward_set_id', '0'))
            rewards = _decorate_reward_rows(_resolve_reward_rows_from_set_id(rsid), lc) if rsid != '0' else []
            if not rewards:
                continue
            entries.append({
                'weight': safe_int(crow.get('weight'), 0),
                'rewards': rewards,
            })
        if entries:
            pools.append({
                'lottery_count': safe_int(drop_row.get('lottery_count'), 1),
                'entries': entries,
            })
    return pools


def resolve_stage_score_evaluation(stage_master_id, lc):
    sm = (stage_map or {}).get(normalize_id(stage_master_id), {})
    set_id = normalize_id(sm.get('stage_score_evaluation_set_id', '0'))
    if set_id == '0':
        return None
    thresholds = []
    for row in (stage_score_eval_by_set or {}).get(set_id, []):
        step = safe_int(row.get('step'), 0)
        thresholds.append({
            'step': step,
            'score': safe_int(row.get('score'), 0),
            'rank_label': STAGE_SCORE_EVAL_RANK_LABELS.get(step, f'#{step}'),
        })
    weights = []
    for row in (stage_score_eval_percent_list or []):
        ti = safe_int(row.get('type_index'), 0)
        label_key = f'stage_score_type_{ti}'
        weights.append({
            'type_index': ti,
            'percent': safe_int(row.get('percent'), 0),
            'label': get_ui_label(lc, label_key),
        })
    return {
        'set_id': set_id,
        'base_score': safe_int(sm.get('base_score'), 0),
        'thresholds': thresholds,
        'weights': weights,
    }


def resolve_recommended_sortie_terrain(stage_master_id, lc):
    sm = (stage_map or {}).get(normalize_id(stage_master_id), {})
    terrain_idx = normalize_id(sm.get('terrain_type_index', '0'))
    sortie_idx = normalize_id(sm.get('sortie_terrain_type_index', '0'))
    rec_names = []
    for rti in (stage_recommended_sortie_by_terrain or {}).get(terrain_idx, []):
        name = resolve_stage_terrain_name(rti, lc)
        if name and name not in rec_names:
            rec_names.append(name)
    sortie_name = resolve_stage_terrain_name(sortie_idx, lc) if sortie_idx != '0' else ''
    if not rec_names and not sortie_name:
        return None
    return {
        'stage_terrain': resolve_stage_terrain_name(terrain_idx, lc),
        'recommended': rec_names,
        'sortie_terrain': sortie_name,
    }


def resolve_stage_event_area_skill(event_id, lc):
    eid = normalize_id(event_id)
    if eid == '0':
        return None
    mev = (map_stage_event_lookup or {}).get(eid, {})
    skill_id = normalize_id(mev.get('target_id', '0'))
    if skill_id == '0':
        return None
    skill = (stage_event_area_skill_lookup or {}).get(skill_id, {})
    if not skill:
        return None
    ld = get_lang_data(lc)
    name = _resolve_lang_text_from_maps(ld, skill.get('name_lang_id'), ('stage_event_area_skill_name_map', 'lang_text_map'))
    desc = _resolve_lang_text_from_maps(ld, skill.get('desc_lang_id'), ('stage_event_area_skill_desc_map', 'lang_text_map'))
    icon = find_trait_icon(skill.get('resource_id', '')) or ''
    if not name and not desc:
        return None
    return {'id': skill_id, 'name': name, 'description': desc, 'icon': icon}


def enrich_map_event_areas(areas, lc):
    out = []
    for area in (areas or []):
        row = dict(area)
        skill = resolve_stage_event_area_skill(row.get('event_id', '0'), lc)
        if skill:
            row['event_skill'] = skill
        out.append(row)
    return out
...
def resolve_scenario_stage_name(ld, title_lang_id, stage_id):
    ...
    txt = (ld.get('scenario_stage_text_map') or {}).get(lid, '')
    if txt:
        return str(txt).strip()
    return f"Unknown ({stage_id})"

def resolve_challenge_series_name(ld, series_challenge_id):
    sc = (main_stage_series_challenge_map or {}).get(normalize_id(series_challenge_id), {})
    sid = normalize_id(sc.get('main_stage_series_id', '0'))
    if sid == '0':
        return ''
...
def resolve_challenge_capturable_units(stage_id, lc):
    ld = get_lang_data(lc)
    out = []
    for cu in (capturable_units_by_stage or {}).get(normalize_id(stage_id), []):
        uid = normalize_id(cu.get('main_unit_id', '0'))
        if uid == '0':
            continue
        uinfo = unit_info_map.get(uid, {}) or {}
        ulid = (ld.get('unit_id_map') or {}).get(uid, '')
        uname = (ld.get('unit_text_map') or {}).get(ulid, '') if ulid else ''
        if not uname:
            uname = f"Unknown ({uid})"
        ...
        out.append({
            'unit_id': uid,
            'name': uname,
            'thum': find_list_thumb(uinfo.get('resource_ids', []), uid, 'images/unit_portraits') or '',
            'rarity': RARITY_MAP.get(uri, 'N'),
            'rarity_icon': RARITY_ICON_MAP.get(uri, ''),
...
def _map_stage_deployed_character_ids(npc_entries, challenge_stage_id=None):
    ids = set()
    if not npc_entries:
        return ids
    for npc in npc_entries:
        ...
        if not nid:
            continue
        nc = _resolve_map_npc_character_rows(nid, challenge_stage_id=challenge_stage_id)
        if not nc:
            continue
...
def _map_stage_deployed_unit_ids(npc_entries, challenge_stage_id=None):
    ids = set()
    if not npc_entries:
        return ids
    for npc in npc_entries:
        ...
        if not nid:
            continue
        nu = _resolve_map_npc_unit_rows(nid, challenge_stage_id=challenge_stage_id)
        if not nu:
            continue
...
def accumulate_npc_unit_condition_pct_by_unit(npc_entries, lc, challenge_stage_id=None):
    ...
    stage_uids = _map_stage_deployed_unit_ids(npc_entries, challenge_stage_id=challenge_stage_id)
    for npc in npc_entries:
        nc = _resolve_map_npc_character_rows(npc.get('id'), challenge_stage_id=challenge_stage_id)
        if not nc:
            continue
...
def accumulate_npc_map_unit_stat_bonuses(npc_entries, lc, merge_mode='unconditional', challenge_stage_id=None):
    """Per stage with m_map_npc placement: squad-wide % from all NPCs' unit + character abilities + each NPC's own non-squad lines.

    ``get_stage`` uses this path for Eternal, Tower (via ``stage_master_id``), Score Attack, Special Event, and
    Challenge Stages (via ``challenge_stage_id`` override rows) — identical rules everywhere.
    ...
    pilot_char_ms_pct = {}
    ld = get_lang_data(lc) or {}
    stage_cids = _map_stage_deployed_character_ids(npc_entries, challenge_stage_id=challenge_stage_id)
    for npc in npc_entries:
        nid = npc['id']
        nu = _resolve_map_npc_unit_rows(nid, challenge_stage_id=challenge_stage_id)
        if not nu:
...
def get_warship_footprint_cells(x, y, direction, for_buff=False):
    ...
    if d in ('1', '3'):
        # East: pivot at anchor (ox=ax); west: ox=ax−ForwardArea. Icon uses (min_x, max_y) for both.
...
def browse_filters_pool_signature(args, entity=None):
    ...
    if ent == 'supporters':
        ...
        if ent == 'units':
            ...
            parts.append(args.get('map_weapon_range', '').strip())
            parts.append(normalize_filter_combine_op(args.get('map_weapon_range_op'), 'and'))
...
def _precompute_weapon_range_values_present(subset, stat_mode='normal', cond_active=False):
    ld = LANG_DATA.get('EN')
    if not ld:
        return frozenset()
    out = set()
    for uid in unit_info_map:
        for rv in _unit_weapon_subset_effective_ranges(uid, ld, 'EN', stat_mode, subset, cond_active):
            out.add(str(int(rv)))
    return frozenset(out)
...
WEAPON_RANGE_SSP_EX_SSP_VALUES_PRESENT = _precompute_weapon_range_values_present('ssp_ex', 'ssp')
...
WEAPON_RANGE_ALL_SSP_VALUES_PRESENT = _precompute_weapon_range_values_present('all', 'ssp')
WEAPON_RANGE_NON_MAP_COND_VALUES_PRESENT = _precompute_weapon_range_values_present('non_map', 'normal', True)
WEAPON_RANGE_NON_MAP_SSP_COND_VALUES_PRESENT = _precompute_weapon_range_values_present('non_map', 'ssp', True)
WEAPON_RANGE_ALL_SSP_COND_VALUES_PRESENT = _precompute_weapon_range_values_present('all', 'ssp', True)
WEAPON_RANGE_SSP_EX_COND_VALUES_PRESENT = _precompute_weapon_range_values_present('ssp_ex', 'normal', True)
WEAPON_RANGE_SSP_EX_SSP_COND_VALUES_PRESENT = _precompute_weapon_range_values_present('ssp_ex', 'ssp', True)


def resolve_npc_map_terrain(unit_id, map_npc_id, lc):
    uid = normalize_id(unit_id, '0')
    info = unit_info_map.get(uid, {}) if uid != '0' else {}
    levels = dict(_unit_base_terrain_levels(info))
    overrides = (map_npc_terrain_extension_lookup or {}).get(normalize_id(map_npc_id), {})
    for tn, tier in overrides.items():
        levels[tn] = _terrain_tier_norm(tier)
    terrain = []
    for tn in UNIT_TERRAIN_NAMES:
        lv = _terrain_tier_norm(levels.get(tn, 1))
        terrain.append({
            'name': tn,
            'symbol': TERRAIN_SYMBOLS.get(str(lv), TERRAIN_SYMBOLS['1']),
            'level': lv,
            'type_icon': f"/static/images/Terrain/{TERRAIN_TYPE_ICON_MAP.get(tn, '')}" if TERRAIN_TYPE_ICON_MAP.get(tn) else '',
            'level_icon': f"/static/images/Terrain/{TERRAIN_LEVEL_ICON_MAP.get(lv, TERRAIN_LEVEL_ICON_MAP[1])}",
            'map_override': tn in overrides,
        })
    return terrain
...
def _unit_form_matches_role_filter(uid, info, role_filter):
    if not role_filter:
        return False
    return str(info.get('role', '0')) in role_filter


def unit_matches_role_filter(uid, info, role_filter):
    """Browse role filter: pass when any transform/Refit family form matches."""
    if role_filter is None:
        return True
    for fid in _unit_ids_for_terrain_filter(uid, info):
        finfo = unit_info_map.get(fid)
        if finfo and _unit_form_matches_role_filter(fid, finfo, role_filter):
            return True
    return False


def _unit_role_filter_display_id(uid, info, role_filter):
    """Browse list thumb/name: prefer the transform alternate when it alone satisfies the role filter."""
    if not role_filter:
        return uid
    if _unit_form_matches_role_filter(uid, info, role_filter):
        return uid
    for fid in _unit_ids_for_terrain_filter(uid, info):
        if fid == uid:
            continue
...
def _unit_weapon_range_non_map_filter_display_id(
        uid, info, ld, lc, want_filter, stat_mode='normal', combine='and', subset='non_map', cond_active=False):
    """Browse list thumb/name: prefer the transform alternate when it alone satisfies the non-MAP weapon range filter."""
    if not want_filter:
        return uid
    if _unit_form_matches_weapon_range_non_map_filter(
            uid, ld, lc, want_filter, stat_mode, combine, subset, cond_active):
        return uid
    for fid in _unit_ids_for_terrain_filter(uid, info):
        if fid == uid:
            continue
...
def unit_passes_browse_pool_filters(
    ...
    *, q_scope='name_id', apply_lineage=True, apply_series=True, apply_ability=True, apply_terrain=True, apply_weapon_debuff=True,
    ...
    weapon_range_non_map_ssp_ex=False,
    map_weapon_range_filter=None, apply_map_weapon_range=True,
    lineage_combine='and', series_combine='or', ability_combine='and', terrain_combine='and', weapon_debuff_combine='and', weapon_range_combine='and', weapon_range_non_map_combine='and', map_weapon_range_combine='and',
    cond_active=False,
):
...
def lineages_for_unit_browse_filtered(ld, lc, args):
    ...
    map_weapon_range_filter = parse_map_weapon_range_filter(args.get('map_weapon_range', '').strip())
    cond_active = args.get('cond', '').strip().lower() in ('1', 'true', 'yes')
...
def list_units():
    ...
    map_weapon_range_arg = request.args.get('map_weapon_range', '').strip()
    map_weapon_range_filter = parse_map_weapon_range_filter(map_weapon_range_arg)
        ...
        if weapon_debuff_filter:
            if uid not in _debuff_memo:
                ...
                range_dk = collect_unit_weapon_range_debuff_keys(
                    uid, _ld_f, _lc_f, stat_mode, cond_active=cond_list,
                )
...
_PUBLISHED_BANNER_POOL_VOTES_FILE = os.path.join(app_dir, 'data', 'published', 'banner_pool_votes.json')
...
def _path_is_writable_dir(path):
    ...
    try:
        ...
        probe = os.path.join(path, '.ggen_write_probe')
...
def _banner_pool_votes_github_repo():
    return (os.environ.get('GGEN_BANNER_VOTES_GITHUB_REPO') or os.environ.get('GITHUB_REPOSITORY') or '').strip()


def _banner_pool_votes_github_path():
...
def _banner_pool_votes_import_url():
    explicit = (os.environ.get('GGEN_BANNER_VOTES_IMPORT_URL') or '').strip()
    if explicit:
        return explicit
    repo = _banner_pool_votes_github_repo()
...
def _banner_pool_votes_on_railway():
...
def _banner_pool_votes_pick_richer(a, b):
    """Prefer the snapshot with more recorded votes (never replace rich data with empty)."""
    if not _banner_pool_votes_has_data(a):
        return b if _banner_pool_votes_has_data(b) else (a if isinstance(a, dict) else {})
    if not _banner_pool_votes_has_data(b):
        return a
    return b if _banner_pool_votes_vote_count(b) > _banner_pool_votes_vote_count(a) else a
    ...
    try:
        ...
        if cat == 'challenge_stage':
            for sid, ch in (main_scenario_stage_challenge_map or {}).items():
                ...
                if sq:
                    searchable = f"{sid} {sname} {sn} {series_name}".lower()
                    if not search_row_matches_query(sq, searchable, None, entity_id=sid):
                        continue
                sm = stage_map.get(sid, {})
                dti = safe_int(ssc.get('stage_difficulty_type_index'), safe_int(sm.get('stage_difficulty_type_index'), 1))
...
def get_stage(stage_id):
    try:
        ...
        if is_score_attack:
            ...
            ssc = (scenario_stage_map or {}).get(stage_id, {})
            mc = (main_stage_challenge_map or {}).get(stage_id, {})
            mmeta = map_stage_meta_by_stage_id.get(stage_id, {}) if map_stage_meta_by_stage_id else {}
            est = {
                'stage_number': safe_int(ch.get('stage_number'), 0),
                'title_name_lang_id': ssc.get('title_name_lang_id', '0'),
                'thumbnail_resource_id': ssc.get('thumbnail_resource_id', ''),
                'stage_difficulty_type_index': safe_int(ssc.get('stage_difficulty_type_index'), safe_int(mmeta.get('stage_difficulty_type_index'), 1)),
                'display_unit_id': '0',
                'challenge_series_id': normalize_id(ch.get('main_stage_series_challenge_id', '0')),
                'group1_sortie_count': safe_int(mc.get('group1_sortie_count'), 0),
                'group2_sortie_count': safe_int(mc.get('group2_sortie_count'), 0),
                'group1_sortie_restriction_set_id': normalize_id(mc.get('group1_sortie_restriction_set_id', '0')),
                'group2_sortie_restriction_set_id': normalize_id(mc.get('group2_sortie_restriction_set_id', '0')),
        ...
        if mse:
            ...
            squad_tb, self_tb, squad_by_source_tb, pilot_char_ms_pct_tb = accumulate_npc_map_unit_stat_bonuses(
                nt, lc, challenge_stage_id=_challenge_sid)
            squad_tb_c, self_tb_c, _, pilot_char_ms_pct_tb_c = accumulate_npc_map_unit_stat_bonuses(
                nt, lc, merge_mode='conditional_title_gated', challenge_stage_id=_challenge_sid)
            unit_cond_pct_by_uid = accumulate_npc_unit_condition_pct_by_unit(nt, lc, challenge_stage_id=_challenge_sid)
            stage_unit_ids = _map_stage_deployed_unit_ids(nt, challenge_stage_id=_challenge_sid)
            for npc in nt:
                nid = npc['id']; nid_norm = normalize_id(nid)
                nu = _resolve_map_npc_unit_rows(nid_norm, challenge_stage_id=_challenge_sid)
                nc = _resolve_map_npc_character_rows(nid_norm, challenge_stage_id=_challenge_sid)
                ...
                if ue:
                    ...
                    up['level'] = safe_int(ue.get('level'), 0)
                    up['terrain'] = resolve_npc_map_terrain(upuid, nid_norm, lc)
                    if len(nu) > 1:
                        alt_rows = []
                        for alt_ue in nu[1:]:
                            alt_uid = normalize_id(alt_ue.get('unit_id', '0'))
                            if alt_uid == '0':
                                continue
                            alt_lid = ld.get('unit_id_map', {}).get(alt_uid, '')
                            alt_name = ld.get('unit_text_map', {}).get(alt_lid, f'Unit {alt_uid}') if alt_lid else f'Unit {alt_uid}'
                            alt_rows.append({'unit_id': alt_uid, 'name': alt_name, 'level': safe_int(alt_ue.get('level'), 0)})
                        if alt_rows:
                            up['alternate_loadouts'] = alt_rows
            ...
            for npc in nt:
                ...
                if bid != '0':
                    npc_oaid = 1
                    nu_b = _resolve_map_npc_unit_rows(normalize_id(npc['id']), challenge_stage_id=_challenge_sid)
                    if nu_b:
