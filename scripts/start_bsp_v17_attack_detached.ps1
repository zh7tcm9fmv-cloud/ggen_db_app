# Start BSP v17 Attack-only rebuild (Supporters-only boards) detached.
# Survives IDE close. Logs to data/published/bsp_v17_detached.log
# Resume from last checkpoint; auto-loop on crash.
# Non-Attack units keep using v16 fallback until/unless rebuilt.
...
$Log = Join-Path $Published 'bsp_v17_detached.log'
$PidFile = Join-Path $Published 'bsp_v17_detached.pid'
...
Add-Content -Path '$Log' -Value "`n=== detached BSP v17 Attack start `$stamp ===`n" -Encoding utf8
& '$Py' -u '$Script' --loop --workers 3 --checkpoint 10 --unit-role 1 *>> '$Log' 2>&1
"@
...
Write-Host "BSP v17 Attack detached PID=$($p.Id) (survives Cursor close)"
Write-Host "Log: $Log"
